"""Interactive setup helpers used by ``kagweb init``.
...
from kagweb.services.llm.config import get_token_limit_kwargs
from kagweb.services.provider_registry import PROVIDERS, ProviderSpec, find_by_name
...
def _all_providers_except(featured: set[str]) -> list[ProviderSpec]:
    ...
    return [
        ...
        if spec.name not in featured and not spec.is_oauth  # OAuth flows use `kagweb login`
    ]

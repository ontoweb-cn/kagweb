...
```bash
kagweb run <capability> <message> [options]
```
...
```bash
kagweb session list [--limit 20]
kagweb session show <id>
...
```bash
kagweb provider login openai-codex      # 执行 OpenAI Codex OAuth 登录
...
`openai-codex` 使用 KAGWeb 自己的独立 OAuth 流程登录。它不需要 `OPENAI_API_KEY`，也不会读取或同步本机 `~/.codex`；凭据保存在 `data/system/user-secrets/<owner>/private/openai-codex/`（沙箱访问不到的目录），与 Web 设置页共用。

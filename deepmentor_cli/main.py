"""CLI entry point for the standalone ``deepmentor-cli`` package."""
...
app = typer.Typer(
    name="deepmentor",
    help="DeepMentor CLI – agent-first interface for capabilities, tools, and knowledge.",
...
def run_capability(
    ...
    from deepmentor.app import DeepMentorApp
...
def start(
    ...
    detach: bool = typer.Option(
        False,
        "--detach",
        help="Run outside the current console; stop later with `deepmentor stop`.",
    ),
    ...
    from deepmentor.runtime.launcher import start as start_web
...
def stop(
    ...
    """Stop a DeepMentor launcher started with ``--detach``."""
    from deepmentor.runtime.launcher import stop as stop_web
...
def serve(
    ...
    """Start the DeepMentor API server."""
    import asyncio
    import sys

    set_mode(RunMode.SERVER)
    if port is None:
        from deepmentor.services.setup import get_backend_port

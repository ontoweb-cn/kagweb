...
from deepmentor.runtime.home import DEEPMENTOR_HOME_ENV, get_runtime_home

from . import init_wizard as wiz


def _reset_runtime_singletons() -> None:
    """Drop cached service instances so the new DEEPMENTOR_HOME takes effect.

    ``deepmentor init`` may pass ``--home`` to target a different workspace; the
...
def run_init(*, cli_only: bool = False, home: str | Path | None = None) -> None:
    ...
    from deepmentor.services.config import get_model_catalog_service, get_runtime_settings_service

...
`deepmentor init --cli` 和普通 `deepmentor init` 使用同一套 `data/user/settings/` 配置目录；区别是 `--cli` 不询问 Web 后端/前端端口，仍会创建 `system.json`、`auth.json`、`integrations.json`、`model_catalog.json`、`main.yaml` 和 `agents.yaml`，并继续询问 LLM 配置。Embedding 配置默认跳过；如果要使用 `deepmentor kb ...` 或 RAG，请在向导里选择配置 embedding，或稍后编辑 `data/user/settings/model_catalog.json`。
...
```bash
deepmentor run <capability> <message> [options]
```
...
```bash
# 对话
deepmentor run chat "什么是傅里叶变换？" -l zh

# 深度解题
deepmentor run deep_solve "证明 n^3-n 能被 6 整除" -t rag --kb math-textbook

# 简要回答
deepmentor run deep_solve "求 sin(x) 的导数" --config detailed_answer=false

# 智能出题
deepmentor run deep_question "线性代数" --config num_questions=5 --config difficulty=hard

# 仿真出题
deepmentor run deep_question "模拟考试" --config mode=mimic --config paper_path=exam.json

# 深度研究
deepmentor run deep_research "Transformer 最新进展" \
  --config-json '{"mode":"report","depth":"deep","sources":["web","papers"]}'

# 可视化
deepmentor run visualize "画出注意力机制的数据流图" --config render_mode=mermaid

# 数学动画
deepmentor run math_animator "展示正弦函数变换" --config quality=high

# 掌握式学习
deepmentor run mastery_path "带我系统掌握特征值和特征向量"

# JSON 输出（适合 agent 解析）
deepmentor run deep_solve "求解 x^2=4" -f json
```
...
```bash
deepmentor serve [--host 0.0.0.0] [--port 8001] [--reload]
```

`deepmentor serve` 需要完整 Web/API 依赖；如果你是通过本地 `./packaging/deepmentor-cli` 安装的 CLI-only 包，请先卸载本地 CLI 包并切换到 `pip install -U deepmentor`。
...
```bash
deepmentor kb list                                # 列出所有知识库
deepmentor kb info <name>                         # 查看详情
deepmentor kb create <name> --doc file.pdf        # 创建并导入文档
deepmentor kb create <name> --docs-dir ./docs/    # 从目录批量导入
deepmentor kb add <name> --doc extra.pdf          # 追加文档
deepmentor kb set-default <name>                  # 设为默认
deepmentor kb search <name> "查询内容"             # 搜索
deepmentor kb delete <name> --force               # 删除
```

### `session` — 会话

```bash
deepmentor session list [--limit 20]
deepmentor session show <id>
deepmentor session open <id>                      # 进入 REPL 继续对话
deepmentor session rename <id> --title "新标题"
deepmentor session delete <id>
```

### `notebook` — 笔记本

```bash
deepmentor notebook list
deepmentor notebook create "笔记" --description "描述"
deepmentor notebook show <id>
deepmentor notebook add-md <id> ./notes.md
deepmentor notebook replace-md <id> <record_id> ./updated.md
deepmentor notebook remove-record <id> <record_id>
```
...
deepmentor memory clear --force
```

### `plugin` — 插件信息

```bash
deepmentor plugin list                            # 查看所有工具和 capability
deepmentor plugin info <name>                     # 查看详情
```
...
```bash
deepmentor provider login openai-codex      # 执行 OpenAI Codex OAuth 登录
...
`openai-codex` 使用 DeepMentor 自己的独立 OAuth 流程登录。它不需要 `OPENAI_API_KEY`，也不会读取或同步本机 `~/.codex`；凭据保存在 `data/system/user-secrets/<owner>/private/openai-codex/`（沙箱访问不到的目录），与 Web 设置页共用。
...
```bash
# 1. 创建知识库
deepmentor kb create calculus --doc 微积分教材.pdf

# 2. 用知识库解题
deepmentor run deep_solve "求 ∫sin(x)cos(x)dx" -t rag --kb calculus -l zh

# 3. 基于知识库出题
deepmentor run deep_question "微积分" --kb calculus \
  --config num_questions=5 --config difficulty=medium -l zh

# 4. 深度研究某课题
deepmentor run deep_research "注意力机制演进" \
  --config-json '{"mode":"report","depth":"deep","sources":["papers","web"]}' -l zh

# 5. 查看会话记录
deepmentor session list
```

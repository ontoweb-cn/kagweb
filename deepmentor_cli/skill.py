...
``eduhub`` (DeepMentor's native open skill registry). Installs run the full
...
def _resolve_token(explicit: str | None, hub: str) -> str | None:
    ...
    from deepmentor.services.skill import credentials

    return (
        explicit
        or os.environ.get("DEEPMENTOR_HUB_TOKEN")
...
def _summary_table(
    ...
    *,
    ...
    from deepmentor.services.skill.taxonomy import domain_label, track_label
...
def register(app: typer.Typer) -> None:
    ...
    def skill_search(
        ...
        """Search a skill hub."""
        from deepmentor.services.skill.hub import HubError, default_hub, get_hub_provider
    ...
    def skill_login(
        ...
        from deepmentor.services.skill import credentials
        from deepmentor.services.skill.hub import HubError, default_hub, get_hub_provider
        from deepmentor.services.skill.taxonomy import Option
    ...
    def skill_logout(
        ...
        """清除本地保存的某个 hub 登录令牌。"""
        from deepmentor.services.skill import credentials
        from deepmentor.services.skill.hub import default_hub
    ...
    def skill_publish(
        ...
        token: str | None = typer.Option(
            None, "--token", help="Publish token; else env / `deepmentor skills login`."
        ),
        ...
        from deepmentor.services.skill.hub import (
            HubError,
            default_hub,
            preflight_skill_dir,
            publish_to_hub,
        ...
        from deepmentor.services.skill.service import SkillImportError
        ...
        if not pre.ok:
            ...
            console.print("  [dim]格式规范见 https://eduhub.deepmentor.info/skill-format.md[/]")
            raise typer.Exit(code=1)
        console.print(
            f"[green]✓[/] 格式预检通过 "
        ...
        if not tok:
            console.print(
                "[bold red]未登录。[/] 运行 [bold]deepmentor skills login[/] 完成浏览器授权，"
                "或用 --token / $DEEPMENTOR_HUB_TOKEN 传入令牌。"
            )
            raise typer.Exit(code=1)
        ...
        console.print(
            f"  [dim]install with:[/] deepmentor skill install {outcome.hub}:{outcome.slug}"
        )
    ...
    def skill_list() -> None:
        ...
        from deepmentor.services.skill.service import get_skill_service

# ============================================
# KAGWeb Multi-Stage Dockerfile
# ============================================
# This Dockerfile builds a production-ready image for KAGWeb
...
# Subpath deployments (e.g. https://ai.wust.edu.cn/kagweb): pass
# --build-arg NEXT_PUBLIC_BASE_PATH=/kagweb. It is baked at build time
...
COPY kagweb/__version__.py /app/kagweb/__version__.py
...
# KAGWEB_API_BASE_URL (exported by the entrypoint on every start).
...
# Labels
LABEL maintainer="KAGWeb Team" \
      description="KAGWeb: AI-Powered Personalized Learning Assistant"
...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    NODE_ENV=production \
    KAGWEB_IGNORE_PROCESS_ENV_OVERRIDES=1
...
# (system.json, default on), exported to KAGWEB_SANDBOX_ALLOW_SUBPROCESS at
...
# (KAGWEB_SANDBOX_RUNNER_URL), which build_backend() prefers.
...
# user). Each child (backend/frontend) is dropped to this `kagweb` user via
# the per-program `user=kagweb` directive, so the app processes stay
# non-root in either runtime. UID 1000 also matches the host user under
# keep-id with a bind mount on ./data.
RUN groupadd --system --gid 1000 kagweb \
    && useradd --system --uid 1000 --gid 1000 --no-create-home --shell /usr/sbin/nologin kagweb \
    && chown -R kagweb:kagweb /app/data /app/web/.next
...
# kagweb user (UID 1000) via per-program `user=kagweb`; see the note on
...
# the selected file over programs.conf when KAGWEB_COMPONENT is set to
...
# runAsUser: 1000) rewrites programs.conf when KAGWEB_COMPONENT selects a
# split deployment, so the selectable files must be owned by that user.
RUN chown kagweb:kagweb /etc/supervisor/conf.d \
...
WS_MAX_SIZE=$(python -c "from kagweb.services.config import get_ws_max_size; print(get_ws_max_size())" 2>/dev/null || echo 16777216)
KEEP_ALIVE=$(python -c "from kagweb.services.config import HTTP_KEEP_ALIVE_TIMEOUT; print(HTTP_KEEP_ALIVE_TIMEOUT)" 2>/dev/null || echo 300)
exec python -m uvicorn kagweb.api.main:app --host ${BACKEND_HOST} --port ${BACKEND_PORT} --workers ${BACKEND_WORKERS} --no-access-log --ws-max-size ${WS_MAX_SIZE} --timeout-keep-alive ${KEEP_ALIVE}
EOF
...
# the configured backend at request time, reading KAGWEB_API_BASE_URL
...
export KAGWEB_IGNORE_PROCESS_ENV_OVERRIDES=1
...
# only runs when /app/data's owner actually differs from the kagweb user
...
if [ "$(id -u)" = "0" ] && [ "$(stat -c %u /app/data 2>/dev/null || echo 0)" != "$(id -u kagweb)" ]; then
    chown -R kagweb:kagweb /app/data 2>/dev/null || true
fi
...
#
#   environment:
#     KAGWEB_EXTRAS: "math-animator,partners"
#     KAGWEB_APT_PACKAGES: "ffmpeg"
#
...
if [ -n "${KAGWEB_APT_PACKAGES:-}" ]; then
    echo "🔧 Ensuring system packages: ${KAGWEB_APT_PACKAGES}"
    apt_missing=""
    for pkg in $(echo "${KAGWEB_APT_PACKAGES}" | tr ',' ' '); do
...
if [ -n "${KAGWEB_EXTRAS:-}" ]; then
    echo "🔧 Ensuring Python extras: ${KAGWEB_EXTRAS}"
    python /app/scripts/install_extras.py "${KAGWEB_EXTRAS}" || true
    chown -R kagweb:kagweb "$PIP_CACHE_DIR" 2>/dev/null || true
fi

echo "⚙️  Loading runtime JSON settings..."
eval "$(python - <<'PY'
import shlex
from kagweb.services.config import export_runtime_settings_to_env
...
# kagweb/services/config/runtime_settings.py). web/proxy.ts reads them at
...
# `kagweb start` paths stay in sync.
echo "📌 API Base URL (proxy): ${KAGWEB_API_BASE_URL:-http://localhost:${BACKEND_PORT}}"
echo "📌 Auth enabled: ${KAGWEB_AUTH_ENABLED:-false}"
...
# Split-deployment switch: KAGWEB_COMPONENT selects which supervisord
...
# with KAGWEB_COMPONENT=backend / frontend and gets a single-program
...
COMPONENT=${KAGWEB_COMPONENT:-all}
case "${COMPONENT}" in
    all) : ;;
    backend|frontend)
        ...
        ;;
    *)
        echo "ERROR: KAGWEB_COMPONENT must be all, backend or frontend (got: ${COMPONENT})" >&2
        exit 1
        ;;
esac
...
# and rootless-keep-id runtimes; children drop to the kagweb user via
...
COPY --chown=kagweb:kagweb --from=frontend-builder /app/web ./web

# `next dev` runs as the unprivileged kagweb user (via `user=kagweb` in
...
RUN rm -rf /app/web/.next \
    ...
    && chown kagweb:kagweb /app/web /app/web/.next

...
<p align="center"><img src="../../assets/figs/logo/logo.png" alt="Logótipo do DeepMentor" height="56" style="vertical-align: middle;">&nbsp;<img src="../../assets/figs/logo/banner.png" alt="DeepMentor" height="48" style="vertical-align: middle;"></p>

# DeepMentor: Tutoria Personalizada Vitalícia
...
[Recursos](#-recursos-principais) · [Começar](#-começar) · [Explorar](#-explorar-o-deepmentor) · [CLI](#️-deepmentor-cli--interface-nativa-de-agentes) · [Ecossistema](#-ecossistema--eduhub-e-a-comunidade-de-skills) · [Comunidade](#-comunidade)

</div>

---

> 🤝 **Damos as boas-vindas a todo tipo de contribuições!** Vote em itens do roadmap ou proponha novos em [`Roadmap`](https://github.com/HKUDS/DeepMentor/issues/498), e consulte o nosso [Guia de Contribuição](../../CONTRIBUTING.md) para a estratégia de branches, padrões de código e como começar.

### 📰 Notícias

- **2026-05-22** 🌐 Site oficial de documentação disponível em [**deepmentor.info**](https://deepmentor.info/) — guias, referências e tours de capacidades num só lugar.
...
- **2026-04-10** 📄 O nosso artigo está agora no arXiv! Leia o [preprint](https://arxiv.org/abs/2604.26962) para saber mais sobre o design e as ideias por trás do DeepMentor.
- **2026-02-06** 🚀 Atingimos 10k estrelas em apenas 39 dias! Um enorme obrigado à nossa incrível comunidade pelo apoio!
- **2026-01-01** 🎊 Feliz Ano Novo! Junte-se ao nosso [Discord](https://discord.gg/eRsjPgMU4t), [WeChat](https://github.com/HKUDS/DeepMentor/issues/78) ou [Discussions](https://github.com/HKUDS/DeepMentor/discussions) — juntos damos forma ao futuro do DeepMentor!
- **2025-12-29** 🎓 DeepMentor está oficialmente lançado!

## ✨ Recursos Principais

O DeepMentor é um espaço de trabalho de aprendizagem nativo de agentes que conecta tutoria, resolução de problemas, geração de quizzes, pesquisa, visualização e prática de domínio num sistema extensível.
...
O DeepMentor inclui quatro caminhos de instalação. Todos partilham um layout de espaço de trabalho: as configurações vivem em `data/user/settings/` sob o diretório a partir do qual é iniciado (ou sob `DEEPMENTOR_HOME` / `deepmentor start --home` se definido explicitamente). Para a aplicação completa, o fluxo recomendado é **escolher um diretório de espaço de trabalho → instalar → `deepmentor init` → `deepmentor start`**.

<details>
<summary><b>Opção 1 — Instalar a partir do PyPI</b> · aplicação web local completa + CLI, sem necessidade de clonar</summary>

Aplicação web local completa + CLI, sem necessidade de clonar. Requer **Python 3.11–3.14** e um runtime **Node.js 20+** no PATH (o servidor standalone Next.js empacotado é iniciado por `deepmentor start`).
...
`deepmentor init` solicita a porta de backend (predefinição `8001`), a porta de frontend (predefinição `3782`), provedor LLM / URL base / chave API / modelo, um provedor de embeddings opcional para Base de Conhecimento / RAG e um provedor de pesquisa opcional para Web Search.

Após `deepmentor start`, abra a URL do frontend impressa no terminal — por predefinição [http://127.0.0.1:3782](http://127.0.0.1:3782). Prima `Ctrl+C` nesse terminal para parar tanto o backend como o frontend. Omitir `deepmentor init` é adequado para um teste rápido; a aplicação arranca com portas predefinidas e configuração de modelo vazia, configure-as depois em **Settings → Models**.
...
`deepmentor start` compila o frontend `web/` local para produção uma vez e reutiliza-o; `--dev` executa o Next.js com HMR. O layout de configuração, as portas e o `Ctrl+C` correspondem à Opção 1.
...
**Servidor de desenvolvimento bloqueado:** se `deepmentor start --dev` reportar um frontend existente que não responde, pare o PID que imprime. Se não houver nenhum processo Next.js em execução, os ficheiros de bloqueio estão desatualizados — remova-os e tente novamente:
...
- `ghcr.io/hkuds/deepmentor:latest` — lançamento estável mais recente
- `ghcr.io/hkuds/deepmentor:<version>` — lançamento exato sem o `v` inicial (por exemplo, `:1.6.3`); os pré-lançamentos recebem apenas a respetiva tag de versão
...
- **Desconectado:** adicione `-d`, depois `docker logs -f deepmentor` para seguir, `docker stop deepmentor` para parar, `docker rm deepmentor` antes de reutilizar o nome. O volume `deepmentor-data` mantém as suas configurações e espaço de trabalho entre reinicializações.
...
`next_public_api_base_external` (e o seu alias `public_api_base`) são aceites como fallbacks de menor precedência. CORS usa **origens** de frontend, não URLs de API. Com a autenticação desativada, o DeepMentor permite origens normais de navegador HTTP/HTTPS por predefinição. Com a autenticação ativada, adicione as origens exatas do frontend:
...
A instalação local de `deepmentor-cli` não inclui ativos web nem dependências de servidor. Mantenha o checkout de fonte por perto — a instalação editável aponta para ele. Para adicionar a aplicação web mais tarde, instale o pacote PyPI (Opção 1) e execute `deepmentor init` + `deepmentor start` a partir do mesmo espaço de trabalho.
...
As skills de escritório integradas — **docx / pdf / pptx / xlsx** — funcionam fazendo com que o modelo escreva um script Python curto (`python-docx`, `reportlab`, `openpyxl`, …), o execute através das ferramentas `exec` / `code_execution` e devolva um URL de download. Essas ferramentas são montadas sempre que um backend de sandbox está ativo. O DeepMentor seleciona o backend configurado mais robusto pela seguinte ordem:

- **Sidecar runner:** `DEEPMENTOR_SANDBOX_RUNNER_URL` encaminha a execução para o serviço endurecido e de privilégios mínimos de `Dockerfile.runner`.
...
O DeepMentor separa o código instalado do seu espaço de trabalho de runtime. Por predefinição, o espaço de trabalho é o diretório onde executa `deepmentor init` / `deepmentor start`; `--home PATH` ou `DEEPMENTOR_HOME` substitui-o. A saída de runtime é o diretório `data` dentro desse espaço de trabalho, pelo que a linha do banner de arranque que começa por `Workspace:` identifica o que deve ser limpo.

1. Pare a aplicação. Prima `Ctrl+C` no terminal que executa `deepmentor start`, ou execute `deepmentor stop [--home PATH]` para um launcher iniciado com `--detach`; pare também quaisquer Partners em execução e contentores Docker desconectados antes de eliminar dados.
...
3. Remova o pacote instalado. Use o comando correspondente à distribuição:
   ...
   Se o ambiente virtual foi criado apenas para o DeepMentor, remova-o através do seu gestor de ambientes. Para uma instalação a partir do código-fonte, desative o ambiente, saia do diretório do código-fonte e execute `git status --short` dentro desse checkout exato. Só mova o checkout para o Lixo/Reciclagem depois de confirmar que não contém trabalho não relacionado ou não submetido.
...
<div align="center">
<img src="../../assets/figs/web-1.6.0/OVERVIEW.png" alt="Página inicial do DeepMentor — o espaço de trabalho Chat com todas as superfícies na barra lateral" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/system%20architecture.png" alt="Arquitetura do sistema DeepMentor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/home/00-overview.png" alt="Espaço de trabalho de chat DeepMentor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/chat-agent-loop.png" alt="Loop de agente de chat DeepMentor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/partners/00-partners%20overview.png" alt="Espaço de trabalho de partners DeepMentor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/partners-architecture.png" alt="Arquitetura de partners DeepMentor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/myagents/00-overview.png" alt="Espaço de trabalho Meus Agentes DeepMentor" width="900">
</div>

Meus Agentes transforma outros agentes em contexto para o DeepMentor, e faz duas coisas distintas. **Conectar um agente ao vivo** — Claude Code, Codex, Antigravity, Kimi, opencode, MiMo Code, Hermes Agent, OpenClaw ou DeepSeek Harness na sua máquina, ou um dos seus Partners — e consultá-lo a partir de dentro de um turno de chat: o DeepMentor *executa* mesmo o outro agente e transmite o seu trabalho para o painel de Atividade via a ferramenta `consult_subagent`. Selecione-o e o respetivo limite de rondas com o chip de Agente, ou filtre a mesma lista de agentes conectados com `@`; a escolha permanece associada à sessão.
...
**Importar conversas anteriores** — traga o seu histórico existente do Claude Code e Codex como agentes nomeados, pesquisáveis e retomáveis. Escolha o histórico do Claude por projeto / diretório de trabalho e o histórico do Codex por data do calendário; atualizar ressincroniza esse âmbito e obtém novas conversas. Referencie uma a partir de um turno do Chat via `+` → Meus Agentes, e o DeepMentor lê-a como uma transcrição de terceiros — permanece *a conversa deles*, não a voz própria do DeepMentor.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/co-writer/00-overview.png" alt="Espaço de trabalho Co-Writer DeepMentor" width="900">
</div>
...
A sua ideia central é a **edição cirúrgica**: selecione um trecho e peça ao DeepMentor para reescrever, expandir ou encurtar. O agente de edição pode fundamentar a alteração numa base de conhecimento ou evidência web e mantém um rasto das suas chamadas de ferramentas. Se o rascunho não tiver mudado durante o trabalho, o resultado substitui diretamente o texto selecionado e continua reversível com **Undo**.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/book/00-book_overview.png" alt="Biblioteca de livros DeepMentor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/knowledge/00-overview.png" alt="Centro de Conhecimento DeepMentor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/learning-space/00-overview.png" alt="Hub do Espaço de Aprendizado DeepMentor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/00-overview.png" alt="Visão geral da memória DeepMentor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/01-3%20layer%20memory%20graph.png" alt="Gráfico de memória de três camadas DeepMentor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/settings/00-setting%20overview.png" alt="Hub de configurações DeepMentor" width="900">
</div>
...
Se o DeepMentor reportar a porta de callback de reserva `1457`, use:
...
A autenticação está **desativada por predefinição** — o DeepMentor corre em modo de utilizador único. Ative-a e uma árvore `data/` aloja um espaço de trabalho de administrador, espaços de trabalho por utilizador isolados e espaços de trabalho de partners lado a lado:
...
## ⌨️ DeepMentor CLI — Interface Nativa de Agentes

Um binário `deepmentor`, duas formas de entrada: um **REPL** interativo para pessoas que vivem no terminal, e **JSON** estruturado para outros agentes que conduzem o DeepMentor como uma ferramenta. As mesmas capacidades, ferramentas e bases de conhecimento de qualquer forma.

<details>
<summary><b>Conduzir você mesmo</b></summary>

`deepmentor chat` abre um REPL interativo e seleciona um modo com `--capability`; `deepmentor run <capability> "<message>"` recebe essa capacidade como primeiro argumento posicional e sai após um turno. Ambos aceitam `--tool`, `--kb` e `--config`.
...
O DeepMentor foi construído para ser *operado por outro agente*. Adicione `--format json` a qualquer `run` e cada turno transmite **NDJSON — um evento por linha** (`content`, `tool_call`, `tool_result`, `done`, …), cada linha marcada com o seu `session_id`. As execuções são seguras sem TTY: uma pausa `ask_user` sem TTY resolve-se automaticamente com uma resposta vazia em vez de bloquear.
...
O repositório inclui um [`SKILL.md`](../../SKILL.md) raiz — um documento de transferência de ~200 linhas que ensina qualquer LLM com uso de ferramentas toda a superfície numa leitura. Passe-o ao Claude Code, Codex ou OpenCode (eles pegam no `SKILL.md` automaticamente), ou envolva `deepmentor run` como uma ferramenta num loop LangChain / AutoGen. Receitas completas: [Agent Handoff](https://deepmentor.info/docs/cli/agent-handoff/).
...
| Comando | Descrição |
|:---|:---|
| `deepmentor init` | Criar ou atualizar `data/user/settings` para o espaço de trabalho atual |
| `deepmentor doctor [--online]` | Verificar se o espaço de trabalho está pronto para iniciar uma sessão; `--online` também sonda o provedor de modelo configurado, `--format json` imprime o relatório |
| `deepmentor start [--home PATH] [--dev] [--detach] [--no-browser]` | Lançar backend + frontend juntos, opcionalmente desconectado ou sem abrir o navegador |
| `deepmentor stop [--home PATH]` | Parar um launcher iniciado com `--detach` |
| `deepmentor serve [--port PORT]` | Iniciar apenas o backend FastAPI |
| `deepmentor run <capability> <message>` | Executar um turno de capacidade único (`chat`, `ask_questions`, `deep_solve`, `deep_question`, `deep_research`, `visualize`, `math_animator`, `mastery_path`, `immersive_reading`, `course_study`, `immersive_watching`); adicionar `--format json` para saída NDJSON |
| `deepmentor chat` | REPL interativo com controlos de capacidade, ferramenta, KB, notebook e histórico |
| `deepmentor partner list/create/start/stop` | Gerir partners ligados por IM |
| `deepmentor kb list/info/create/add/search/set-default/delete/list-sources/sync` | Gerir bases de conhecimento e sincronizar fontes GitHub/web registadas (com comandos para adicionar ou remover fontes) |
| `deepmentor skill search/install/list/remove/login/logout/publish/update` | Gerir habilidades, instalar de hubs e publicar as suas (`eduhub:<slug>` por padrão, veja Ecossistema) |
| `deepmentor memory show/clear` | Inspecionar documentos de memória L2/L3 ou limpar memória L1/toda |
| `deepmentor session list/show/open/rename/delete` | Gerir sessões partilhadas |
| `deepmentor notebook list/create/show/add-md/replace-md/remove-record` | Gerir notebooks a partir de ficheiros Markdown |
| `deepmentor book list/health/refresh-fingerprints` | Inspecionar livros e atualizar impressões digitais de fontes |
| `deepmentor plugin list/info` | Inspecionar ferramentas e capacidades registadas |
| `deepmentor config show` | Imprimir resumo de configuração |
| `deepmentor provider login <provider>` | Autenticação de provedor (`openai-codex` login OAuth; `github-copilot` valida uma sessão de autenticação Copilot existente; `codebuddy` valida a autenticação do SDK CodeBuddy e inicia o login quando necessário) |
...
O pacote de apenas CLI encontra-se em `packaging/deepmentor-cli`. Neste checkout, instale-o a partir de fonte:
...
As skills do DeepMentor usam o formato aberto **Agent-Skills** — uma pasta com um playbook `SKILL.md` (frontmatter YAML + Markdown) e ficheiros de referência opcionais. Nada nisto é específico do DeepMentor, por isso qualquer registo que fale o formato torna-se uma fonte para a sua biblioteca. O DeepMentor inclui o **[EduHub](https://eduhub.deepmentor.info/)** — o nosso próprio registo de skills focado em educação — como hub padrão.

<details>
<summary><b>EduHub — o ecossistema de habilidades do DeepMentor</b></summary>

[**EduHub**](https://eduhub.deepmentor.info/) é o hub da comunidade que o DeepMentor lançou para partilhar skills de agentes orientadas para o ensino — tutores socráticos, construtores de cartões flash, feedback de redações, planos de exames, explicadores de conceitos e muito mais. Está integrado no DeepMentor, por isso não há nada a configurar: um slug simples ou um prefixo `eduhub:` resolve para ele.
...
O EduHub também é um registo independente compatível com ClawHub, por isso agentes que não são o DeepMentor (Claude Code, Codex, …) podem usá-lo diretamente através da CLI `eduhub` — `npx eduhub install socratic-tutor`.
...
- o frontmatter é normalizado para o esquema do DeepMentor e `always:` é **removido**, por isso uma skill descarregada nunca pode forçar-se em cada prompt do sistema;
...
Como o DeepMentor fala o formato aberto Agent-Skills, o **[ClawHub](https://clawhub.ai/)** também funciona como fonte de primeira classe — está integrado juntamente com o EduHub. Escolha-o com o prefixo de hub:
...
<p align="center">
  Usando o código: <b><code>DEEPMENTOR20</code></b> — obtenha $20 de desconto na sua primeira <a href="https://developer.pageindex.ai/">subscrição PageIndex</a>!
</p>
...
| [**Claude Code**](https://github.com/anthropics/claude-code) | CLI de programação agêntica que inspirou o loop de agente do DeepMentor |
...
Queremos que o DeepMentor continue a iterar e a melhorar — e, em última análise, a tornar-se um presente que devolvemos à comunidade de código aberto. O nosso [**roadmap**](https://github.com/HKUDS/DeepMentor/issues/498) é atualizado continuamente; vote em itens lá ou proponha novos. Se quiser contribuir, veja o [**Guia de Contribuição**](../../CONTRIBUTING.md) para a estratégia de branches, padrões de código e como começar.

<div align="center">

Esperamos que o DeepMentor se torne um presente para a comunidade. 🎁
...
<p align="center">
 <a href="https://www.star-history.com/hkuds/deepmentor">
  <picture>
   ...
   <img alt="Classificação do histórico de estrelas" src="https://api.star-history.com/badge?repo=HKUDS/DeepMentor" />
  </picture>
 </a>
</p>
...
<p>
  <img src="https://visitor-badge.laobi.icu/badge?page_id=HKUDS.DeepMentor&style=for-the-badge&color=00d4ff" alt="Visualizações">
</p>

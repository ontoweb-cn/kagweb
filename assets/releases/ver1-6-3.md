...
Deep Research retries incomplete report sections without leaking duplicate partial prose. Local execution enforces output and process limits. The launcher adds a macOS startup command, browser-safe Windows behavior, detached/no-browser operation, and `kagweb stop`; interrupted chat streams also recover more reliably.

## Upgrade Notes

Run `pip install -U kagweb`; Docker users pull `ghcr.io/hkuds/kagweb:latest`. Existing settings, accounts, knowledge indexes, books, Reading state, and sessions need no manual migration or re-index; startup migrates legacy chat and workspace-preference data in place.
...
**Full Changelog**: https://github.com/HKUDS/KAGWeb/compare/v1.6.2...v1.6.3

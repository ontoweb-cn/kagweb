...
`pip install -U kagweb`; Docker users pull `ghcr.io/hkuds/kagweb:latest`. No manual migration or re-index is required.
...
- **Shared edits are optimistic.** If another editor advances a Book revision first, refresh the Book and apply the change again; KAGWeb will not silently overwrite the newer revision.
- **Web and GitHub sources remain opt-in.** Existing knowledge bases are unchanged until you register a source and run sync.
- **Search remains optional.** Select Serply during `kagweb init` or configure it later under **Settings → Models → Search**; existing search settings continue to work.
- The English and Chinese guides have been updated together at [kagweb.info](https://kagweb.info/).

**Full Changelog**: https://github.com/HKUDS/KAGWeb/compare/v1.5.17...v1.6.0

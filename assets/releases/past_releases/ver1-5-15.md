# DeepMentor v1.5.15 Release Notes
...
Two entry-point groups let an installed package extend DeepMentor without touching the tree. `deepmentor.plugins` was already imported by the capability registry and `/plugins/list` behind a `try/except` — but the module never existed, so plugin discovery was a dead path; it is now real. `deepmentor.loop_capabilities` registers chat-loop capabilities alongside the built-ins, which win on name collisions. Broken entry points are skipped rather than fatal, and discovery is cached per process so plugin capabilities stay singletons.
...
`/whisper` is a two-seat counselling-practice room contributed by [@alanguan73](https://github.com/alanguan73): the tutor plays the visitor, you take the trainee seat, and a supervisor whispers coaching only the trainee sees, with an end-of-session debrief. It lives under **Learning Space → More Projects** and needs its own capability package installed — the surface ships, the counselling capability is not part of DeepMentor.
...
The docs at [deepmentor.info](https://deepmentor.info/) were refreshed alongside this release.
...
**Full Changelog**: https://github.com/HKUDS/DeepMentor/compare/v1.5.14...v1.5.15

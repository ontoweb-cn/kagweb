...
- **Login** accepts a username (not just an email), surfaces bad-login errors
  ...
  flag.
- **CLI** gains `kagweb skill` commands — browser login plus publish/update
  for your own skills.
...
- Drop-in from v1.4.5: `pip install -U kagweb`; Docker users pull
  `ghcr.io/hkuds/kagweb:latest`. No migrations — existing data loads as-is.
...
- New engines and services are opt-in: GraphRAG needs the extra
  (`pip install 'kagweb[graphrag]'`), document parsing needs
  `kagweb[parse]`, and voice / image / video need their model catalogs
  configured.

**Full Changelog**: https://github.com/HKUDS/KAGWeb/compare/v1.4.5...v1.4.6

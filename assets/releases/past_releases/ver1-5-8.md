# DeepMentor v1.5.8 Release Notes
...
`deepmentor start` from a checkout now builds the frontend once into `.next-deepmentor` and serves the standalone output, reusing that build until an input actually changes (a fingerprint over the `web/` tree, the version file, and the build-time public settings). Hot reload moves to `deepmentor start --dev`, which keeps using `.next` — separate directories, so neither mode invalidates the other's output while it is running. A long-running deployment no longer pays for a dev compiler's caches.
...
After a turn, a finished book job, or a completed knowledge-base task, DeepMentor collects cycles and — on Linux — calls `malloc_trim` so glibc hands free heap pages back instead of holding arenas that make RSS look permanently pinned. Containers and `deepmentor start` also set `MALLOC_ARENA_MAX=2` and a lower trim threshold, both overridable by an operator who has tuned their own.
...
- `deepmentor.agents`, `deepmentor.core.agentic`, the provider-core package, the document extractor's parser libraries (PyMuPDF, pypdf, python-docx, openpyxl, python-pptx) and the LLM error-mapping rules all resolve on first use rather than at import time.
- `python -m deepmentor.api.run_server` no longer starts with `--reload`; export `DEEPMENTOR_DEV_RELOAD=1` for it. Reload runs a supervisor plus a worker and retains file-watcher state, which is development machinery. `deepmentor serve --reload` is unchanged.

## Upgrade Notes

Drop-in from v1.5.7: `pip install -U deepmentor`; Docker users pull `ghcr.io/hkuds/deepmentor:latest`. No schema or config changes.

- **Source checkouts build before they serve.** The first `deepmentor start` after upgrading runs `npm run build` and takes correspondingly longer; later launches reuse it until the source changes. Use `deepmentor start --dev` for the previous hot-reload loop. The new `web/.next-deepmentor/` build directory is git-ignored.
- **`python -m deepmentor.api.run_server` no longer reloads by default.** Export `DEEPMENTOR_DEV_RELOAD=1` to restore it.

**Full Changelog**: https://github.com/HKUDS/DeepMentor/compare/v1.5.7...v1.5.8

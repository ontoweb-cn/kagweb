...
KAGWeb skills share their format with the Agent-Skills ecosystem
(`SKILL.md` + support files), so external registries are now a native
source for the skill library:

```bash
kagweb skill search "git release notes"        # semantic search on the hub
kagweb skill install clawhub:gh-release-notes  # verify → fetch → register
kagweb skill install some-skill@1.2.0          # hub defaults to clawhub; @ pins a version
kagweb skill list                              # local skills with hub provenance
```
...
- Drop-in from v1.4.3: `pip install -U kagweb`; Docker users pull
  `ghcr.io/hkuds/kagweb:latest`. No migrations.
- `kagweb skill install` talks to clawhub.ai over HTTPS; air-gapped
  deployments can point a mirror or a fetch CLI at it via
  `settings/skill_hubs.json`.

**Full Changelog**: https://github.com/HKUDS/KAGWeb/compare/v1.4.3...v1.4.4

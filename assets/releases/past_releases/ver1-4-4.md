# DeepMentor v1.4.4 Release Notes
...
DeepMentor skills share their format with the Agent-Skills ecosystem
(`SKILL.md` + support files), so external registries are now a native
source for the skill library:

```bash
deepmentor skill search "git release notes"        # semantic search on the hub
deepmentor skill install clawhub:gh-release-notes  # verify → fetch → register
deepmentor skill install some-skill@1.2.0          # hub defaults to clawhub; @ pins a version
deepmentor skill list                              # local skills with hub provenance
```
...
- Drop-in from v1.4.3: `pip install -U deepmentor`; Docker users pull
  `ghcr.io/hkuds/deepmentor:latest`. No migrations.
- `deepmentor skill install` talks to clawhub.ai over HTTPS; air-gapped
  deployments can point a mirror or a fetch CLI at it via
  `settings/skill_hubs.json`.

**Full Changelog**: https://github.com/HKUDS/DeepMentor/compare/v1.4.3...v1.4.4

...
- **Minimal thinking mode is provider-safe** - DeepSeek, DashScope, VolcEngine,
  ...
  `reasoning_effort=minimal`; KAGWeb sends the provider-specific disable
  signal instead.
...
- **`kagweb start` launches the full web stack** - the CLI now delegates to
...
- Added CLI coverage so `kagweb start` propagates the launcher exit code.
...
**Full Changelog**: https://github.com/HKUDS/KAGWeb/compare/v1.3.8...v1.3.9

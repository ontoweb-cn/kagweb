# DeepMentor v1.5.6 Release Notes
...
v1.5.5 shipped OpenAI Codex OAuth with a caveat — on a remote box, sign in from the server instead. That caveat is gone. The browser's `localhost` and the server's `localhost` are different machines, so a reverse proxy alone can never carry the callback; DeepMentor now forwards only the exact callback path through the Web origin to a state-validating broker, and the settings card and CLI print the real callback address, the SSH tunnel command, an explicit authorize action, and a countdown that survives a page reload ([#716](https://github.com/HKUDS/DeepMentor/issues/716)).

Two follow-ups make it work for everyone else on the deployment. The callback is delivered by OAuth `state` — a secret this process minted for one login, compared in constant time — rather than by the request's user, because the browser arrives through your own tunnel and carries no session cookie; without this, remote sign-in returned `login_not_active` for every non-administrator. And a partner turn now resolves Codex credentials to the owner's root instead of the partner's synthetic workspace, so chatting with a partner no longer fails with "Sign in to Codex before using this model" ([#713](https://github.com/HKUDS/DeepMentor/issues/713)).
...
`parse_language()` ended in a bare `return "zh"`, so ja/ko/fr/de/ru/ar all resolved to Chinese — which is why a Japanese book came back with Chinese quizzes ([#712](https://github.com/HKUDS/DeepMentor/issues/712)). It now delegates to the existing normalizer and passes unknown codes through, and the two fallback chains that assumed the answer was zh or en follow suit: prompts try `[code, base locale, "en"]`, so `zh-tw` reuses the Chinese files and `ja` lands on English rather than Chinese.

### Assorted fixes

- Book creation and block regeneration run over WebSocket instead of a request that a proxy could time out, and spine synthesis and Visualize's analysis stage use non-streaming calls so a single-chapter fallback or a truncated code block stops happening ([#707](https://github.com/HKUDS/DeepMentor/issues/707)). All three now route through the agent seam, which restores the image attachments, trace metadata, and per-agent routing the direct factory calls were dropping.
- `deepmentor start` on a fresh source checkout installs `web/node_modules` for you instead of crashing with a missing-module error ([#710](https://github.com/HKUDS/DeepMentor/issues/710)).
- Admin user management, sign-out, confirm dialogs, and toasts are localized, so switching the UI to Chinese no longer leaves account management in English ([#718](https://github.com/HKUDS/DeepMentor/issues/718)).
- The service lazy-loader uses literal import paths ([#717](https://github.com/HKUDS/DeepMentor/issues/717)) — behaviour-neutral, but it makes the module graph statically analyzable.

## Upgrade Notes

Drop-in from v1.5.5: `pip install -U deepmentor`; Docker users pull `ghcr.io/hkuds/deepmentor:latest`. No migrations, no schema or config changes.

- **Remote Codex sign-in now uses an SSH tunnel**, and the v1.5.5 advice to run `deepmentor provider login openai-codex` on the server is no longer the only path. DeepMentor prints the exact `ssh -N -L …` command; run only the one matching the callback port it reports, and note that `3782` in that command is an example — replace it with the Web port actually listening on the SSH host's loopback.
- A Tencent IMA knowledge base needs a `client_id` and API key from [ima.qq.com/agent-interface](https://ima.qq.com/agent-interface) plus the library id. Documents are added in IMA, not through DeepMentor.

**Full Changelog**: https://github.com/HKUDS/DeepMentor/compare/v1.5.5...v1.5.6

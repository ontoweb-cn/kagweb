# DeepMentor v1.4.14 Release Notes
...
LightRAG indexing hard-failed with "Parser 'mineru' is not properly installed" even when you'd picked a different parsing engine, because RAG-Anything ran a one-time install check on its default parser. DeepMentor already feeds it pre-parsed content, so that spurious check is now skipped and indexing proceeds regardless of which parse engine you chose (issue #594).
...
Drop-in from v1.4.13 on the default JSON/SQLite backend: `pip install -U deepmentor`; Docker users pull `ghcr.io/hkuds/deepmentor:latest`. No migrations and no behavior changes for existing single-user setups.
...
**Full Changelog**: https://github.com/HKUDS/DeepMentor/compare/v1.4.13...v1.4.14

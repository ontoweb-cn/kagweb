...
- **Chat** — RAG coexists with an exclusive knowledge capability instead of being suppressed ([#650](https://github.com/HKUDS/KAGWeb/issues/650)); DeepSeek's DSML text tool-call markup is parsed ([#666](https://github.com/HKUDS/KAGWeb/issues/666)); autoscroll releases on manual scroll-up ([#649](https://github.com/HKUDS/KAGWeb/issues/649)).
- **Memory** — `write_memory` preference adds are idempotent ([#647](https://github.com/HKUDS/KAGWeb/issues/647)); fact and line-edit JSON parse cleanly even with trailing prose.
- **Personas** — the peer / teacher / research-assistant presets ship seeded ([#659](https://github.com/HKUDS/KAGWeb/issues/659)).
...
Drop-in from v1.5.2: `pip install -U kagweb`; Docker users pull `ghcr.io/hkuds/kagweb:latest`. No migrations — existing knowledge bases, partners, personas, and mastery paths keep working. One packaging change: KAGWeb now requires **Python 3.11–3.13** (`requires-python = ">=3.11,<3.14"`); on 3.14 `pip` finds no wheels for some compiled base dependencies (e.g. `faiss-cpu`) and now refuses with a readable message instead of a failed source build ([#664](https://github.com/HKUDS/KAGWeb/issues/664)).

**Full Changelog**: https://github.com/HKUDS/KAGWeb/compare/v1.5.2...v1.5.3

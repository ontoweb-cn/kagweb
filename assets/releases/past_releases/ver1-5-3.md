# DeepMentor v1.5.3 Release Notes
...
- **Chat** — RAG coexists with an exclusive knowledge capability instead of being suppressed ([#650](https://github.com/HKUDS/DeepMentor/issues/650)); DeepSeek's DSML text tool-call markup is parsed ([#666](https://github.com/HKUDS/DeepMentor/issues/666)); autoscroll releases on manual scroll-up ([#649](https://github.com/HKUDS/DeepMentor/issues/649)).
- **Memory** — `write_memory` preference adds are idempotent ([#647](https://github.com/HKUDS/DeepMentor/issues/647)); fact and line-edit JSON parse cleanly even with trailing prose.
- **Personas** — the peer / teacher / research-assistant presets ship seeded ([#659](https://github.com/HKUDS/DeepMentor/issues/659)).
...
Drop-in from v1.5.2: `pip install -U deepmentor`; Docker users pull `ghcr.io/hkuds/deepmentor:latest`. No migrations — existing knowledge bases, partners, personas, and mastery paths keep working. One packaging change: DeepMentor now requires **Python 3.11–3.13** (`requires-python = ">=3.11,<3.14"`); on 3.14 `pip` finds no wheels for some compiled base dependencies (e.g. `faiss-cpu`) and now refuses with a readable message instead of a failed source build ([#664](https://github.com/HKUDS/DeepMentor/issues/664)).

**Full Changelog**: https://github.com/HKUDS/DeepMentor/compare/v1.5.2...v1.5.3

# KAGWeb v1.5.7 Release Notes

**Release Date:** 2026.07.31

A follow-up to [v1.5.6](ver1-5-6.md) that opens the tutor up to tools KAGWeb never shipped: a curated store of hosted MCP services each person installs for themselves, and 101 command-line apps from the CLI-Anything catalog the chat agent can call directly. Both arrive with the authorisation and isolation a shared deployment actually needs — installing is a privileged action, running is not, and nothing that authorises a person stays anywhere the code sandbox can read. Drop-in — no migrations.
...
- The composer keeps focus after you send, and gets it back when you switch tabs away and return ([#720](https://github.com/HKUDS/KAGWeb/pull/720)).
- Released container images carry provenance and SBOM attestations ([#722](https://github.com/HKUDS/KAGWeb/pull/722)).
...
Drop-in from v1.5.6: `pip install -U kagweb`; Docker users pull `ghcr.io/hkuds/kagweb:latest`. No schema or config changes, and an existing Codex login carries over — its credentials relocate themselves the first time they are resolved.
...
**Full Changelog**: https://github.com/HKUDS/KAGWeb/compare/v1.5.6...v1.5.7

...
- **The `kagweb-cli` wheel ships the agents' prompt YAML again** — it had none, so prompt loading failed and the empty result was cached for the life of the process. Misses are no longer cached either, so a resource that appears after startup is picked up on retry.
...
**Full Changelog**: https://github.com/HKUDS/KAGWeb/compare/v1.5.9...v1.5.10

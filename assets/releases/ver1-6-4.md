# DeepMentor v1.6.4 Release Notes
...
Built-in tools and capabilities now load only when used, while provider SDK clients share a bounded pool instead of growing without limit. Document parsing and other heavy jobs run through isolated worker processes, so a parser crash or memory spike is less likely to take the main server with it. Deployments can tune concurrency with `DEEPMENTOR_ISOLATED_WORKERS`.
...
Run `pip install -U deepmentor`; Docker users pull `ghcr.io/hkuds/deepmentor:latest`. Existing settings, books, sessions, and learning data need no manual migration.
...
- Extensions importing private modules below `deepmentor.services.llm.executors` or provider implementation modules must move to the public factory and protocol surfaces. Deprecated cloud/local shims remain for this release.
- `DEEPMENTOR_ISOLATED_WORKERS` is optional and defaults to `2`; lower it on very small hosts or raise it only after measuring parser memory.
- The English and Chinese guides have been updated together at [deepmentor.info](https://deepmentor.info/).

**Full Changelog**: https://github.com/HKUDS/DeepMentor/compare/v1.6.3...v1.6.4

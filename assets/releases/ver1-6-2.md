...
Run `pip install -U kagweb`; Docker users pull `ghcr.io/hkuds/kagweb:latest`. Existing data, knowledge indexes, video progress, and reading snapshots need no migration.

- **Public-caption tutoring is optional.** Install `pip install -U 'kagweb[video-learning]'` to enable transcript-grounded video explanations. YouTube/Invidious playback still works without it.
...
- **The Web Playground and `/api/v1/plugins/*` tester API are removed.** `kagweb plugin list/info` still inspects runtime tools and capabilities. Browser visualizers now use `/api/v1/visualizers` and declarative visualizer packages.
- The English and Chinese guides have been updated together at [kagweb.info](https://kagweb.info/).

**Full Changelog**: https://github.com/HKUDS/KAGWeb/compare/v1.6.1...v1.6.2

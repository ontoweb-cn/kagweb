...
| **M4 构建+多用户** | 文档上传 → `/public/v1/builder/kag/submit` 构建流水线与监控；T2 项目 ACL（membership + `kagweb_<uid>` 归因） | 非/admin 用户按 membership 受控访问——**M4-B T2 已完成**（`member_store` JSON `{project_id: {owner, members}}`：无 owner 记录的项目对非 admin 不可见、admin 恒全量、owner/成员可见；`list` 按 membership 过滤、`get`/`schema`/`alter`/`graph` 改项目级门禁 `_require_project_access`（写从 admin-only 放宽为 membership）；**userNo 归因**：KAGWeb 用户 `u_<32hex>`（34 字符）超 OpenSPG 6-20 上限 → `derive_user_no` = `kagweb_<sha256(id)[:12]>`（19 字符稳定唯一，M-10 由 8hex 放宽到 12hex 率除碰撞），create 登记调用者为 owner；成员管理 `GET/PUT /members`（读=项目访问、写=admin）；后端 1705 全绿，5 新单测）；**M4-A 构建受理层已完成**（`POST /api/kag/projects/{id}/build`（项目成员 + same-origin）→ `submit_builder` 经 `POST /public/v1/builder/kag/submit`（wire `{projectId, workerNum, command}`，受理 200 RUNNING）→ task_store 记 `kind=build`，tasks 页显示 Build 标签；**可用性声明（设计风险 #5）**：本地 compose 无 computing engine driver——任务受理成功但执行会失败（`cannot find driver for`），本里程碑交付受理层，端到端执行依赖 M5 上游化配置 executor；openapi 扩 members/build 端点重生成（纯增量）、check:fast 全绿）；**M4 评审修复轮已完成**（M-10 `derive_user_no` hash 8→12 hex（32bit→48bit）——原约 6.5 万用户 50% 碰撞，48bit 对 10 万用户实测零碰撞，仍 ≤20 字符上限；M-7 `get_tasks` 按 membership 过滤——修复非 admin 经 `/api/kag/tasks` 读取**全部项目**任务（question/answer/references 泄露）的漏洞，admin 全量、非 admin 仅可见项目（显式 project_id 时逐项目校验）；M-8 重写 member_store 去重（原 `not (u in seen or seen.add(u))` 晦涩）；N 记录——本地无登录形态经 `local_admin_user()`（role=admin）恒全量、无回归，create 响应 `{id}` 可正确登记 owner）；后端 1706 全绿 |
| **M5 上游化** | 契约冻结评审、`kag[mcp]` extra、KAG CI 接入（§8 门槛） | Bridge 进入 KAG 仓库——**M5-A 契约冻结已完成**（A.1 新增冻结小节：4 工具 inputSchema/返回 schema 自 M3.3 冻结，破坏性变更须先升工具 vN 版本再发布，KAGWeb 侧零改动不变量重申，当前版本 v1）；**M5-B（代码入 KAG 仓库 + `kag[mcp]` extra + KAG CI）启动——第一增量已提交**（KAG 分支 `feat/mcp-frozen-contract`，commit `ca69a671`）：**关键事实**——上游 KAG 已有 `kag mcp-server`（`kag/bin/commands/mcp_server.py` → `kag/mcp/server/kag_mcp_server.py`，工具面 `qa-pipeline`/`kb-retrieve`，transport sse/stdio，进程内 `do_qa_pipeline`），**契约与独立 kag-bridge 不同**——故 M5-B 非迁移而是**契约对齐进上游 tool 面**：新增 `kag-solve`（复用上游 `OpenSPGReporter` 纯内存产 StreamData，返回冻结的 `{answer, reference, subgraph, cost_ms, namespace}`）+ `kag-status`，与独立 bridge 双宿主、KAGWeb 零改动；**实测发现上游单例陷阱**：`KAGConfigMgr.all_config` 首调即缓存，模块顶层 `import OpenSPGReporter` 的加载链在无配置 cwd 抢先初始化 → 后续 chdir 也拿空配置——`_load_kag_config` 改用 `KAG_CONFIG.initialize(config_file=...)` 显式重置，stdio 实测 `kag_status` 返回 `{bridge:ok, project:3, namespace:m0ProbeLive, llm_configured:true}`；`kag-solve` 复用 M3 实测过的同一 `do_qa_pipeline` 路径（subgraph 形态证同）；**第二增量已完成**（commit `156ed685`，补全 4 工具冻结契约）：`kag-schema`（`ReasonerClient.get_reason_schema` → `{project_id, namespace, spg_types}`，key 为 namespace 全名，与 bridge 一致）+ `kag-reason`（直调 `POST /public/v1/reason/run` 于线程池——knext ReasonTask 模型丢 `resultMessage`，直调保留冻结的错误详情；params 值 JSON 字符串化使其数组可用；rows ≤200、错误截 600）；stdio 实测：kag_schema 列出 19 类型、kag_reason workFor FINISH 3 rows（张三→开元大学），与 M3 bridge E2E 完全一致——**上游 kag mcp-server 现已四工具全对齐冻结契约（双宿主）**；**接线说明已落地**（A.1 双宿主接入段：独立 kag-bridge 与上游 `kag mcp-server` 两套 stdio `.mcp.json` 配置，含实测边界——四工具 stdin 直接 `serve()` 验通、CLI 子命令入口 Connection closed 归上游 CLI 层）；**待续（外部依赖）**：上游 CI/test 纳入门槛②③、上游 owner 评审合并 |
...
**双宿主接入（M5-B，.mcp.json 接线）**：同一冻结契约有两个宿主，KAGWeb 经 kag settings 域任选其一（.mcp.json 生成逻辑零改动，见 §5.3 的 stdio 分支）：

- **独立 kag-bridge**（默认）：`bridge_command=<venv>/python`、`bridge_args=["-m","kag_bridge"]`——M1–M4 全程 E2E 验证；
- **上游 `kag mcp-server`**（M5-B 对齐）：`bridge_command=<venv>/python`、`bridge_args=["-m","kag.bin.kag_cmds","mcp-server","--transport","stdio","--enabled-tools","kag-solve,kag-schema,kag-reason,kag-status"]`、`env.KAG_PROJECT_DIR=<项目目录>`。**实测边界**：四工具契约与 stdio 协议经直接 `KagMcpServer(...).serve()` 验通（kag_status ok / kag_schema 19 类型 / kag_reason FINISH 3 rows）；但 `kag.bin.kag_cmds` CLI 子命令入口在本环境冒烟出"Connection closed"——属**上游 CLI 包装层事项**（疑似 import 链 stdout 污染或参数接线），非工具契约问题，运营前需在上游侧确认该入口。

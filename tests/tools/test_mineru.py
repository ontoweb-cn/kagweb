...
def test_cloud_pdf_over_page_limit_is_split_and_merged(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    pymupdf = pytest.importorskip("pymupdf")
    ...
    document.close()
    monkeypatch.setattr(mineru_backend, "MAX_CLOUD_PDF_PAGES", 2)

    calls: list[Path] = []

    def fake_cloud(path, output_base, _config, **_kwargs):  # noqa: ANN001, ANN003
        path = Path(path)
        calls.append(path)
        with pymupdf.open(path) as chunk:
            page_count = len(chunk)
        workdir = Path(output_base) / path.stem
        workdir.mkdir(parents=True)
        (workdir / "part.md").write_text(f"# {path.stem}", encoding="utf-8")
        (workdir / "part_content_list.json").write_text(
            json.dumps(
                [
                    {"type": "text", "page_idx": page, "text": f"part {len(calls)} page {page}"}
    ...
    workdir = mineru_backend.parse_pdf_to_workdir(
        source,
        tmp_path / "out",
        config=MinerUConfig(mode="cloud", api_token="tok"),
        on_output=reports.append,
    )
    ...
    assert workdir == tmp_path / "out"
    assert "# long.part-001" in (workdir / "long.md").read_text(encoding="utf-8")
    blocks = json.loads((workdir / "long_content_list.json").read_text(encoding="utf-8"))
    assert [block["page_idx"] for block in blocks] == [0, 1, 2, 3, 4]
    assert any("splitting 5 pages into 3 parts" in report for report in reports)
...
def test_download_retries_a_transient_tls_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    attempts = 0

    def flaky_download(*_args, **_kwargs):  # noqa: ANN002, ANN003
        nonlocal attempts
        attempts += 1
        if attempts < 3:
            request = real_httpx.Request("GET", "https://archive.example/result.zip")
            raise real_httpx.ConnectError("TLS connection closed", request=request)
        return _Resp(content=b"archive")

    monkeypatch.setattr(mineru_cloud.httpx, "get", flaky_download)
    monkeypatch.setattr(mineru_cloud.time, "sleep", lambda _delay: None)

    assert mineru_cloud._download("https://archive.example/result.zip") == b"archive"
    assert attempts == 3

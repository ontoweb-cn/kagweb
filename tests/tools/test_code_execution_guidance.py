...
from deepmentor.tools.builtin import CodeExecutionTool
...
def test_chat_prompts_prefer_code_execution_for_python_files() -> None:
    root = Path(__file__).parents[2] / "deepmentor" / "agents" / "chat" / "prompts"
...
def test_solve_prompts_route_python_files_to_code_execution() -> None:
    root = Path(__file__).parents[2] / "deepmentor" / "capabilities" / "solve" / "prompts"
    shell_only_guidance = {
...
def test_office_skills_use_code_execution_for_python_deliverables() -> None:
    root = Path(__file__).parents[2] / "deepmentor" / "skills" / "builtin"

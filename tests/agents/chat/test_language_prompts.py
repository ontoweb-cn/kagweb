...
def _fake_llm_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("deepmentor.agents.base_agent.get_llm_config", lambda: cfg)
...
def test_agentic_chat_final_prompt_uses_selected_language(
    ...
    monkeypatch.setattr(
        "deepmentor.agents.chat.agentic_pipeline.get_tool_registry",
        lambda: FakeRegistry(),
    )
    ...
    assert "你是 DeepMentor" in zh_prompt
    assert "You are DeepMentor" in en_prompt

...
courses_router_module = importlib.import_module("deepmentor.api.routers.courses")
courses_service_module = importlib.import_module("deepmentor.services.courses")
courses_router = courses_router_module.router

from deepmentor.services.courses import COURSE_RESOURCE_KINDS, CourseService
...
def failing_subsystems(monkeypatch) -> None:
    monkeypatch.setattr(
        "deepmentor.multi_user.knowledge_access.current_kb_manager",
        lambda: _FailingSubsystem(),
    )
    monkeypatch.setattr(
        "deepmentor.book.engine.get_book_engine",
        lambda: _FailingSubsystem(),
    )
    monkeypatch.setattr(
        "deepmentor.services.notebook.service.get_notebook_manager",
        lambda: _FailingSubsystem(),
    )
    monkeypatch.setattr(
        "deepmentor.learning.service.LearningService",
        lambda: _FailingSubsystem(),
    )
    monkeypatch.setattr(
        "deepmentor.reading.catalog_store.ReadingCatalogStore",
        lambda: _FailingSubsystem(),
    )
    monkeypatch.setattr(
        "deepmentor.services.partners.get_partner_manager",
        lambda: _FailingSubsystem(),
    )
    monkeypatch.setattr(
        "deepmentor.services.session.get_session_store",
        lambda: _FailingSubsystem(),
    )
    monkeypatch.setattr(
        "deepmentor.services.session.get_sqlite_session_store",
        lambda: _FailingSubsystem(),
    )

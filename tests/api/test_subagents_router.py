...
if FastAPI is not None and TestClient is not None:
    subagents_module = importlib.import_module("deepmentor.api.routers.subagents")
...
def client(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "deepmentor.services.subagent.config._settings_path",
...
def test_backend_options_endpoint_shape(client, monkeypatch):
    ...
    from deepmentor.services.subagent.models import BackendOptions, ModelOption
...
def test_message_connection_streams_and_persists(client, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("deepmentor.services.subagent.get_backend", lambda kind: _FakeBackend())

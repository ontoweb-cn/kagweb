...
import deepmentor.services.config as config_module
...
import deepmentor.services.rag.pipelines.ima.config as ima_config_module
...
if FastAPI is not None and TestClient is not None:
    knowledge_router_module = importlib.import_module("deepmentor.api.routers.knowledge")
...
def _capture_probe(monkeypatch) -> list[tuple[str, str, str]]:
    ...
    import deepmentor.services.rag.pipelines.ima.probe as probe_module
...
def test_create_preserves_known_nondefault_provider(monkeypatch, tmp_path: Path) -> None:
    ...
    pageindex_config = importlib.import_module("deepmentor.services.rag.pipelines.pageindex.config")
...
def test_upload_task_marks_provider_failures_as_error(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "deepmentor.knowledge.add_documents.RAGService",
        _FailingRagService,
    )
...
def test_reindex_accepts_default_alias(monkeypatch, tmp_path: Path) -> None:
    ...
    embedding_signature = importlib.import_module("deepmentor.services.rag.embedding_signature")
    index_versioning = importlib.import_module("deepmentor.services.rag.index_versioning")
    monkeypatch.setattr(
...
def test_reindex_task_persists_completed_progress(monkeypatch, tmp_path: Path) -> None:
    ...
    rag_service_module = importlib.import_module("deepmentor.services.rag.service")
    manager_module = importlib.import_module("deepmentor.knowledge.manager")
...
def test_reindex_task_fails_closed_when_terminal_progress_is_not_persisted(
    ...
    rag_service_module = importlib.import_module("deepmentor.services.rag.service")
    manager_module = importlib.import_module("deepmentor.knowledge.manager")
    progress_module = importlib.import_module("deepmentor.knowledge.progress_tracker")
...
def test_update_config_coerces_legacy_provider_to_llamaindex() -> None:
    ...
    config_module = importlib.import_module("deepmentor.services.config")
    app = _build_app()
...
def _patch_server_probe(monkeypatch, *, ok: bool, error: str | None = None) -> None:
    ...
    from deepmentor.services.rag.pipelines.lightrag_server import probe as probe_module
...
def _patch_weknora_probe(monkeypatch, *, ok: bool, error: str | None = None) -> None:
    from deepmentor.services.rag.pipelines.weknora import probe as probe_module
...
def test_create_pageindex_oss_persists_optional_mode(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(knowledge_router_module, "KnowledgeBaseInitializer", _FakeInitializer)
    monkeypatch.setattr(knowledge_router_module, "_kb_base_dir", tmp_path / "knowledge_bases")
    preflight = importlib.import_module("deepmentor.services.rag.preflight")
...
def test_create_mode_aware_kb_persists_per_kb_search_mode(monkeypatch, tmp_path: Path) -> None:
    ...
    preflight = importlib.import_module("deepmentor.services.rag.preflight")
    monkeypatch.setattr(preflight, "engine_preflight", lambda _provider: {"ok": True, "checks": []})
    lightrag_config = importlib.import_module("deepmentor.services.rag.pipelines.lightrag.config")
...
def test_upload_progress_counts_completed_files_and_reports_reliable_stages(
    ...
    def _record_update(self, stage, message="", current=0, total=0, **kwargs):
        ...
        # this test is about.
        from deepmentor.knowledge.progress_tracker import render_message_template
...
def test_lightrag_config_endpoint_round_trips_the_indexing_knobs(
    ...
    monkeypatch.setattr(
        "deepmentor.services.config.get_runtime_settings_service",
        lambda: service,
    )
...
def test_lightrag_server_defaults_are_redacted_and_reused_for_probe(
    ...
    probe_module = importlib.import_module("deepmentor.services.rag.pipelines.lightrag_server.probe")
...
def test_llamaindex_config_endpoint_round_trips_reranker_knobs(monkeypatch, tmp_path: Path) -> None:
    ...
    assert initial.json()["reranker_endpoint"] == ""
    ...
    assert saved.json()["reranker_endpoint"] == "http://localhost:8000"
    assert saved.json()["rerank_top_k"] == 25

    again = client.get("/api/knowledge-bases/rag-pipelines/llamaindex/config").json()
    assert again["reranker_model"] == "BAAI/bge-reranker-base"
    assert again["reranker_endpoint"] == "http://localhost:8000"

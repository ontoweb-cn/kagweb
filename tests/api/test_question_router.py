...
def _cleanup_question_router_module():
    yield
    sys.modules.pop("deepmentor.api.routers.question", None)
...
def _fake_config_module() -> types.ModuleType:
    """Stand in for ``deepmentor.services.config``, deferring the rest to the real one.
    ...
    handler lazily imports ``deepmentor.api.routers.auth``, which pulls
    *unrelated* loaders (auth settings, integrations, …) out of this same
    ...
    ``deepmentor.api.routers.auth`` in ``sys.modules`` — so the lazy import was
    a cache hit that never reached this stand-in. Green in a full run, red on
    its own.
    """
    real = importlib.import_module("deepmentor.services.config")
    module = types.ModuleType("deepmentor.services.config")
...
def _load_question_router_module(monkeypatch: pytest.MonkeyPatch):
    ...
    fake_agents = _package("deepmentor.agents")
    fake_agents_question = types.ModuleType("deepmentor.agents.question")
    fake_agents_question.AgentCoordinator = object
    fake_agents.question = fake_agents_question
    monkeypatch.setitem(sys.modules, "deepmentor.agents", fake_agents)
    monkeypatch.setitem(sys.modules, "deepmentor.agents.question", fake_agents_question)

    fake_logging = _package("deepmentor.logging")
    ...
    monkeypatch.setitem(sys.modules, "deepmentor.logging", fake_logging)

    monkeypatch.setitem(sys.modules, "deepmentor.services.config", _fake_config_module())

    fake_llm_package = _package("deepmentor.services.llm")
    fake_llm_config = types.ModuleType("deepmentor.services.llm.config")
    fake_llm_config.get_llm_config = lambda: None
    fake_llm_package.config = fake_llm_config
    monkeypatch.setitem(sys.modules, "deepmentor.services.llm", fake_llm_package)
    monkeypatch.setitem(sys.modules, "deepmentor.services.llm.config", fake_llm_config)

    fake_settings_package = _package("deepmentor.services.settings")
    fake_interface_settings = types.ModuleType("deepmentor.services.settings.interface_settings")
    ...
    monkeypatch.setitem(sys.modules, "deepmentor.services.settings", fake_settings_package)
    monkeypatch.setitem(
        sys.modules,
        "deepmentor.services.settings.interface_settings",
        fake_interface_settings,
    )

    fake_tools = _package("deepmentor.tools")
    fake_tools_question = types.ModuleType("deepmentor.tools.question")
    ...
    monkeypatch.setitem(sys.modules, "deepmentor.tools", fake_tools)
    monkeypatch.setitem(sys.modules, "deepmentor.tools.question", fake_tools_question)

    return importlib.import_module("deepmentor.api.routers.question")

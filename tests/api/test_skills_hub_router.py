...
catalog so the web panel can render it in DeepMentor's own UI (no iframe, no
...
from deepmentor.services.skill import hub as hub_module
from deepmentor.services.skill.hub import ClawHubProvider
...
def _mock_provider() -> ClawHubProvider:
                    ...
                    "skills": [
                        {
                            ...
                            "owner": {
                                "displayName": "DeepMentor",
                                "htmlUrl": "https://deepmentor.info",
                            },
                        }
                    ]
                },
        ...
        if path == "/api/v1/skills/socratic-tutor":
            return httpx.Response(
                ...
                json={
                    ...
                    "owner": {"displayName": "DeepMentor", "htmlUrl": "https://deepmentor.info"},
    ...
    return ClawHubProvider(
        "eduhub",
        base_url="https://eduhub.deepmentor.info/api/v1",
        client=httpx.Client(transport=httpx.MockTransport(handler)),
    )


def _build_app() -> FastAPI:
    router = importlib.import_module("deepmentor.api.routers.skills").router
    app = FastAPI()
...
def test_hub_catalog_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert data["web_url"] == "https://eduhub.deepmentor.info"
    row = data["skills"][0]
    assert row["slug"] == "socratic-tutor"
    assert row["downloads"] == 8 and row["stars"] == 2
    assert row["owner"] == "DeepMentor"
...
def test_hub_detail_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert data["web_url"] == "https://eduhub.deepmentor.info/skills/socratic-tutor"

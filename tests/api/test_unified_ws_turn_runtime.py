...
from deepmentor.services.session.turn_runtime import TurnRuntimeManager
...
async def test_turn_runtime_replays_events_and_materializes_messages(
    ...
    monkeypatch.setattr("deepmentor.services.llm.config.get_llm_config", lambda: SimpleNamespace())
    monkeypatch.setattr(
        "deepmentor.services.session.context_builder.ContextBuilder", FakeContextBuilder
    )
    monkeypatch.setattr("deepmentor.runtime.orchestrator.ChatOrchestrator", FakeOrchestrator)
    monkeypatch.setattr(
        "deepmentor.book.context.build_book_context",
    ...
    monkeypatch.setattr(
        "deepmentor.services.memory.get_memory_store",
    ...
    monkeypatch.setattr(
        "deepmentor.services.skill.get_skill_service",
        _fake_skill_service,
    )
    monkeypatch.setattr(
        "deepmentor.services.persona.get_persona_service",
...
async def test_turn_runtime_persists_llm_selection_in_turn_snapshot(
    ...
    monkeypatch.setattr(
        "deepmentor.services.config.get_model_catalog_service",
        lambda: SimpleNamespace(load=_model_catalog),
    )
    monkeypatch.setattr(
        "deepmentor.services.model_selection.runtime.activate_llm_selection",
        fake_activate,
    )
    monkeypatch.setattr(
        "deepmentor.services.model_selection.runtime.reset_llm_selection",
    ...
    monkeypatch.setattr("deepmentor.services.skill.get_skill_service", _fake_skill_service)
    monkeypatch.setattr("deepmentor.services.persona.get_persona_service", _fake_persona_service)

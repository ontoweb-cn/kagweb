...
notebook_router = importlib.import_module("deepmentor.api.routers.question_notebook").router
sessions_router = importlib.import_module("deepmentor.api.routers.sessions").router
...
def store(tmp_path: Path, monkeypatch) -> SQLiteSessionStore:
    ...
    monkeypatch.setattr(
        "deepmentor.api.routers.question_notebook.get_sqlite_session_store",
        lambda: instance,
    )
    monkeypatch.setattr(
        "deepmentor.api.routers.sessions.get_sqlite_session_store",
        lambda: instance,
    )
    return instance
...
def course_service(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "deepmentor.services.courses.get_course_service",
        lambda: service,
    )
    return service

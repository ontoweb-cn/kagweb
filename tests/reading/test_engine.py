...
from deepmentor.reading import (
    Annotation,
    MaterialNotFound,
    ReadingError,
    ReadingStore,
...
from deepmentor.reading.extract import (
    SECTION_TARGET_CHARS,
    extract_material,
    first_line_label,
    split_into_sections,
)
from deepmentor.reading.models import parse_text_selectors
from deepmentor.reading.search import normalise, search_units, terms_of
from deepmentor.services.parsing.types import ParsedDocument
...
def test_scanned_pdf_uses_configured_parser_blocks_and_keeps_page_locators(
    ...
    class _ParserService:
        ...
        def parse(self, source: Path) -> ParsedDocument:
            ...
            return ParsedDocument(
                ...
                blocks=[
                    {"page_idx": 0, "type": "text", "text": "OCR page one"},
                    {"page_idx": 1, "type": "equation", "text": "x = y"},
                ],
                engine="mineru",
            )

    import deepmentor.services.parsing as parsing

    monkeypatch.setattr(parsing, "get_parse_service", lambda: _ParserService())
    ...
    assert extraction.units == ("OCR page one", "x = y")
    # Parser block text carries inline ``$...$`` math — flag it so the reader
    # typesets formulas instead of showing LaTeX source.
    assert extraction.content_format == "web_markdown"


def test_scanned_pdf_uses_markdown_text_view_when_parser_has_no_page_blocks(
    monkeypatch, tmp_path: Path
) -> None:
    path = _write_pdf(tmp_path / "scan.pdf", [""])
    ...
    assert extraction.render_mode == "text"
    assert extraction.has_raw_view is False
    assert extraction.extractor == "docling-ocr"
    assert extraction.units[0].startswith("# Scan title")
    assert extraction.content_format == "web_markdown"
    assert [entry.title for entry in extraction.outline] == ["Scan title", "Details"]


def test_scanned_pdf_reingest_upgrades_legacy_plain_text_flag(monkeypatch, tmp_path: Path) -> None:
    """An OCR PDF stored before units were flagged Markdown upgrades in place.

    Re-uploading the same scan must rebuild the manifest with
    ``web_markdown`` (formula rendering) while keeping the annotations the
    learner already made — page locators come from the same parse, so none of
    them can have moved.
    """
    ...
    class _ParserService:
        ...
        def parse(self, source: Path) -> ParsedDocument:
            return ParsedDocument(
                markdown="# Scan\n\nBody $x = y$.",
                blocks=[
                    {"page_idx": 0, "type": "text", "text": "Scan body $x = y$."},
                ],
                engine="mineru",
            )
    ...
    # Rewind the manifest to the legacy state this upgrade exists for.
    manifest_path = store.root / manifest.material_id / "manifest.json"
    legacy = json.loads(manifest_path.read_text(encoding="utf-8"))
    legacy["content_format"] = "plain_text"
    manifest_path.write_text(json.dumps(legacy, ensure_ascii=False), encoding="utf-8")

    upgraded = store.ingest(path)

    assert upgraded.content_format == "web_markdown"
    assert [row.quote for row in store.annotations(manifest.material_id)] == [
        saved.quote
    ]
...
def test_position_round_trip_validates_locator_and_anchor(
    ...
    from deepmentor.reading import ReadingPosition
...
def test_reupload_rejects_legacy_epub_upgrade_with_annotations(
    ...
    from deepmentor.reading import ReadingUpgradeConflict

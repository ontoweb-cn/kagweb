...
from deepmentor.api.routers import reading_extensions
from deepmentor.reading import ReadingStore
from deepmentor.reading.extensions import ReadingContext, ReadingExtensionRegistry
from deepmentor.reading.study_guidance import StudyGuidanceExtension
...
async def test_study_guidance_returns_a_bounded_card(monkeypatch):
    ...
    monkeypatch.setattr("deepmentor.reading.study_guidance.complete", complete)
...
def test_study_guidance_is_registered_as_a_packaged_extension():
    ...
    group = project["project"]["entry-points"]["deepmentor.reading_extensions"]

    assert group["guided_learning"] == ("deepmentor.reading.study_guidance:StudyGuidanceExtension")
...
def test_forged_selection_is_rejected_before_the_extension_runs(monkeypatch, tmp_path):
    ...
    material = ReadingStore().ingest(source)
    monkeypatch.setattr(
        "deepmentor.reading.study_guidance.complete",

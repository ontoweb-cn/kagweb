...
from deepmentor.reading.catalog_models import IngestionStatus, SourceKind
...
from deepmentor.reading.ingestion import (
...
from deepmentor.reading.models import ReadingError
from deepmentor.reading.store import ReadingStore
from deepmentor.services.web_source.snapshot_assets import SnapshotAsset
from deepmentor.tools.web_fetch import FetchOutcome, _extract_readable
...
async def test_failed_url_import_is_retryable(stores, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.ERROR, logger="deepmentor.reading.ingestion"):
...
def test_transcript_normalization_has_a_storage_budget(monkeypatch) -> None:
    monkeypatch.setattr("deepmentor.reading.ingestion.MAX_TRANSCRIPT_BYTES", 8)

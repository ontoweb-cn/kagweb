...
The store is isolated by pointing ``DEEPMENTOR_HOME`` at a temp dir and resetting
...
from deepmentor.capabilities.explore_context import ExploreContextCapability
from deepmentor.capabilities.reading import (
    MATERIAL_ID_KEY,
    MATERIAL_KWARG,
    READING_TOOL_NAMES,
    VIEWPORT_KEY,
...
from deepmentor.capabilities.reading.tools import (
    MaterialOutlineTool,
    ReaderAnnotateTool,
    ReaderGotoTool,
    ReadMaterialTool,
...
def reading_home(monkeypatch, tmp_path: Path):
    """A real per-user store rooted in a temp DEEPMENTOR_HOME."""
    monkeypatch.setenv("DEEPMENTOR_HOME", str(tmp_path))
...
def timed_material(reading_home: Path):
    from deepmentor.reading import ReadingStore, UnitReference
...
def _mode_context(**metadata) -> UnifiedContext:
    from deepmentor.capabilities.reading.capability import MODE_KEY
...
def test_nothing_open_means_no_viewport_seed_and_no_locate_pass() -> None:
    from deepmentor.capabilities.reading.capability import VIEWPORT_KEY

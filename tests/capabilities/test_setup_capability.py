...
from deepmentor.agents._shared.tool_composition import ToolMountFlags, compose_enabled_tools
from deepmentor.capabilities import active_loop_capabilities, any_exclusive_capability_active
from deepmentor.capabilities.setup import SETUP_TOOL_NAMES, SetupCapability
from deepmentor.capabilities.setup.apply import apply_setting
from deepmentor.capabilities.setup.binding import message_signals_setup, setup_gaps
from deepmentor.capabilities.setup.jobs import run_job
from deepmentor.capabilities.setup.tools import (
    ApplySettingTool,
    InspectSetupTool,
...
from deepmentor.services.config import settings_spec as spec_module
from deepmentor.services.config.settings_spec import ProbeResult, setting_specs
...
def isolated_settings(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    """
    from deepmentor.services.config import model_catalog, runtime_settings
    from deepmentor.services.path_service import PathService
    from deepmentor.services.settings import interface_settings
...
async def test_apply_personal_setting_persists(isolated_settings: Path) -> None:
    from deepmentor.services.settings.interface_settings import get_ui_settings
...
async def test_apply_rejects_an_engine_that_is_not_installed(
    ...
    """
    from deepmentor.services.config.runtime_settings import get_runtime_settings_service
...
async def test_a_non_admin_cannot_write_a_global_row(
    ...
    from deepmentor.capabilities.setup import access
...
async def test_a_hung_endpoint_cannot_hang_the_turn(
    ...
    from deepmentor.services.config.settings_spec import _PROBE_TIMEOUT_SECONDS, _probe_llm
    ...
    import deepmentor.services.llm as llm_module
...
def test_the_first_run_offer_is_not_spent_by_an_ordinary_request(
    ...
    """
    from deepmentor.capabilities.setup.binding import setup_activation
...
def test_unresolvable_identity_refuses_global_writes(
    ...
    monkeypatch.setattr(access, "_is_partner_turn", lambda: False)
    monkeypatch.setattr("deepmentor.multi_user.context.get_current_user", _explode)
...
async def test_a_silent_job_still_emits_progress(
    ...
    """
    from deepmentor.capabilities.setup import jobs
...
def test_setup_jobs_are_never_listed_twice(isolated_settings: Path) -> None:
    from deepmentor.capabilities.setup.jobs import available_jobs
...
def test_system_block_names_the_installs_gaps(isolated_settings: Path) -> None:
    context = UnifiedContext(user_message="帮我配置一下 DeepMentor")

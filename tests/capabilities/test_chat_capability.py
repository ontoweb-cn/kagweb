...
def test_turn_model_resolves_from_the_metadata_selection(monkeypatch) -> None:
    ...
    assert _build_request(context, backend=object(), session_workspace=False).model == "deepseek-v3"
    ...
    assert _build_request(dangling, backend=object(), session_workspace=False).model == ""

    bare = UnifiedContext(session_id="s", user_message="hi", metadata={})
    assert _build_request(bare, backend=object(), session_workspace=False).model == ""

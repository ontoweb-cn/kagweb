...
from deepmentor.capabilities.subagent import (
    SUBAGENT_TOOL_NAMES,
    ConsultSubagentTool,
    SubagentCapability,
    connection_for_turn,
)
from deepmentor.capabilities.subagent import binding as subagent_binding
from deepmentor.core.context import TurnRuntimeContext, UnifiedContext
from deepmentor.runtime.registry.tool_registry import get_tool_registry
from deepmentor.services.subagent.config import BackendConfig
from deepmentor.services.subagent.types import ConsultResult, SubagentEvent
...
async def test_consult_streams_events_and_threads_session(monkeypatch) -> None:
    ...
    monkeypatch.setattr("deepmentor.services.subagent.get_backend", lambda kind: backend)
...
async def test_session_id_persists_across_turns(monkeypatch, tmp_path) -> None:
    ...
    # the local agent keeps context across DeepMentor's separate messages.

...
from deepmentor.core.errors import (
    ConfigurationError,
    DeepMentorError,
...
class TestErrors:
    def test_base_error_str_without_details(self) -> None:
        err = DeepMentorError("boom")
        assert str(err) == "boom"
        assert err.details == {}

    def test_base_error_str_with_details(self) -> None:
        err = DeepMentorError("boom", details={"key": "val"})
        assert "key" in str(err)

    def test_hierarchy(self) -> None:
        assert issubclass(ConfigurationError, DeepMentorError)
        assert issubclass(ValidationError, DeepMentorError)
        assert issubclass(ServiceError, DeepMentorError)

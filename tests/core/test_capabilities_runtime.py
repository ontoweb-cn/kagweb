...
import deepmentor.agents.chat.agentic_pipeline as chat_pipeline
from deepmentor.agents.chat.capability import ChatCapability
from deepmentor.agents.question.capability import DeepQuestionCapability
from deepmentor.agents.research.capability import DeepResearchCapability
from deepmentor.agents.visualize.capability import VisualizeCapability
from deepmentor.capabilities.ask_questions.capability import AskQuestionsCapability
from deepmentor.capabilities.solve.capability import DeepSolveCapability
...
from deepmentor.runtime.bootstrap.builtin_capabilities import BUILTIN_CAPABILITY_CLASSES
...
async def test_ask_questions_capability_forces_card_on_selected_turn(
    ...
    monkeypatch.setattr(
        "deepmentor.capabilities.ask_questions.capability.AgenticChatPipeline",
        FakePipeline,
    )
...
async def test_chat_capability_streams_content_and_geogebra_context(
    ...
    monkeypatch.setattr("deepmentor.agents.chat.capability.AgenticChatPipeline", FakePipeline)
...
async def test_deep_solve_capability_runs_chat_loop_in_solve_mode(
    ...
    monkeypatch.setattr("deepmentor.capabilities.solve.capability.AgenticChatPipeline", FakePipeline)
...
async def test_deep_question_capability_uses_single_call_followup_agent(
    ...
    _install_module(
        monkeypatch,
        "deepmentor.agents.question.coordinator",
        AgentCoordinator=FakeCoordinator,
    )
    _install_module(
        monkeypatch,
        "deepmentor.agents.question.agents.followup_agent",
        FollowupAgent=FakeFollowupAgent,
    )
    _install_module(
        monkeypatch,
        "deepmentor.services.llm.config",
...
async def test_deep_research_capability_delegates_to_pipeline(
    ...
    """
    import deepmentor.agents.research.capability as deep_research_mod
    import deepmentor.agents.research.request_config  # noqa: F401
    ...
    class FakeAgenticChatPipeline:
        ...
        async def run(self, context: UnifiedContext, stream: StreamBus) -> dict[str, Any]:
            ...
            context.metadata["_visualizer_result"] = {
                "schema_version": "deepmentor.visualization/v1",
                "render_type": "svg",
                "renderer": {
                    "id": "svg",
                    "version": "1.0.0",
...
async def test_deep_question_rejects_empty_custom_topic_before_workspace(
    ...
    monkeypatch.setattr(
        "deepmentor.services.llm.config.get_llm_config",
    ...
    monkeypatch.setattr(
        "deepmentor.services.path_service.get_path_service",

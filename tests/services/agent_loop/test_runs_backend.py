...
async def test_clarify_is_surfaced_and_answered_on_its_own_endpoint(tmp_path) -> None:
    ...
    # The field is ``response`` — the name the issue's contract proposes.
    # Both Intellect implementations also accept ``answer`` as an alias here,
    # so this pins the one this client chooses to send.
...
async def test_the_degraded_poll_emits_each_event_once() -> None:
    """A dropped stream must not double the events the poll synthesizes.

    The poll runs after the subscription is gone, and its frames are yielded
    straight through, so a duplicated `yield` there multiplies every note:
    two progress markers, two usage records, and — once the poll also carries
    the run's `output` — two copies of the answer. Asserting only that the
    kinds are present (as the original test did) cannot see this, because a
    duplicate satisfies "in kinds" just as well as the single event does.
    """
    calls: list[httpx.Request] = []

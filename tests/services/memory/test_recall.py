...
from deepmentor.services.memory.snapshot.entity import EntityStamp
...
def _stub_stamps(monkeypatch: pytest.MonkeyPatch, by_surface: dict[str, list[EntityStamp]]) -> None:
    from deepmentor.services.memory.snapshot import adapters
...
def memory_root(tmp_path: Path):
    from deepmentor.services.memory.paths import memory_path_service_override

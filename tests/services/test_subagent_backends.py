...
from deepmentor.services.subagent import process as process_mod
...
from deepmentor.services.subagent.config import (
    CONSULT_BUDGET_MAX,
...
from deepmentor.services.subagent.process import resolve_cli_command, stream_process_lines
from deepmentor.services.subagent.types import ConsultResult
...
def test_claude_command_applies_model_effort_system_prompt() -> None:
    backend = ClaudeCodeBackend()
    cfg = BackendConfig(model="opus", effort="high", system_prompt="consulted by DeepMentor")
    ...
    assert "--append-system-prompt" in cmd and "consulted by DeepMentor" in cmd
...
async def test_hermes_consult_captures_answer_and_session(monkeypatch) -> None:
    from deepmentor.services.subagent import hermes as hermes_mod
...
def test_openclaw_command_owns_stable_session_key() -> None:
    ...
    assert cmd[cmd.index("--session-key") + 1] == "deepmentor-abc"
...
async def test_openclaw_consult_parses_gateway_json(monkeypatch) -> None:
    from deepmentor.services.subagent import openclaw as openclaw_mod
    ...
    assert result.success is True
    assert result.session_id.startswith("deepmentor-")
...
async def test_deepseek_headless_fallback_streams_reasoning(monkeypatch) -> None:
    from deepmentor.services.subagent import deepseek_harness as dsh_mod
...
def test_deepseek_sdk_notification_mapping() -> None:
    ...
    from deepmentor.services.subagent.deepseek_harness import _sdk_notification_events
...
def test_materialize_images_writes_only_resolvable_images(tmp_path) -> None:
    ...
    from deepmentor.core.context import Attachment
    from deepmentor.services.subagent.images import materialize_images
...
def test_parse_claude_model_screen() -> None:
    from deepmentor.services.subagent.claude_models import _parse_model_screen
...
def test_claude_models_cache_roundtrip(monkeypatch, tmp_path) -> None:
    from deepmentor.services.subagent import claude_models as cm
...
async def test_claude_options_prefers_synced_cache(monkeypatch) -> None:
    ...
    curated fallback."""
    from deepmentor.services.subagent import claude_models as cm
    from deepmentor.services.subagent import models as models_mod
...
def test_session_registry_roundtrip(monkeypatch, tmp_path) -> None:
    from deepmentor.services.subagent import sessions as sess
...
def test_registry_partner_is_non_cli_backend() -> None:
    from deepmentor.services.subagent import PARTNER_BACKEND_KIND, get_backend, list_backend_kinds
...
async def test_detect_all_excludes_partner_backend() -> None:
    from deepmentor.services.subagent import detect_all
...
def _patch_manager(monkeypatch, manager) -> None:
    import deepmentor.services.partners as partners_pkg
...
async def test_partner_consult_rechecks_revoked_grant(monkeypatch, tmp_path) -> None:
    ...
    import deepmentor.multi_user.partner_access as partner_access
    from deepmentor.multi_user.paths import user_context
    from deepmentor.services.subagent.partner import PartnerBackend
...
def test_partner_event_mapping_covers_channels() -> None:
    from deepmentor.core.stream import StreamEvent, StreamEventType
    from deepmentor.services.subagent.partner import _to_subagent_events
...
async def _drive_opencode(events, *, auto_approve=True):
    from deepmentor.services.subagent.opencode_family import OpencodeBackend
...
def test_opencode_final_text_skips_synthetic_parts() -> None:
    from deepmentor.services.subagent.opencode_family import _text_from_parts
...
def test_server_registry_reaps_idle_and_dead_handles() -> None:
    from deepmentor.services.subagent import opencode_server as srv

...
from deepmentor.services.cli_apps import get_entry
from deepmentor.services.cli_apps.installer import install_app, uninstall_app
from deepmentor.services.cli_apps.models import (
    AppRuntime,
    AppTrust,
    CliAppEntry,
    InstallKind,
...
from deepmentor.services.cli_apps.paths import (
    abi_stamp,
    app_dir,
    bin_dir,
    install_log_path,
...
from deepmentor.services.cli_apps.state import load_installed
...
def run(monkeypatch: pytest.MonkeyPatch) -> _Recorder:
    recorder = _Recorder()
    monkeypatch.setattr("deepmentor.services.cli_apps.installer._run", recorder)
    return recorder
...
def test_an_install_that_produces_no_entry_point_is_a_failure(
    ...
    monkeypatch.setattr(
        "deepmentor.services.cli_apps.installer._run", _Recorder(make_executable=False)
    )
...
def test_a_failed_update_puts_the_working_install_back(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("deepmentor.services.cli_apps.installer._run", _Recorder())
    assert asyncio.run(install_app(_entry())).ok
    executable = bin_dir("demo", AppRuntime.PYTHON) / "cli-anything-demo"
    executable.write_text("#!/bin/sh\necho original\n", encoding="utf-8")

    monkeypatch.setattr("deepmentor.services.cli_apps.installer._run", _Recorder(fail_at=1))
    outcome = asyncio.run(install_app(_entry()))

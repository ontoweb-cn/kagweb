...
from deepmentor.services.config.model_catalog import SERVICE_NAMES
...
RUNTIME_ENV_KEYS = (
    "DEEPMENTOR_VERSION_CHECK_ENABLED",
    "BACKEND_PORT",
    "FRONTEND_PORT",
...
def test_runtime_process_env_is_explicit_override(tmp_path: Path) -> None:
    service = RuntimeSettingsService(
        tmp_path / "settings",
        process_env={
            "DEEPMENTOR_VERSION_CHECK_ENABLED": "false",
            "BACKEND_PORT": "9100",
            "AUTH_ENABLED": "true",
...
def test_render_environment_uses_json_backed_runtime_names(monkeypatch, tmp_path: Path) -> None:
    ...
    assert env["DEEPMENTOR_VERSION_CHECK_ENABLED"] == "true"
    ...
    # middleware). DEEPMENTOR_AUTH_ENABLED gates the login redirect;
    # DEEPMENTOR_API_BASE_URL is where the frontend server reaches the backend
    ...
    assert env["DEEPMENTOR_AUTH_ENABLED"] == "true"
    assert env["DEEPMENTOR_API_BASE_URL"] == "http://127.0.0.1:8010"
...
def test_api_base_url_falls_back_to_ipv4_loopback(monkeypatch, tmp_path: Path) -> None:
    ...
    assert env["DEEPMENTOR_API_BASE_URL"] == "http://127.0.0.1:8042"
...
def test_api_base_url_prefers_a_configured_base_over_the_loopback(
    ...
    assert env["DEEPMENTOR_API_BASE_URL"] == "http://backend.internal:9000"
...
def test_startup_ensure_creates_missing_runtime_jsons_with_defaults(
    ...
    from deepmentor.services.config import model_catalog as model_catalog_module
    from deepmentor.services.config import runtime_settings as runtime_settings_module
...
def test_document_parsing_v1_to_v2_migration(tmp_path: Path) -> None:
    ...
    from deepmentor.services.config.runtime_settings import _atomic_write_json
...
def test_compute_ws_max_size_floor_and_inflation() -> None:
    from deepmentor.services.config.runtime_settings import compute_ws_max_size

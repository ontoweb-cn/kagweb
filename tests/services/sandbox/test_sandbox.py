...
from deepmentor.services.sandbox.backends import (
    BwrapBackend,
    RestrictedSubprocessBackend,
    _decode_process_output,
)
from deepmentor.services.sandbox.config import SandboxSettings, build_backend
from deepmentor.services.sandbox.quota import QuotaExceeded, UserExecQuota
from deepmentor.services.sandbox.service import SandboxService
...
def test_backend_selection_runner_url() -> None:
    from deepmentor.services.sandbox.backends import RunnerSidecarBackend
...
def test_backend_selection_none_without_optin() -> None:
    ...
    from deepmentor.services.sandbox.backends import RestrictedSubprocessBackend
...
def test_restricted_subprocess_prefers_configured_virtualenv(
    ...
    """Bare ``python``/``pip`` must resolve inside DeepMentor's own env.

    A Windows desktop install can inherit MSYS Python before the interpreter
    that launched DeepMentor.  That split makes ``pip install`` and the next
    ``python -c`` target different environments.
    """
    venv = tmp_path / "deepmentor-venv"
...
def test_windows_process_output_falls_back_to_the_console_encoding(
    ...
    message = "不是内部或外部命令"
    monkeypatch.setattr("deepmentor.services.sandbox.backends.sys.platform", "win32")
    monkeypatch.setattr(
        "deepmentor.services.sandbox.backends.locale.getpreferredencoding",
...
def test_runner_server_validates_request_shape() -> None:
    from deepmentor.services.sandbox.runner import server

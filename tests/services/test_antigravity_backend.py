...
async def _consult(backend: AntigravityBackend, monkeypatch, *events, exit_code: str = "0"):
    monkeypatch.setattr(
        "deepmentor.services.subagent.antigravity.stream_process_lines",
...
async def test_non_json_stdout_becomes_a_log_line_not_a_crash(monkeypatch) -> None:
    ...
    monkeypatch.setattr("deepmentor.services.subagent.antigravity.stream_process_lines", _fake)
    seen: list[Any] = []
...
def test_missing_cli_detail_says_something_the_status_chip_does_not() -> None:
    ...
    """
    from deepmentor.services.subagent.process import not_found_detail

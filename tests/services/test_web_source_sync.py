...
from deepmentor.services.web_source.crawler import (
    CrawledPage,
    CrawlResult,
    _fetch_page,
    _is_internal,
...
from deepmentor.services.web_source.sync import WebSyncResult, sync_source
...
def test_to_filename_no_prefix_collision():
    ...
    en = _to_filename("https://docs.deepmentor.info/docs/intro", "/")
    zh = _to_filename("https://docs.deepmentor.info/zh-cn/docs/intro", "/zh-cn/")
...
async def test_fetch_page_blocks_private_redirect_before_request():
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        with patch(
            "deepmentor.services.web_source.crawler._is_disallowed_host",
...
async def test_sync_source_first_run(tmp_path: Path):
    ...
    with patch(
        "deepmentor.services.web_source.crawler.crawl_docs_site", new_callable=AsyncMock
    ) as mock_crawl:
        mock_crawl.return_value = mock_result
        with patch(
            "deepmentor.knowledge.add_documents.add_documents", new_callable=AsyncMock
        ) as mock_add:
...
async def test_sync_source_unchanged_pages(tmp_path: Path):
    ...
    with patch(
        ...
        mock_crawl.return_value = mock_result
        with patch("deepmentor.knowledge.add_documents.add_documents", new_callable=AsyncMock):
            result = await sync_source("kb", source, base_dir=base_dir)
...
async def test_sources_with_same_page_path_use_distinct_raw_files(tmp_path: Path):
    ...
    with patch(
        "deepmentor.services.web_source.crawler.crawl_docs_site",
...
async def test_removed_page_is_purged_before_full_index_rebuild(tmp_path: Path):
    ...
    with patch(
        ...
        with patch(
            "deepmentor.services.rag.service.RAGService.initialize",
            new_callable=AsyncMock,
            return_value=True,
        ) as rebuild:
            outcome = await sync_source("kb", source, base_dir=base_dir)
...
def test_extract_navigation_docusaurus():
    ...
    from deepmentor.services.web_source.html_extractor import extract_navigation
...
def test_extract_headings_basic():
    ...
    from deepmentor.services.web_source.html_extractor import extract_headings
...
def test_infer_navigation_from_urls():
    ...
    from deepmentor.services.web_source.crawler import CrawledPage, _infer_navigation
...
def test_navigation_tree_building():
    ...
    from deepmentor.services.web_source.sync import _flat_to_tree
...
def test_navigation_manifest_empty():
    ...
    from deepmentor.services.web_source.sync import _build_navigation_manifest

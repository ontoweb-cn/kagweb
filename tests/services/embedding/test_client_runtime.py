...
from deepmentor.services.embedding.client import (
    EmbeddingClient,
...
async def test_embedding_client_batches_requests(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "deepmentor.services.embedding.client._resolve_adapter_class", lambda _b: _FakeAdapter
    )
...
async def test_embedding_client_serializes_concurrent_calls_without_blocking_loop(
    ...
    _FakeAdapter.instances = []
    monkeypatch.setattr(
        "deepmentor.services.embedding.client._resolve_adapter_class",
...
async def test_embedding_client_forwards_input_type_to_every_batch(monkeypatch) -> None:
    ...
    _FakeAdapter.instances = []
    monkeypatch.setattr(
        "deepmentor.services.embedding.client._resolve_adapter_class", lambda _b: _RoleAwareAdapter
    )
    client = EmbeddingClient(_build_config("openai"))
...
def test_get_embedding_client_refreshes_when_config_changes(monkeypatch) -> None:
    from deepmentor.services.embedding import client as client_module

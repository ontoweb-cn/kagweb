...
class _Collection:
    ...
    def get_list(self, page: int, per_page: int, query_params: dict | None = None) -> _Result:
        ...
        if sort:
            field = sort.lstrip("-")
            # Sequence tiebreaker: two messages written within one clock tick
            # (fast Windows runs) must still sort deterministically.
            matched.sort(
                key=lambda record: (getattr(record, field, 0), record.id),

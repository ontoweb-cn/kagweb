...
from deepmentor.services.rag.index_versioning import resolve_storage_dir_for_read
from deepmentor.services.rag.pipelines.graphrag import config as gr_config
from deepmentor.services.rag.pipelines.graphrag import engine, ingestion, storage
from deepmentor.services.rag.pipelines.graphrag.errors import (
...
from deepmentor.services.rag.pipelines.graphrag.pipeline import (
    GraphRagPipeline,
    _context_to_graph,
    _context_to_sources,
    _load_text_unit_source_names,
)
...
def test_build_settings_reuses_deepmentor_reasoning_request_options(
    binding: str,
    model: str,
...
def test_build_settings_does_not_guess_compatibility_from_model_name(monkeypatch) -> None:
    monkeypatch.setattr(
        "deepmentor.services.llm.capabilities.supports_response_format",
...
def test_compatibility_probe_resolves_candidate_without_mutating_active_model(
    ...
    from deepmentor.services.rag.pipelines.graphrag import compatibility
...
def test_embedding_preflight_uses_graphrag_client_and_checks_dimension(monkeypatch) -> None:
    ...
    assert captured == {
        "input": ["DeepMentor GraphRAG embedding compatibility test"],
        "timeout": 25,
    }
...
def test_ingestion_uses_active_parse_service_for_parser_files(tmp_path, monkeypatch) -> None:
    from deepmentor.services import parsing
...
def _stub_build(monkeypatch) -> list[dict]:
    ...
    monkeypatch.setattr(
        "deepmentor.services.config.resolve_llm_runtime_config",
        lambda: _Cfg("gpt-4o-mini", "https://llm.test/v1", "sk-llm"),
    )
    monkeypatch.setattr(
        "deepmentor.services.embedding.get_embedding_config",
...
def test_context_to_sources_maps_text_unit_to_raw_document() -> None:
    sources = _context_to_sources(
        {
            "sources": [{"id": "3", "text": "grounded content"}],
            "reports": [{"title": "Community 0", "content": "summary"}],
        },
        {"3": "Meta-Ctrl.pdf"},
    )
    ...
    assert len(sources) == 1
    assert sources[0]["title"] == "Meta-Ctrl.pdf"
    assert sources[0]["source"] == "Meta-Ctrl.pdf"
    assert sources[0]["chunk_id"] == "3"


def test_text_unit_source_names_join_documents_to_raw_files(tmp_path) -> None:
    root = tmp_path / "version-1"
    raw_dir = tmp_path / "raw"
    raw_dir.mkdir()
    (raw_dir / "Meta-Ctrl.pdf").write_bytes(b"%PDF-1.4")
    ...
    try:
        ...
        ).to_parquet(root / "output" / "documents.parquet", index=False)
        pd.DataFrame(
            [
                {
                    "id": "unit-7",
        ...
        ).to_parquet(root / "output" / "text_units.parquet", index=False)
    except ImportError:  # pragma: no cover - GraphRAG's parquet extras are optional
        pytest.skip("pandas/pyarrow is required for GraphRAG output tables")

    names = _load_text_unit_source_names(root)

    assert names["unit-7"] == "Meta-Ctrl.pdf"
    assert names["7"] == "Meta-Ctrl.pdf"


def test_context_to_graph_extracts_entities_and_edges() -> None:
    graph = _context_to_graph(
        {
            "entities": [
                {
                    "title": "Meta-Ctrl",
                    "type": "method",
                    "description": "A semantic constrained decoding framework.",
                    "degree": 4,
                },
                {
                    "title": "Ctrl-G",
                    "type": "method",
                    "description": "Syntactic constrained decoding.",
                    "degree": 3,
                },
            ],
            "relationships": [
                {
                    "source": "Meta-Ctrl",
                    "target": "Ctrl-G",
                    "description": "Builds on the probability framework of.",
                    "weight": 0.75,
                }
            ],
        }
    ...
    assert [n["id"] for n in graph["nodes"]] == ["Meta-Ctrl", "Ctrl-G"]
    assert graph["nodes"][0]["description"].startswith("A semantic")
    assert len(graph["edges"]) == 1
    assert graph["edges"][0]["source"] == "Meta-Ctrl"
    assert graph["edges"][0]["target"] == "Ctrl-G"
    assert graph["edges"][0]["weight"] == 0.75


def test_context_to_graph_accepts_graphrag_3_entity_context_and_enriches() -> None:
    graph = _context_to_graph(
        {
            "entities": [
                {
        ...
        },
        {
            "青年突击队长": {
                "type": "person",
                "description": "负责青年突击队的日常管理工作和建设。",
                "degree": 8,
            },
            "青年突击队": {
                "type": "organization",
                "description": "由青年成员组成的攻关团队。",
                "degree": 8,
            },
        },
    )
    ...
    endpoint = next(node for node in graph["nodes"] if node["id"] == "青年突击队")
    assert endpoint["type"] == "organization"
    assert endpoint["description"] == "由青年成员组成的攻关团队。"
    assert endpoint["degree"] == 8


def test_context_to_graph_adds_minimal_nodes_for_missing_endpoints() -> None:
    # local search clips entities and relationships independently, so an edge
    # can reference an entity outside the (clipped) entity list.
    graph = _context_to_graph(
        {
            "entities": [{"title": "A", "description": "a", "degree": 1}],
            "relationships": [{"source": "A", "target": "B", "description": "link", "weight": 1}],
        }
    )
...
def test_context_to_graph_returns_none_without_entity_edges() -> None:
    # global / basic searches carry no entity-relationship pairs.
    assert _context_to_graph({"entities": [], "relationships": []}) is None
    assert _context_to_graph({"reports": [{"title": "c0"}]}) is None
    assert _context_to_graph({}) is None
    assert _context_to_graph("not-a-dict") is None
...
def test_router_provider_preflight_does_not_require_a_model_probe() -> None:
    from deepmentor.api.routers import knowledge
...
def test_router_graphrag_readiness_does_not_make_a_paid_model_call(monkeypatch) -> None:
    from deepmentor.api.routers import knowledge
    from deepmentor.services.rag import preflight
    from deepmentor.services.rag.pipelines.graphrag import compatibility

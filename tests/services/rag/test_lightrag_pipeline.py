...
from deepmentor.services.llm.exceptions import LLMAPIError, LLMAuthenticationError
from deepmentor.services.parsing.types import ParsedDocument
from deepmentor.services.rag.factory import get_pipeline, list_pipelines, normalize_provider_name
from deepmentor.services.rag.index_versioning import list_kb_versions
from deepmentor.services.rag.pipelines.lightrag import config, engine, storage
from deepmentor.services.rag.pipelines.lightrag.ingress import (
    IngressError,
    bundles_root,
    freeze_document,
...
from deepmentor.services.rag.pipelines.lightrag.pipeline import (
    BatchOutcome,
    LightRagBatchError,
...
def test_lightrag_llm_selection_requires_both_catalog_ids(monkeypatch) -> None:
    ...
    for settings, expected in settings_cases:
        monkeypatch.setattr(
            "deepmentor.services.config.load_lightrag_settings",
...
def test_lightrag_llm_adapter_resolves_dedicated_catalog_selection(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "deepmentor.services.model_selection.runtime.resolve_llm_config_for_selection",
        lambda selection: selected_config,
    )
    monkeypatch.setattr("deepmentor.services.llm.client.LLMClient", Client)
...
def test_workspace_is_stable_and_version_specific(tmp_path: Path) -> None:
    ...
    assert first.startswith("deepmentor_")
...
def test_source_resolver_only_reads_version_ingress(tmp_path: Path, monkeypatch) -> None:
    ...
    assert rag._resolve_source_file_for_parser("doc.pdf", parser_engine="deepmentor") == str(
        expected.resolve()
    )
...
def test_source_resolver_rejects_noncanonical_paths(tmp_path: Path, bad: str) -> None:
    ...
    with pytest.raises(IngressError):
        rag._resolve_source_file_for_parser(bad, parser_engine="deepmentor")
...
def test_source_resolver_rejects_symlink_and_ambiguity(tmp_path: Path) -> None:
    ...
    with pytest.raises(IngressError, match="missing"):
        rag._resolve_source_file_for_parser("doc.pdf", parser_engine="deepmentor")
...
def test_vision_adapter_maps_one_base64_unchanged(monkeypatch) -> None:
    ...
    monkeypatch.setattr("deepmentor.services.llm.get_llm_client", lambda: Client())
...
def test_embedding_adapter_preserves_query_and_document_roles(monkeypatch) -> None:
    ...
    monkeypatch.setattr("deepmentor.services.embedding.get_embedding_config", EmbeddingConfig)
    monkeypatch.setattr("deepmentor.services.embedding.get_embedding_client", Client)
...
def test_llm_adapter_retries_only_transient_failures(monkeypatch) -> None:
    ...
    monkeypatch.setattr("deepmentor.services.llm.get_llm_client", Client)
...
def test_preparse_runs_once_and_bundle_ignores_later_parser_drift(
    ...
    monkeypatch.setattr("deepmentor.services.parsing.get_parse_service", ParseService)
...
def test_storage_publishes_schema_two_metadata(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "deepmentor.services.rag.embedding_signature.embedding_meta_fields",
        lambda: {"embedding": "e"},
    )
...
def test_search_failure_is_not_reported_as_empty_success(tmp_path: Path, monkeypatch) -> None:
    ...
    pipeline = LightRagPipeline(kb_base_dir=str(tmp_path))
    monkeypatch.setattr(
        "deepmentor.services.rag.pipelines.lightrag.pipeline.resolve_storage_dir_for_read",
        lambda *_args: root,
    )

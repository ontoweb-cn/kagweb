...
from deepmentor.services.parsing import cache as parsing_cache
from deepmentor.services.parsing.types import ParsedDocument
from deepmentor.services.rag.pipelines.lightrag import ingress, sidecar
...
def test_official_legacy_mineru_golden_maps_without_private_builder(tmp_path: Path) -> None:
    ...
    provenance = json.loads(
        (golden.parent / "deepmentor-fixture-provenance.json").read_text(encoding="utf-8")
    )
...
def test_authenticated_current_mineru_golden_maps_from_public_artifacts(tmp_path: Path) -> None:
    ...
    provenance = json.loads(
        (capture / "deepmentor-fixture-provenance.json").read_text(encoding="utf-8")
    )
    ...
    assert document.blocks[0].heading == "DeepMentor LightRAG Bridge Fixture"

...
from deepmentor.book.agents.source_explorer import SourceExplorer, _balanced_slice
from deepmentor.book.models import SourceChunk
...
def fake_metadata(monkeypatch):
    ...
    monkeypatch.setattr("deepmentor.multi_user.knowledge_access.resolve_kb_metadata", _resolve)
    return table
...
async def test_pageindex_is_read_by_source_explorer_agent_not_rag(monkeypatch) -> None:
    monkeypatch.setattr(
        "deepmentor.multi_user.knowledge_access.resolve_kb",
        lambda name, **_kwargs: SimpleNamespace(base_dir="/kb", name=name),
    )
    monkeypatch.setattr(
        "deepmentor.services.rag.provider_binding.resolve_bound_provider",
    ...
    monkeypatch.setattr(
        "deepmentor.services.rag.pipelines.pageindex.reasoning.read_pageindex_with_agent",
        read,
    )
    ...
    monkeypatch.setattr("deepmentor.tools.rag_tool.rag_search", no_rag)

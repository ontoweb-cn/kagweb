"""CLI smoke tests for the standalone ``deepmentor-cli`` package."""
...
from deepmentor.app import DeepMentorApp, TurnRequest
...
def _install_fake_runtime(monkeypatch, captured_requests: list[TurnRequest]) -> None:
    ...
    monkeypatch.setattr("deepmentor.app.facade.DeepMentorApp.start_turn", _start_turn)
    monkeypatch.setattr("deepmentor.app.facade.DeepMentorApp.stream_turn", _stream_turn)
...
def test_chat_repl_survives_invalid_utf8_input(monkeypatch) -> None:
    ...
    monkeypatch.setattr("deepmentor_cli.chat._read_repl_input", _read_input)
...
def test_session_list_command_uses_shared_store(monkeypatch) -> None:
    ...
    monkeypatch.setattr("deepmentor.app.facade.DeepMentorApp.list_sessions", _list_sessions)
...
def test_start_command_delegates_to_runtime_launcher(monkeypatch) -> None:
    ...
    monkeypatch.setattr("deepmentor.runtime.launcher.start", _fake_start)
...
def test_stop_command_delegates_to_detached_launcher(monkeypatch) -> None:
    calls: list[object] = []
    monkeypatch.setattr(
        "deepmentor.runtime.launcher.stop",

...
CLI-only DeepMentor distribution. It installs the `deepmentor` command and the
...
FastAPI/Uvicorn server dependencies used by `deepmentor start`.
...
the `deepmentor` command at these source files.

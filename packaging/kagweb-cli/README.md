...
CLI-only KAGWeb distribution. It installs the `kagweb` command and the
...
FastAPI/Uvicorn server dependencies used by `kagweb start`.
...
the `kagweb` command at these source files.

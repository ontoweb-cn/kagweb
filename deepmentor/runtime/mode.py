...
Controls whether DeepMentor is running as a CLI application or an API server.
...
def _resolve_mode() -> RunMode:
    raw = os.environ.get("DEEPMENTOR_MODE", "").strip().lower()
...
def set_mode(mode: RunMode) -> None:
    ...
    global _current_mode
    _current_mode = mode
    os.environ["DEEPMENTOR_MODE"] = mode.value

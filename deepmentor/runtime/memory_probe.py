"""Resident-memory snapshot of the running DeepMentor process tree.

The counterpart to :mod:`deepmentor.runtime.memory_reclaim`: that module *acts*
...
Two things make "how much memory does DeepMentor use" more than one ``getrusage``
call:

* The backend is only one of the processes. ``deepmentor.runtime.launcher``
  ...
  ``uvicorn deepmentor.api.main:app`` run would climb into the developer's shell
...
# find the root of the DeepMentor process tree it belongs to.
SUPERVISOR_PID_ENV = "DEEPMENTOR_SUPERVISOR_PID"
...
class ProcessMemory:
    """One process in the DeepMentor tree."""
...
def _classify(pid: int, name: str, cmdline: str, root_pid: int | None = None) -> str:
    ...
    the supervisor runs as a bare ``deepmentor`` console script with nothing in
    ...
    if "next" in lowered or "node" in lowered:
        return "web"
    if "uvicorn" in lowered or "deepmentor.api" in lowered:
        return "backend"
...
def _scan_proc() -> tuple[list[ProcessMemory], bool]:
    """psutil-free fallback for Linux — the platform DeepMentor ships in Docker on."""

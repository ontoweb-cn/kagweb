...
An *external provider* is something that contributes tools DeepMentor did not
...
``load_tools`` (see :mod:`deepmentor.runtime.registry.deferred_tools`). What
they do *not* share is authorisation, so that stays per-kind and explicit in
:mod:`~deepmentor.runtime.providers.authorize`.
...
upwards. Import it directly (``from deepmentor.runtime.providers.view import
...
from deepmentor.runtime.providers.text import (

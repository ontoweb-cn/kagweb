"""Branded banner + localized labels for ``deepmentor start`` / ``deepmentor init``.
...
from deepmentor.__version__ import __version__
...
LABELS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "init.next_step": "Run `deepmentor start` to launch DeepMentor.",
        "init.choice": "Choice",
        ...
        "start.detached_started": "DeepMentor is starting in the background (launcher PID {pid}).",
        "start.detached_already_running": (
            "A detached DeepMentor launcher is already running (PID {pid}). Log: {log}"
        ),
        "start.detached_log": "Log: {path}",
        "start.detached_stop_hint": 'Stop it with `deepmentor stop --home "{home}"`.',
        ...
        "start.frontend_restart_failed": (
            ...
            "Stop PID {pid} and run `deepmentor start` again."
        ),
    ...
    "zh": {
        ...
        "init.next_step": "运行 `deepmentor start` 启动 DeepMentor。",
        "init.choice": "请选择",
        ...
        "start.detached_started": "DeepMentor 正在后台启动（launcher PID {pid}）。",
        "start.detached_already_running": (
            "已有后台 DeepMentor launcher 正在运行（PID {pid}）。日志：{log}"
        ),
        "start.detached_log": "日志：{path}",
        "start.detached_stop_hint": '运行 `deepmentor stop --home "{home}"` 停止。',
        ...
        "start.frontend_restart_failed": (
            ...
            "请先停止 PID {pid},然后重新运行 `deepmentor start`。"
        ),
...
def resolve_language(default: str = "en") -> str:
    ...
    """
    try:
        from deepmentor.services.settings.interface_settings import get_ui_language
...
def render_banner(language: str | None, *, mode_key: str | None = None) -> Panel:
    ...
    return Panel(
        Align.left(body),
        title="[bold bright_cyan]DeepMentor[/]",

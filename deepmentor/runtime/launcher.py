"""Local Web launcher for the installed DeepMentor app."""
...
from deepmentor.runtime.banner import labels_for, print_banner, resolve_language
from deepmentor.runtime.home import (
    DEEPMENTOR_HOME_ENV,
    PACKAGE_ROOT,
    get_runtime_home,
    validate_runtime_home,
)
from deepmentor.runtime.memory_probe import SUPERVISOR_PID_ENV
from deepmentor.runtime.process import is_process_alive
from deepmentor.services.app_update import LAUNCHER_PID_ENV
...
SOURCE_PRODUCTION_DIST_DIR = ".next-deepmentor"
SOURCE_BUILD_MARKER = ".deepmentor-build.json"
...
DETACHED_WORKER_ENV = "DEEPMENTOR_DETACHED_WORKER"
DETACHED_TOKEN_ENV = "DEEPMENTOR_DETACHED_TOKEN"
...
def _reset_runtime_singletons() -> None:
    """Make a just-selected DEEPMENTOR_HOME visible to path/config singletons."""
    try:
...
# Readiness probes only ever target loopback URLs built by the launcher, so
# they must bypass http_proxy/https_proxy: a local proxy (e.g. Clash on
# 7890) returns 502 for 127.0.0.1 targets, and its no_proxy entries like
# "127.*" are not understood by urllib's bypass matcher.
...
def _wait_for_http(
    *,
    ...
    while time.monotonic() < deadline:
        ...
        try:
            with _LOOPBACK_OPENER.open(url, timeout=1):  # noqa: S310  # nosec B310 - http(s) health-check URL constructed by caller
...
def _http_ready(url: str, *, timeout: float) -> bool:
    try:
        with _LOOPBACK_OPENER.open(url, timeout=timeout):  # noqa: S310  # nosec B310 - launcher health check
            return True
    except (urlerror.URLError, TimeoutError, OSError):
        return False
...
def _packaged_web_dir() -> Path | None:
    try:
        import deepmentor_web
    except ImportError:
        return None
    path = Path(deepmentor_web.__file__).resolve().parent
...
def _copy_packaged_web_if_needed(
    ...
    *,
    ...
    marker = cache / ".deepmentor-web-runtime.json"
...
def _ensure_web_dependencies(source: Path, npm: str) -> None:
    ...
    if result.returncode != 0:
        raise SystemExit(
            ...
            "Fix the error above, then retry `deepmentor start`."
        )
...
def _source_production_env(*, api_base: str, auth_enabled: bool) -> dict[str, str]:
    ...
    env = os.environ.copy()
    env["DEEPMENTOR_NEXT_DIST_DIR"] = SOURCE_PRODUCTION_DIST_DIR
...
def _source_build_fingerprint(source: Path, env: dict[str, str]) -> str:
    ...
    version_file = PACKAGE_ROOT / "deepmentor" / "__version__.py"
...
def _ensure_source_production_build(
    ...
    *,
    ...
    Long-running ``deepmentor start`` processes must not use ``next dev``: its
...
def _resolve_frontend(
    ...
    *,
    ...
    if packaged is not None:
        if not node:
            raise SystemExit("Node.js 20+ is required to run the packaged DeepMentor Web app.")
    ...
    raise SystemExit(
        "DeepMentor Web assets are not installed. Install the full app with `pip install -U deepmentor`, "
...
def _launch_detached(
    ...
    *,
    ...
    env = os.environ.copy()
    env[DEEPMENTOR_HOME_ENV] = str(runtime_home.resolve())
...
def stop(home: str | Path | None = None, *, timeout: float = 15) -> bool:
    """Request a graceful stop from a detached DeepMentor launcher."""
...
def start(
    ...
    *,
    ...
    os.environ[DEEPMENTOR_HOME_ENV] = str(runtime_home)
    ...
    common_env[DEEPMENTOR_HOME_ENV] = str(runtime_home)
    ...
    common_env["DEEPMENTOR_API_BASE_URL"] = backend_url
    common_env["DEEPMENTOR_AUTH_ENABLED"] = "true" if auth_enabled else "false"
    common_env["PYTHONUNBUFFERED"] = "1"
    common_env["PYTHONIOENCODING"] = "utf-8:replace"
    # Anchor for deepmentor.runtime.memory_probe: the backend and the frontend
    # are siblings under this process, so only the supervisor's pid identifies
    # the tree that is "DeepMentor". Without it the probe can only measure the
    # backend itself.
    ...
    # ``DEEPMENTOR_NEXT_DIST_DIR`` is a build-only override.  In particular it
    ...
    # clean the live ``.next-deepmentor`` tree out from under the standalone
    ...
    common_env.pop("DEEPMENTOR_NEXT_DIST_DIR", None)

    backend_cmd = [
        sys.executable,
        "-m",
        "uvicorn",
        "deepmentor.api.main:app",
        "--host",
        "0.0.0.0",
        "--port",
        str(backend_port),
        ...
        # middleware (deepmentor/api/main.py) surfaces only non-200s, so routine

"""Runtime home resolution for installed and source DeepMentor runs."""
...
DEEPMENTOR_HOME_ENV = "DEEPMENTOR_HOME"
...
def validate_runtime_home(home: str | Path) -> None:
    ...
    if resolved in invalid_homes:
        raise ValueError(
            f"Invalid DeepMentor runtime home: {resolved}. Runtime data must live under "
            f"{PACKAGE_ROOT / 'data'} without nesting. Start DeepMentor with "
            f"DEEPMENTOR_HOME={PACKAGE_ROOT} from {PACKAGE_ROOT}."
        )
...
def get_runtime_home(home: str | Path | None = None) -> Path:
    ...
    2. ``DEEPMENTOR_HOME`` environment variable.
    ...
    raw = home if home is not None else os.getenv(DEEPMENTOR_HOME_ENV)

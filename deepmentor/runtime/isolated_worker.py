...
fixed by DeepMentor code and are never accepted from an API request.
"""
...
_WORKER_MODULE = "deepmentor.runtime.worker_process"


def _worker_limit() -> int:
    try:
        return max(1, int(os.environ.get("DEEPMENTOR_ISOLATED_WORKERS", "2") or "2"))
    except ValueError:
        return 2
...
def run_in_isolated_process_sync(
    ...
    *args: Any,
    ...
    with _WORKER_SLOTS:
        with tempfile.TemporaryDirectory(prefix="deepmentor-worker-") as temp_dir:

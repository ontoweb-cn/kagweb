...
class Extraction:
    ...
    # How the unit text should be rendered. Everything DeepMentor extracts
    # itself is plain text; only a configured document parser emits Markdown
    # (MinerU writes inline math as ``$...$``), and marking that here is what
    # lets the reader typeset formulas instead of showing LaTeX source.
...
def extract_material(path: str | Path) -> Extraction:
    ...
    if not any(unit.strip() for unit in extraction.units) and suffix == ".pdf":
        parsed_extraction = _extract_scanned_pdf_with_configured_parser(source, extraction)
...
def _extract_scanned_pdf_with_configured_parser(
    source: Path, fallback: Extraction
) -> Extraction | None:
    """Run the active document parser when a PDF has no native text layer.

    The ordinary PDF route deliberately stays local and page-faithful.  Only a
    scan reaches this fallback, which makes the configured MinerU/Docling OCR
    setting useful in Immersive Reading without changing how regular PDFs are
    stored, rendered, or annotated.

    Parsers with page-addressable blocks (MinerU) keep the raw PDF reader and
    its page locators.  Markdown-only parsers still make the scan readable in
    the text reader, where sections are the honest locator space.
    """
    from deepmentor.services.parsing import get_parse_service

    service = get_parse_service()
    engine = service.active_engine().strip().lower()
    # The built-in text-only parser uses the same no-OCR extractor that has
    # already produced no text. Avoid doing that work twice and retain the
    # existing, actionable scan message.
    if engine == "text_only":
        return None

    try:
        parsed = service.parse(source)
    except Exception as exc:  # parser adapters expose several engine-specific errors
        logger.exception("Configured parser %s failed for scanned PDF %s", engine, source.name)
        raise ReadingError(
            f"{source.name}: {engine} could not OCR this scanned PDF ({exc})"
        ) from exc

    page_units = _page_units_from_parser_blocks(parsed.blocks, page_count=len(fallback.units))
    if any(unit.strip() for unit in page_units):
        return Extraction(
            units=page_units,
            unit="page",
            extractor=f"{parsed.engine or engine}-ocr",
            has_raw_view=True,
            title=fallback.title,
            outline=fallback.outline,
            render_mode="pdf",
            # Parser block text carries inline ``$...$`` math; the reader
            # typesets it rather than showing the LaTeX source.
            content_format="web_markdown",
        )

    units, outline = split_markdown_by_headings(parsed.markdown)
    if units:
        return Extraction(
            units=units,
            unit="section",
    ...
    raise ReadingError(
        f"{source.name}: {engine} finished OCR but returned no readable content."
    )


def _page_units_from_parser_blocks(
    blocks: object, *, page_count: int
) -> tuple[str, ...]:
    """Turn parser blocks carrying zero-based ``page_idx`` values into pages.

    MinerU's public content-list format gives every semantic block a page
    index.  Keeping that association is important: a page locator must still
    point at the same page in the PDF viewer.  Unknown block shapes are simply
    ignored; the parser's canonical Markdown then remains the safe fallback.
    """
    if not isinstance(blocks, list) or page_count <= 0:
        return ()
    ...
    for block in blocks:
        ...
        if not 0 <= page_index < page_count:
            continue
        text = _parser_block_text(block)
        if text:
            pages[page_index].append(text)
    return tuple("\n\n".join(parts).strip() for parts in pages)


def _parser_block_text(block: dict[object, object]) -> str:
    """Return the readable fields of one parser block without assuming a schema."""
    pieces: list[str] = []
    for key in ("text", "table_body", "content"):
        value = block.get(key)
        if isinstance(value, str) and value.strip():
            pieces.append(value.strip())
    for key in ("table_caption", "table_footnote", "chart_caption", "chart_footnote"):
        value = block.get(key)
        if isinstance(value, str) and value.strip():
            pieces.append(value.strip())
        elif isinstance(value, list):
            pieces.extend(item.strip() for item in value if isinstance(item, str) and item.strip())
    return "\n".join(pieces)


def _extract_epub(source: Path) -> Extraction:
    """Preserve EPUB spine order so browser and assistant locators agree."""
    from deepmentor.utils.document_extractor import DocumentExtractionError, extract_epub_spine

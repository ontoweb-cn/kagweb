...
from deepmentor.reading.extract import extract_material, synthesise_outline
    ...
    def ingest(self, source: Path | str, *, filename: str | None = None) -> MaterialManifest:
        ...
        with self._locked(material_id):
            ...
            if existing is not None and self._is_complete(material_id, existing):
                ...
                # Scanned PDFs stored before OCR output was flagged as
                # Markdown: the units already contain inline ``$...$`` math,
                # only the flag — and with it formula rendering — is missing.
                # Page locators come from the same parse cache, so annotations
                # survive the rebuild untouched.
                wants_markdown_upgrade = (
                    path.suffix.lower() == ".pdf"
                    and existing.render_mode == "pdf"
                    and existing.extractor.endswith("-ocr")
                    and existing.content_format == "plain_text"
                )
                if not (wants_epub_upgrade or wants_markdown_upgrade):
                    return existing
                if wants_epub_upgrade and self.annotations(material_id):
        ...
        with self._locked(resolved_content):
            for state_dir in (ANNOTATIONS_DIR, POSITIONS_DIR):
                ...
                try:
                    ...
                    pass
        from deepmentor.reading.epub_bilingual import delete_epub_pairings_for_material

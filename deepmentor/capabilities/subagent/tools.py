...
:class:`~deepmentor.capabilities.subagent.capability.SubagentCapability`, which
...
if TYPE_CHECKING:  # avoid importing the services package at module-load time (cycle)
    from deepmentor.services.subagent import SubagentEvent
...
class ConsultSubagentTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        # Head this round with the question DeepMentor is putting to the agent, so
        ...
        if result.session_id:
            ...
            if session_key_value:
                from deepmentor.services.subagent.sessions import remember_session

...
(resolved by :mod:`deepmentor.capabilities.subagent.binding`). As a
...
from deepmentor.capabilities.protocol import KnowledgeCapability, PromptBlock
from deepmentor.capabilities.subagent.binding import connection_for_turn, subagent_refs
from deepmentor.capabilities.subagent.tools import SUBAGENT_TOOL_NAMES
...
class SubagentCapability(KnowledgeCapability):
    ...
    def augment_kwargs(
        ...
        if conn is None:
            return kwargs
        from deepmentor.services.subagent import load_subagent_settings
        from deepmentor.services.subagent.sessions import get_session, session_key
        ...
        # session the user/DeepMentor built up earlier (and the sidebar shares).
...
_DEFAULT_CONSULT_INSTRUCTION = (
    "You are being consulted programmatically by DeepMentor on the user's behalf, "
...
def _resolve_budget(context: UnifiedContext) -> int:
    ...
    from deepmentor.services.subagent import load_subagent_settings
    from deepmentor.services.subagent.config import CONSULT_BUDGET_MAX, CONSULT_BUDGET_MIN
...
def _system_text(language: str, name: str, budget: int, kind: str = "") -> str:
    from deepmentor.services.subagent import PARTNER_BACKEND_KIND

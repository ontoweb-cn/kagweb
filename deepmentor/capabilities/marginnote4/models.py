...
These types describe MN4 objects as they land in DeepMentor after sync.
...
* ``raw`` preserves the original MN4 JSON for fields DeepMentor does not model
...
class MarginNoteObject:
    ...
    Attributes:
        ...
        synced_at:  ISO-8601 timestamp when DeepMentor received this version.
...
@dataclass(slots=True)
class PairedDevice:
    """A MarginNote 4 device registered with this DeepMentor instance.
...
class LearningEvent:
    ...
    Captures a review or study interaction so DeepMentor can track mastery

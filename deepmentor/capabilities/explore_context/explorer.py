...
* **Fallback** (:meth:`_single_pass`) — for providers without native tool
  ...
  single pass, modelled on :class:`~deepmentor.agents.notebook.analysis_agent`.
"""
...
from deepmentor.services.llm import clean_thinking_tags, get_llm_config, get_token_limit_kwargs
...
class ContextExplorer:
    ...
    async def _call_llm(
        ...
        if threads_session_id(self.binding):
            kwargs["deepmentor_session_id"] = f"{session_id}:explore"
        if tool_schemas:

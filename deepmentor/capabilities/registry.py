...
from deepmentor.capabilities.protocol import LoopExtension
from deepmentor.core.context import UnifiedContext
from deepmentor.core.entry_points import load_entry_point_group
from deepmentor.runtime.capability_catalog import EmptyConfig, get_capability_catalog

logger = logging.getLogger(__name__)

EXTENSIONS_GROUP = "deepmentor.extensions"
LOOP_CAPABILITIES_GROUP = "deepmentor.loop_capabilities"
...
BUILTIN_LOOP_CAPABILITY_SPECS: tuple[LoopCapabilitySpec, ...] = (
    LoopCapabilitySpec(
        "ask_questions",
        "deepmentor.capabilities.ask_questions.loop:AskQuestionsLoopCapability",
    ),
    LoopCapabilitySpec("mastery", "deepmentor.capabilities.mastery.loop:MasteryLoopCapability"),
    LoopCapabilitySpec("solve", "deepmentor.capabilities.solve.loop:SolveLoopCapability"),
    LoopCapabilitySpec("obsidian", "deepmentor.capabilities.obsidian.capability:ObsidianCapability"),
    LoopCapabilitySpec(
        "marginnote4",
        "deepmentor.capabilities.marginnote4.capability:MarginNoteCapability",
    ),
    LoopCapabilitySpec(
        "subagent",
        "deepmentor.capabilities.subagent.capability:SubagentCapability",
    ),
    LoopCapabilitySpec("ima", "deepmentor.capabilities.ima.capability:ImaCapability"),
    LoopCapabilitySpec(
        "immersive_reading",
        "deepmentor.capabilities.reading.capability:ReadingCapability",
    ),
    LoopCapabilitySpec(
        "course_study",
        "deepmentor.capabilities.course_study.capability:CourseStudyLoopCapability",
    ),
    LoopCapabilitySpec(
        "immersive_watching",
        "deepmentor.capabilities.watching.capability:WatchingCapability",
    ),
    LoopCapabilitySpec(
        "explore_context",
        "deepmentor.capabilities.explore_context.capability:ExploreContextCapability",
    ),
    LoopCapabilitySpec("setup", "deepmentor.capabilities.setup.capability:SetupCapability"),
    LoopCapabilitySpec(
        "partner_authoring",
        "deepmentor.capabilities.partner_authoring.capability:PartnerAuthoringCapability",
    ),
    LoopCapabilitySpec(
        "partner_group",
        "deepmentor.capabilities.partner_group.capability:PartnerGroupCapability",
    ),
    LoopCapabilitySpec(
        "visualization_generation",
        "deepmentor.visualizers.loop_capability:VisualizationLoopCapability",
    ),
)

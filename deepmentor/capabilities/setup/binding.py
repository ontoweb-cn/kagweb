...
def setup_gaps() -> tuple[SetupGap, ...]:
    ...
    """
    from deepmentor.services.config.settings_spec import setting_specs
...
# configuring DeepMentor itself.
_DIRECT_PATTERN = re.compile(
    r"(帮我配置|帮我设置|自己配置|配置一下\s*deepmentor|设置一下\s*deepmentor|初始化配置)"
    r"|(\bconfigure\s+deepmentor\b|\bset\s?up\s+deepmentor\b|\bsetup\s+wizard\b)",
    re.IGNORECASE,
)

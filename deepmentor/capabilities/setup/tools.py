"""The four tools through which DeepMentor configures itself.

They are deliberately generic: each one walks
:mod:`deepmentor.services.config.settings_spec` rather than knowing anything
...
``apply_setting``
    Commit one row, after the checks in :mod:`deepmentor.capabilities.setup.apply`.
...
class InspectSetupTool(BaseTool):
    ...
    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            name="inspect_setup",
            description=(
                "Read DeepMentor's own configuration: every setting you can "
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from deepmentor.capabilities.setup.access import can_write
        from deepmentor.capabilities.setup.binding import setup_gaps
        from deepmentor.capabilities.setup.jobs import available_jobs
        from deepmentor.services.config.settings_spec import specs_for_area
...
class ApplySettingTool(BaseTool):
    ...
    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            name="apply_setting",
            description=(
                "Change one DeepMentor setting. The value must be one of the "
    ...
    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            ...
            description=(
                ...
                "entered directly into DeepMentor. Call it, tell the user what "

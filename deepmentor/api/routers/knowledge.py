...
from deepmentor.api.utils.task_log_stream import capture_task_logs, get_task_stream_manager
from deepmentor.knowledge.add_documents import DocumentAdder, remove_raw_document
from deepmentor.knowledge.initializer import KnowledgeBaseInitializer
from deepmentor.knowledge.kb_types import is_connected_kb, supports_local_raw_files
from deepmentor.knowledge.manager import KnowledgeBaseManager
from deepmentor.knowledge.naming import validate_knowledge_base_name
from deepmentor.knowledge.progress_tracker import ProgressStage, ProgressTracker
from deepmentor.logging import PROCESS_LOG_PRIVATE_ATTR
from deepmentor.multi_user.context import get_current_user
from deepmentor.multi_user.knowledge_access import (
    assert_writable,
    current_kb_base_dir,
    current_kb_manager,
...
from deepmentor.services.config import PROJECT_ROOT, load_config_with_main
from deepmentor.services.file_io import atomic_write_json
...
from deepmentor.services.rag.linked_kb import (
    LINKABLE_PROVIDERS,
    assert_path_allowed,
    probe_linked_folder,
)
from deepmentor.services.rag.pipelines.ima.client import (
    MAX_PAGE_LIMIT,
    ImaAPIError,
    ImaAuthError,
    ImaClient,
...
from deepmentor.services.rag.pipelines.ima.config import (
    ImaConfig,
    ImaCredentials,
...
from deepmentor.utils.document_validator import DocumentValidator
from deepmentor.utils.error_utils import format_exception_message
...
def _save_zip_archive(
    ...
    from deepmentor.utils.archive_extractor import ArchiveTooLargeError, safe_extract_zip
...
def _save_uploaded_files(
    ...
    from deepmentor.services.pocketbase_client import is_pocketbase_enabled
...
def _upload_file_to_pb(kb_name: str, filename: str, file_path: Path) -> None:
    ...
    try:
        from deepmentor.services.pocketbase_client import get_pb_client
...
def _assert_provider_ready(provider: str) -> None:
    ...
    if provider == PAGEINDEX_PROVIDER:
        from deepmentor.services.rag.pipelines.pageindex.config import is_pageindex_configured
    ...
    if provider == GRAPHRAG_PROVIDER:
        from deepmentor.services.rag.pipelines.graphrag.config import is_graphrag_available
        ...
        if not is_graphrag_available():
            raise HTTPException(
                ...
                detail=(
                    ...
                    "`pip install 'deepmentor[graphrag]'` on the server before "
    ...
    if provider == LIGHTRAG_PROVIDER:
        from deepmentor.services.rag.pipelines.lightrag.config import is_lightrag_available
        ...
        if not is_lightrag_available():
            raise HTTPException(
                ...
                detail=(
                    ...
                    "`pip install 'deepmentor[rag-lightrag]'` on the server before "
...
def _enforce_provider_formats(provider: str, files: list[UploadFile]) -> None:
    ...
    if provider not in {PAGEINDEX_PROVIDER, PAGEINDEX_OSS_PROVIDER}:
        return
    from deepmentor.services.rag.pipelines.pageindex.pipeline import (
...
def _matching_index_is_valid(kb_name: str, matching_version: dict | None) -> bool:
    ...
    if not matching_version:
        return False
    try:
        from deepmentor.services.rag.index_probe import inspect_provider_version
        from deepmentor.services.rag.pipelines.llamaindex.storage import (
...
async def get_rag_providers():
    ...
    try:
        ...
        from deepmentor.services.rag.service import RAGService
...
def _pageindex_config_payload() -> dict:
    ...
    from deepmentor.services.config import get_runtime_settings_service
...
def _validate_lightrag_llm_selection(profile_id: str, model_id: str) -> None:
    ...
    if not profile_id:
        return
    try:
        from deepmentor.services.config import get_model_catalog_service
...
async def update_lightrag_server_pipeline_config(payload: LightRagServerConfigUpdate):
    ...
    try:
        from deepmentor.services.config import get_runtime_settings_service
        from deepmentor.services.rag.pipelines.lightrag_server.config import normalize_base_url
...
async def _probe_graphrag_model_compatibility(profile_id: str, model_id: str) -> dict:
    ...
    from deepmentor.services.rag.pipelines.graphrag.compatibility import (
...
async def update_kb_config(kb_name: str, config: dict):
    ...
    try:
        from deepmentor.services.config import get_kb_config_service
        from deepmentor.services.rag.index_probe import has_ready_provider_index
...
async def probe_lightrag_server_route(payload: ProbeLightRagServerRequest):
    ...
    """
    from deepmentor.services.rag.pipelines.lightrag_server.probe import probe_server
    ...
    if payload.use_saved_api_key and not api_key:
        from deepmentor.services.config import load_lightrag_server_settings
        from deepmentor.services.rag.pipelines.lightrag_server.config import normalize_base_url
...
async def connect_lightrag_server_route(payload: ConnectLightRagServerRequest):
    ...
    """
    from deepmentor.services.rag.pipelines.lightrag_server.config import SUPPORTED_MODES
    from deepmentor.services.rag.pipelines.lightrag_server.probe import probe_server
...
async def probe_weknora_route(payload: ProbeWeKnoraRequest):
    ...
    from deepmentor.services.rag.pipelines.weknora.probe import probe_weknora
...
async def probe_ima_route(payload: ProbeImaRequest):
    ...
    Creates nothing.
    """
    from deepmentor.services.rag.pipelines.ima.probe import probe_knowledge_base
...
def _resolve_kb_raw_dir(kb_name: str, *, allow_unsupported: bool = False) -> Path | None:
    ...
    DeepMentor-managed raw directory.  File listing treats that as an empty
...
async def delete_kb_file(kb_name: str, filename: str):
    ...
    if provider in {PAGEINDEX_PROVIDER, PAGEINDEX_OSS_PROVIDER}:
        from deepmentor.services.rag.factory import get_pipeline
...
async def run_reindex_task(kb_name: str, base_dir: str, task_id: str, signature_hash: str) -> None:
    ...
    with capture_task_logs(task_id):
        try:
            ...
            # provider=None → RAGService resolves the KB's DeepMentor-bound
...
async def reindex_knowledge_base(
    ...
    try:
        ...
        if provider_uses_embedding_versions(kb_provider):
            from deepmentor.services.rag.embedding_signature import signature_from_embedding_config
...
async def websocket_progress(websocket: WebSocket, kb_name: str):
    ...
    from deepmentor.api.routers.auth import ws_auth_failed, ws_require_auth
    from deepmentor.multi_user.context import reset_current_user
...
async def sync_github_sources(kb_name: str):
    with _knowledge_source_errors(kb_name):
        ...
        from deepmentor.services.github_source.sync import sync_source
...
async def sync_web_sources(kb_name: str):
    ...
    with _knowledge_source_errors(kb_name):
        ...
        from deepmentor.services.web_source.sync import sync_source

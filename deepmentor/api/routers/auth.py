...
from deepmentor.services.config import load_auth_settings
...
from deepmentor.multi_user.context import set_current_user, user_from_token_payload
from deepmentor.multi_user.device_credentials import (
...
from deepmentor.services.codex_auth.contracts import CodexAuthError
from deepmentor.services.codex_auth.service import deliver_codex_oauth_callback
...
def _local_admin_token_payload() -> TokenPayload:
    ...
    ``deepmentor/multi_user/paths.py``.
    """
...
async def receive_codex_oauth_callback(
    ...
    try:
        ...
        return HTMLResponse(
            (
                "<!doctype html><title>DeepMentor Codex</title>"
                "<p>Authentication could not be received. Return to DeepMentor and try again.</p>"
            ),
    ...
    return HTMLResponse(
        (
            "<!doctype html><title>DeepMentor Codex</title>"
            "<p>Authentication received. You can return to DeepMentor.</p>"
        ),
        headers=headers,
    )
...
async def update_profile(
    ...
    from deepmentor.multi_user.identity import delete_avatar_file
...
async def put_current_learner_profile(
    ...
    _require_local_learner(current)
    from deepmentor.multi_user.learner_profile import normalize_profile
...
async def get_learner_profile(username: str, _: TokenPayload = Depends(require_admin)) -> dict:
    ...
    from deepmentor.multi_user.identity import get_user
...
async def admin_create_user(
    ...
    for item in list_users():
        if item.get("username") == body.username:
            ...
            break
    if preset == "learner":
        from deepmentor.multi_user.grants import learner_grant, save_grant

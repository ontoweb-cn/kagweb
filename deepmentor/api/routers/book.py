...
from deepmentor.api.utils.http_headers import content_disposition
from deepmentor.book import progress as progress_ops
from deepmentor.book.errors import BookPausedError
from deepmentor.book.export import export_filename, render_book_markdown
from deepmentor.book.models import (
    BlockType,
    BookProposal,
    ContentType,
    LearningCapture,
...
from deepmentor.book.storage import get_book_storage
from deepmentor.book.streaming import SOURCE as BOOK_SOURCE
from deepmentor.multi_user.audit import log_admin_action, log_usage
from deepmentor.multi_user.book_access import (
    ResolvedBook,
    accessible_books,
    can_create_book,
    resolve_book,
...
from deepmentor.multi_user.identity import remove_book_permission_overrides
...
def get_book_engine():
    ...
    from deepmentor.book.engine import get_book_engine as resolve
...
def _auth_enabled() -> bool:
    from deepmentor.services.auth import AUTH_ENABLED
...
async def estimate_basis(depth: str = "standard") -> dict[str, Any]:
    ...
    """
    from deepmentor.book.estimate import chapter_basis
...
async def block_types() -> dict[str, list[dict[str, str | bool]]]:
    from deepmentor.book.agents.page_planner import (
...
async def quiz_attempt(req: QuizAttemptRequest) -> dict[str, Any]:
    ...
    if req.is_correct is not None and resolved_question is not None and book is not None:
        ...
        try:
            from deepmentor.services.session import get_sqlite_session_store
...
async def book_websocket(ws: WebSocket) -> None:
    ...
    **Actions** — run an engine operation and reply with a single result::
    ...
    Actions publish into the book's own stream (see :mod:`deepmentor.book.event_hub`),
    ...
    from deepmentor.book.event_hub import get_book_bus

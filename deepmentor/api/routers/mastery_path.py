...
from deepmentor.learning import policy as learning_policy
from deepmentor.learning import prompts as learning_prompts
from deepmentor.learning.models import (
    KnowledgePoint,
    KnowledgeType,
    LearningModule,
    LearningProgress,
...
from deepmentor.learning.service import LearningService
from deepmentor.learning.storage import LearningStore
from deepmentor.learning.topic_generation import MAX_MODULE_LIMIT
from deepmentor.services.settings.interface_settings import get_response_language
from deepmentor.utils.json_parser import parse_json_response
...
async def _cancel_active_learning_turn(book_id: str) -> None:
    from deepmentor.services.session import get_turn_runtime_manager
...
async def _exclusive_path_mutation(book_id: str):
    ...
    from deepmentor.learning.storage import PathLeaseConflictError
...
async def generate_topic_route(body: GenerateTopicDraftRequest):
    from deepmentor.learning.topic_generation import TopicGenerationError, generate_topic_draft
...
async def create_topic(body: ConfirmTopicRequest):
    from deepmentor.learning.topic_generation import (
...
async def set_learner_override(
    ...
    async with _exclusive_path_mutation(path_id):
        try:
            ...
            from deepmentor.learning.service import MasteryInteractionError
...
async def list_topic_sessions(path_id: str):
    ...
    from deepmentor.learning.navigation import topic_sessions
...
async def get_topic_ask_hint(path_id: str, session_id: str = ""):
    ...
    """
    _validate_book_id(path_id)
    from deepmentor.services.mastery_hints import get_ask_hint
...
async def mastery_topic_websocket(ws: WebSocket) -> None:
    ...
    from deepmentor.learning.event_hub import mastery_topic_event_hub
...
async def get_progress(book_id: str):
    ...
    if progress.pending_question is not None:
        from deepmentor.learning.pending import public_pending_question
...
async def generate_from_notebook(book_id: str, body: GenerateFromNotebookRequest):
    ...
    from deepmentor.services.llm import complete
...
async def generate_from_reading(book_id: str, body: GenerateFromReadingRequest):
    ...
    from deepmentor.reading.knowledge_capture import mastery_source_records
    ...
    try:
        ...
        from deepmentor.reading import ReadingError

...
to pair with this DeepMentor instance and push synced study data.

Authentication layers:
* ``/pair``, ``/devices``, ``/status`` -- DeepMentor session auth (the logged-in
...
from deepmentor.api.routers.auth import require_auth
from deepmentor.capabilities.marginnote4.models import MarginNoteObject, SyncBatch
from deepmentor.capabilities.marginnote4.store import MarginNoteStore, resolve_db_path
...
# -- session-authenticated endpoints (DeepMentor user) -----------------------
...
async def pair_device(body: PairRequest, request: Request) -> PairResponse:
    """Pair a new MN4 device. Requires a DeepMentor session.

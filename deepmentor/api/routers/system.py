...
from deepmentor.api.contracts.turn_protocol import (
...
def get_token_limit_kwargs(*args, **kwargs):
    from deepmentor.services.llm.config import get_token_limit_kwargs as resolve
...
def web_search(*args, **kwargs):
    from deepmentor.services.search import web_search as search
...
async def request_managed_update(_request: ManagedUpdateRequest) -> dict[str, Any]:
    ...
    if not launcher_available():
        raise HTTPException(
            status_code=409,
            detail="Web updates require DeepMentor to be running under `deepmentor start`.",
        )
    installation = get_update_installation()
    ...
    if not result.update_available:
        raise HTTPException(status_code=409, detail="No newer DeepMentor release is available")
    ...
    if job is None:
        raise HTTPException(
            status_code=409,
            detail="Finish the active conversation before updating DeepMentor.",
        )
...
async def get_memory_usage():
    """Resident memory of the running DeepMentor process tree.
...
async def test_search_connection():
    ...
    try:
        ...
        result = web_search("DeepMentor health check", provider=search_config.provider)
        response_time = (time.time() - start_time) * 1000

...
from deepmentor.app.container import get_application_container
from deepmentor.multi_user.context import get_current_user
from deepmentor.multi_user.partner_access import (
...
from deepmentor.partners.config.paths import get_partner_media_dir
...
from deepmentor.services.partners import (
    get_partner_manager,
    slugify_partner_id,
    slugify_soul_id,
)
...
from deepmentor.services.partners.drafts import PartnerDraftStore
...
from deepmentor.services.partners.runtime_status import (
    get_partner_runtime_status_repository,
)
from deepmentor.services.partners.workspace import (
    list_assets,
    provision_assets,
    read_soul,
    remove_asset,
...
def _validate_channels_payload(channels: dict) -> None:
    ...
    from deepmentor.api.routers._partners_channel_schema import resolve_config_model
    from deepmentor.partners.channels.registry import discover_all
...
def _validate_llm_selection_payload(
    ...
    from deepmentor.multi_user.model_access import apply_allowed_llm_selection
    from deepmentor.services.config import get_model_catalog_service
    from deepmentor.services.model_selection import apply_llm_selection_to_catalog
    from deepmentor.services.partners.model_runtime import normalize_partner_llm_selection
...
def _caller_tool_reach() -> tuple[set[str] | None, set[str] | None]:
    ...
    from deepmentor.multi_user.tool_access import allowed_mcp_tools, allowed_optional_tools
...
def _resolve_soul_content(soul: SoulSpec | None) -> tuple[str, dict[str, str]]:
    ...
    from deepmentor.services.partners.workspace import DEFAULT_SOUL
...
async def start_partner(partner_id: str):
    ...
    # comes back in this state after a DeepMentor restart (a manual /stop clears
...
async def create_partner_link_code(partner_id: str):
    ...
    """
    from deepmentor.services.partners.links import issue_link_code
...
async def list_partner_links(partner_id: str):
    ...
    from deepmentor.services.partners.links import list_links
...
async def delete_partner_link(partner_id: str, key: str):
    ...
    from deepmentor.services.partners.links import remove_link
...
async def partner_command_palette():
    from deepmentor.services.partners.commands import partner_command_palette
...
def _partner_upload_caps() -> tuple[int, int]:
    try:
        from deepmentor.services.config.runtime_settings import get_chat_attachment_limits
...
async def _partner_chat_stream(
    ...
    from deepmentor.core.stream import StreamEventType
...
async def partner_chat_ws(ws: WebSocket, partner_id: str):
    ...
    * ``{"type": "done"}`` / ``{"type": "error"}`` / ``{"type": "proactive"}``.
    ...
    from deepmentor.services.partners.interaction import personal_actor_id

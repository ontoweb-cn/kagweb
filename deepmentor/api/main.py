...
def validate_tool_consistency():
    ...
    """
    try:
        from deepmentor.runtime.registry.capability_registry import get_capability_registry
...
async def lifespan(app: FastAPI):
    ...
    try:
        from deepmentor.events.event_bus import get_event_bus
    ...
    try:
        from deepmentor.services.pocketbase_client import ping_pocketbase
    ...
    try:
        from deepmentor.services.mcp import get_mcp_manager
    ...
    try:
        from deepmentor.services.llm.provider_factory import close_runtime_provider_pool
...
app = FastAPI(
    ...
    # See: https://github.com/HKUDS/DeepMentor/issues/112
...
# ones worth seeing.
#
# The `deepmentor.access` logger gets its own INFO stdout handler rather than
...
_access_logger = logging.getLogger("deepmentor.access")
if not any(getattr(h, "_deepmentor_access_handler", False) for h in _access_logger.handlers):
    ...
    _access_handler._deepmentor_access_handler = True  # type: ignore[attr-defined]
...
from deepmentor.api.routers.multi_user import router as multi_user_router  # noqa: E402
...
@app.get("/")
async def root():
    return {"message": "Welcome to DeepMentor API"}
...
if __name__ == "__main__":
    from deepmentor.api.run_server import main as run_server_main

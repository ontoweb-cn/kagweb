...
Kept in its own low-level module so both :mod:`deepmentor.knowledge.manager`
...
# indexing and document management; DeepMentor only stores retrieval credentials.
...
def supports_local_raw_files(entry: Any) -> bool:
    """Whether the KB owns a DeepMentor-managed local ``raw/`` directory.
    ...
    DeepMentor's raw-file upload and management API.
    """

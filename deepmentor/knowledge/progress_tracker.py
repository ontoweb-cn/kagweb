...
class ProgressTracker:
    ...
    def _notify(self, progress: dict):
        ...
        from deepmentor.runtime.mode import is_server

        if is_server():
            try:
                from deepmentor.knowledge.progress_events import broadcast_progress
    ...
    def update(
        ...
        try:
            ...
            fallback_logger = logging.getLogger("deepmentor.ProgressTracker")
            prefix = f"[{self.task_id}]" if self.task_id else ""
        ...
        if self.task_id:
            try:
                from deepmentor.knowledge.progress_events import emit_task_progress

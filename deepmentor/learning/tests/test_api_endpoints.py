...
from deepmentor.api.routers.mastery_path import router, ws_router
from deepmentor.learning.models import LearningProgress, PendingQuestion, QuizAttempt
...
def app(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "deepmentor.api.routers.mastery_path.LearningStore",
...
class TestListProgress:
    def test_list_runs_blocking_store_work_in_worker_thread(self, client):
        with patch(
            "deepmentor.api.routers.mastery_path.asyncio.to_thread",
...
class TestTopicProductApi:
    ...
    def test_generate_mixed_source_draft_without_persisting(self, client):
        ...
        with patch(
            "deepmentor.learning.topic_generation.complete",
...
class TestGenerateFromNotebook:
    ...
    @patch("deepmentor.services.llm.complete", new_callable=AsyncMock)
    ...
    @patch("deepmentor.api.routers.mastery_path.get_response_language", return_value="en")
    @patch("deepmentor.services.llm.complete", new_callable=AsyncMock)
    ...
    @patch("deepmentor.api.routers.mastery_path.get_response_language", return_value="zh")
    @patch("deepmentor.services.llm.complete", new_callable=AsyncMock)

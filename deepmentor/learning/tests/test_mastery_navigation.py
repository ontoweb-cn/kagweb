...
The navigation layer (:mod:`deepmentor.learning.navigation`) and the four
permanently-mounted tools (:mod:`deepmentor.tools.mastery_nav`) answer "what am
...
from deepmentor.learning import navigation
...
from deepmentor.tools.mastery_nav import (
...
def session_store(tmp_path, monkeypatch) -> SQLiteSessionStore:
    ...
    monkeypatch.setattr("deepmentor.services.session.get_session_store", lambda: store)
    monkeypatch.setattr("deepmentor.services.session.get_sqlite_session_store", lambda: store)
    return store

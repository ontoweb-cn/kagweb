...
try:
    ...
    raise ImportError(
        "Matrix dependencies not installed. Run: pip install deepmentor[matrix]"
    ) from e
...
class MatrixConfig(DeliveryOverrides):
    ...
    async def start(self) -> None:
        ...
        if self.config.e2ee_enabled and not MATRIX_E2EE_AVAILABLE:
            raise ImportError(
                ...
                "`deepmentor[matrix-e2e]` or `requirements/matrix-e2e.txt`, and make "
    ...
    def _decrypt_media_bytes(self, event: MatrixMediaEvent, ciphertext: bytes) -> bytes | None:
        if decrypt_attachment is None:
            logger.warning(
                ...
                "Install deepmentor[matrix-e2e] to decrypt encrypted media."
            )
            return None
        key_obj, hashes, iv = (

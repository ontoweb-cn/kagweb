...
from deepmentor.partners.channels.weixin_qr import ILINK_APP_ID
from deepmentor.partners.config.schema import DeliveryOverrides
from deepmentor.partners.helpers import safe_filename, split_message
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not self._token:
            ...
            # DeepMentor (or auto-starting a Partner) must never take over the
    ...
    async def _send_text(
        ...
        client_id = f"deepmentor-{uuid.uuid4().hex[:12]}"

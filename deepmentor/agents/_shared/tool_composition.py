...
def default_optional_tools(excluded: Iterable[str] = ()) -> list[str]:
    ...
    Sourced from :mod:`deepmentor.tools.builtin` so the /settings/tools UI
...
def compose_enabled_tools(
    ...
    *adds* its owned tools — it never curates or suppresses the reused
    ...
    :class:`~deepmentor.capabilities.protocol.KnowledgeCapability`): the turn
...
def user_has_mastery_topics() -> bool:
    ...
    """
    try:
        from deepmentor.learning.navigation import learner_has_topics

...
from deepmentor.agents.chat.agent_loop import AgentLoop
from deepmentor.agents.chat.context_budget import LLMRequestSnapshot, build_context_budget
from deepmentor.agents.chat.prompt_blocks import ChatPromptAssembler
from deepmentor.capabilities import (
    LoopExtension,
    PromptBlock,
    active_loop_capabilities,
    any_exclusive_capability_active,
)
from deepmentor.capabilities.protocol import END_LOOP
from deepmentor.core.context import UnifiedContext
from deepmentor.core.tool_protocol import ToolLookup
from deepmentor.core.trace import (
    build_trace_metadata,
    derive_trace_metadata,
    merge_trace_metadata,
    new_call_id,
)
from deepmentor.knowledge.manifest import KbManifest, render_manifest_note
from deepmentor.runtime.agentic import (
    DispatchOutcome,
    LLMClientConfig,
    UsageTracker,
...
from deepmentor.runtime.agentic.tool_dispatch import MAX_PARALLEL_TOOL_CALLS
from deepmentor.runtime.providers import ToolScope
from deepmentor.runtime.providers.view import ProviderToolView, build_tool_view
from deepmentor.runtime.registry.deferred_tools import DeferredToolLoader
...
from deepmentor.services.cli_apps.models import TOOL_PREFIX as CLI_APP_TOOL_PREFIX
from deepmentor.services.config import get_chat_params
from deepmentor.services.llm import (
    get_llm_config,
    get_token_limit_kwargs,  # noqa: F401  (re-exported for tests)
    prepare_multimodal_messages,
    supports_tools,  # noqa: F401  (re-exported for tests)
)
from deepmentor.services.llm.context_window import resolve_effective_context_window
from deepmentor.services.prompt import get_prompt_manager
from deepmentor.tools.builtin import PARTNER_BUILTIN_TOOL_NAMES
...
def _drop_unconfigured_generation_tools(tools: list[str]) -> list[str]:
    ...
    if not present:
        return tools
    try:
        from deepmentor.services.config.model_catalog import get_model_catalog_service
...
class AgenticChatPipeline:
    ...
    async def _pageindex_sdk_tool_bundles(self, context: UnifiedContext):
        from deepmentor.multi_user.knowledge_access import resolve_kb
        from deepmentor.services.rag.factory import PAGEINDEX_OSS_PROVIDER, PAGEINDEX_PROVIDER
        from deepmentor.services.rag.pipelines.pageindex.tools import build_sdk_tool_bundle
        from deepmentor.services.rag.provider_binding import resolve_bound_provider
    ...
    async def _exec_allowed(self, context: UnifiedContext) -> bool:
        try:
            from deepmentor.services.sandbox import IsolationLevel, get_sandbox_service
            ...
            if level is IsolationLevel.SYSTEM:
                ...
                from deepmentor.multi_user.tool_access import exec_override
    ...
    async def _execute_tool_call(
        ...
        *,
        ...
        from deepmentor.runtime.agentic import execute_tool_call
    ...
    def _augment_tool_kwargs(
        ...
        if tool_name == "rag":
            ...
            from deepmentor.services.sandbox import Mount
    ...
    async def _seed_search_one_kb(
        ...
        display_args = {"query": query, "kb_name": kb_name, "mode": "hybrid"}
        # KB seed retrieval invokes the same ``rag`` tool as model-initiated
        # calls, but it runs before the reasoning loop. Emit the paired trace
        # here too: without these events the activity panel has no
        # ``tool_result`` to attach retrieved content, sources, or the
        # GraphRAG subgraph to.
        await stream.tool_call(
            tool_name="rag",
            args=display_args,
            source=self.event_source,
            stage=self.event_stage,
            metadata=derive_trace_metadata(retrieve_meta, trace_kind="tool_call"),
        )
        ...
        if not result.get("success"):
            await stream.tool_result(
                tool_name="rag",
                result=str(result.get("result_text") or "RAG retrieval failed"),
                source=self.event_source,
                stage=self.event_stage,
                    ...
                    retrieve_meta,
                    {
                        ...
                        "tool_metadata": {
                            ...
                            **(
                                ...
                                and result["metadata"].get("provider")
                                else {}
                            ),
                        },
                    },
        ...
        if metadata.get("error_type") or metadata.get("needs_reindex"):
            await stream.tool_result(
                tool_name="rag",
                result=str(metadata.get("answer") or metadata.get("content") or "RAG retrieval failed"),
                source=self.event_source,
                stage=self.event_stage,
                metadata=merge_trace_metadata(
                    retrieve_meta,
                    {
                        ...
                        "tool_metadata": {
                            ...
                            "needs_reindex": metadata.get("needs_reindex"),
                            **(
        ...
        if not text:
            await stream.tool_result(
                tool_name="rag",
                result="RAG retrieval returned no content",
                source=self.event_source,
                stage=self.event_stage,
                metadata=merge_trace_metadata(
                    retrieve_meta,
        ...
        # Keep the activity payload bounded: the prompt context is already
        # clipped above, and the full RAG metadata can duplicate the entire
        # retrieved answer inside every persisted event.
        display_metadata = {
            key: metadata[key]
            for key in ("provider", "engine", "mode", "graph")
            if metadata.get(key) is not None
        }
        display_metadata["kb_name"] = kb_name
        await stream.tool_result(
            tool_name="rag",
            result=text,
            source=self.event_source,
            ...
            metadata=merge_trace_metadata(
                retrieve_meta,
                {
                    "trace_kind": "tool_result",
                    "sources": sources,
                    "tool_metadata": display_metadata,
                },
            ),
        )
        return text, sources
    ...
    def _estimate_messages_tokens(messages: list[dict[str, Any]]) -> int:
        from deepmentor.services.session.context_builder import count_tokens
    ...
    def _current_owner_id() -> str:
        ...
        """
        try:
            from deepmentor.multi_user.paths import current_owner_id
    ...
    def _rag_kbs(self, context: UnifiedContext) -> list[str]:
        ...
        from deepmentor.services.rag.pipelines.pageindex import is_pageindex_kb
    ...
    def _collect_kb_manifests(kbs: list[str]) -> list[KbManifest]:
        from deepmentor.multi_user.knowledge_access import resolve_kb_manifest
    ...
    def _workspace_system_note(self, context: UnifiedContext) -> str:
        ...
        if self.language == "zh":
            how_to_write = (
                (
                    ...
                    "DeepMentor 自己的运行环境。通过 exec 使用 PowerShell here-string 将脚本写入文件再运行，例如：\n"
        ...
        how_to_write = (
            (
                ...
                "Both `python` and `python -m pip` resolve to DeepMentor's runtime. "

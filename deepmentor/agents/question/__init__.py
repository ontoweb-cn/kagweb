...
The main entry point is :class:`~deepmentor.agents.question.pipeline.QuestionPipeline`.
...
def __getattr__(name: str) -> Any:
    if name == "AgentCoordinator":
        module = import_module("deepmentor.agents.question.coordinator")
        return getattr(module, name)
    if name == "FollowupAgent":
        module = import_module("deepmentor.agents.question.agents.followup_agent")
        return getattr(module, name)
    if name in {"QuestionPipeline", "QuizTemplate", "QuizPair", "QuizPlan", "QuizHistoryEntry"}:
        module = import_module("deepmentor.agents.question.pipeline")
        return getattr(module, name)

...
:func:`deepmentor.agents.question.mimic_source.parse_exam_paper_to_templates`
...
from deepmentor.services.keypool import KeyPool
...
def _download(zip_url: str) -> bytes:
    """Download a signed MinerU archive, retrying transient TLS/network drops.

    The archive URL is a short-lived CDN link. A repeated request can recover
    from a connection being cut during TLS negotiation, but retrying a client
    error such as an expired/invalid signature cannot help and only delays the
    ...
    for attempt in range(1, _DOWNLOAD_RETRY_ATTEMPTS + 1):
        try:
            ...
            # Signed URLs are not fixed by retrying on 4xx. Server-side 5xx
            # responses are transient enough to merit another connection.
        ...
        if attempt < _DOWNLOAD_RETRY_ATTEMPTS:
            delay = _DOWNLOAD_RETRY_BASE_SECONDS * (2 ** (attempt - 1))
            logger.warning(
                "MinerU result archive download failed (attempt %s/%s); retrying in %ss",
                attempt,
    ...
    assert last_error is not None  # every non-returning path records an HTTP error
    raise MinerUError(
        "Failed to download MinerU result archive after "
        f"{_DOWNLOAD_RETRY_ATTEMPTS} attempts: {last_error}"
    ) from last_error

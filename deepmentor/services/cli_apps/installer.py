...
entry's provenance (see :class:`~deepmentor.services.cli_apps.models.AppTrust`).

What this module does *not* do:

* **Run the registry's install command.** It runs an argv assembled from an
  :class:`~deepmentor.services.cli_apps.models.InstallPlan`. See ``models``.
...
from deepmentor.services.cli_apps.state import (
    InstalledApp,
    forget_install,
    load_installed,
    now_stamp,
...
INSTALL_TIMEOUT_S = int(os.environ.get("DEEPMENTOR_CLI_APP_INSTALL_TIMEOUT_S", "900"))
...
def _pin_of(entry: CliAppEntry) -> str:
    ...
    if entry.install.kind is InstallKind.PINNED_HARNESS:
        from deepmentor.services.cli_apps.catalog import catalog_pin

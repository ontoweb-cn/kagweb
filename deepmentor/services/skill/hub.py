...
A hub skill is the same artefact DeepMentor already speaks natively — a
...
# DeepMentor ships pointing at EduHub, its native open skill registry: a bare
# ``install <slug>`` with no ``<hub>:`` prefix resolves here.
DEFAULT_HUB = "eduhub"
_CLAWHUB_BASE_URL = "https://clawhub.ai/api/v1"
_EDUHUB_BASE_URL = "https://eduhub.deepmentor.info/api/v1"
...
class ClawHubProvider:
    ...
    def fetch(self, slug: str, *, version: str | None = None) -> FetchedSkill:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="deepmentor-skill-"))
        try:
    ...
    def catalog(
        ...
        DeepMentor-native browser renders without a second round-trip.
        """
        if query.strip():

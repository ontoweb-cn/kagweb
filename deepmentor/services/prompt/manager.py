...
from deepmentor.runtime.home import PACKAGE_ROOT
...
class PromptManager:
    ...
    # Modules that are not under deepmentor/agents/ directory
    # Map module_name → on-disk path component under deepmentor/
    ...
    def _candidate_prompt_dirs(self, module_name: str) -> list[Path]:
        ...
        if module_name in self.NON_AGENT_MODULES:
            ...
            current_dir = PACKAGE_ROOT / "deepmentor" / module_name / "prompts"
            return [legacy_dir, current_dir]

        legacy_dir = PACKAGE_ROOT / "src" / "agents" / module_name / "prompts"
        current_dir = PACKAGE_ROOT / "deepmentor" / "agents" / module_name / "prompts"
        return [legacy_dir, current_dir]

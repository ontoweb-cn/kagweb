"""
Authentication service for DeepMentor.
...
Quick setup (single user via data/user/settings/auth.json):
    ...
    3. Generate a password hash:
           python -c "from deepmentor.services.auth import hash_password; print(hash_password('yourpassword'))"
...
from deepmentor.multi_user.models import AccountPreset, Role
from deepmentor.services.config import load_auth_settings, load_integrations_settings
...
if AUTH_ENABLED and not POCKETBASE_ENABLED and not AUTH_SECRET:
    from deepmentor.multi_user.identity import load_or_create_auth_secret
...
def _make_user_record(
    ...
    from deepmentor.multi_user.identity import new_user_id
...
def delete_user(username: str) -> bool:
    ...
    """
    from deepmentor.multi_user.identity import delete_user as _delete_user
...
def set_role(username: str, role: str) -> bool:
    ...
    from deepmentor.multi_user.identity import set_role as _set_role
...
def set_avatar(username: str, avatar: str) -> bool:
    ...
    """
    from deepmentor.multi_user.identity import set_avatar as _set_avatar
...
def get_learner_profile(username: str) -> dict[str, Any] | None:
    ...
    from deepmentor.multi_user.identity import get_learner_profile as _get_profile
...
def set_learner_profile(username: str, profile: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    from deepmentor.multi_user.identity import set_learner_profile as _set_profile
...
def decode_token(token: str) -> TokenPayload | None:
    ...
    if POCKETBASE_ENABLED:
        from deepmentor.services.pocketbase_client import validate_pb_token
    ...
    try:
        ...
        if device_credential_id:
            from deepmentor.multi_user.device_credentials import validate_device_token
...
def authenticate_pb(username: str, password: str) -> tuple[TokenPayload, str] | None:
    ...
    <username>@deepmentor.local to match the email used at registration.
    """
    try:
...
def authenticate_device(pairing_code: str, pin: str) -> TokenPayload | None:
    ...
    if not AUTH_ENABLED or POCKETBASE_ENABLED:
        return None
    from deepmentor.multi_user.device_credentials import begin_device_session

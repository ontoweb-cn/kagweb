"""OpenAI Codex Responses provider backed by DeepMentor's private OAuth service."""
...
from deepmentor.services.codex_auth import CodexAuthError, get_codex_oauth_service
from deepmentor.services.codex_auth.constants import CODEX_DEFAULT_MODEL_ID, CODEX_RESPONSES_URL
from deepmentor.services.codex_auth.contracts import CodexToken
from deepmentor.services.llm.exceptions import LLMProviderTransportError
from deepmentor.services.llm.openai_http_client import disable_ssl_verify_enabled
from deepmentor.services.llm.provider_core.base import LLMProvider, LLMResponse, ToolCallRequest
...
def _build_headers(account_id: str, token: str) -> dict[str, str]:
    if not token:
        raise RuntimeError(
            "OpenAI Codex is not logged in. Run `deepmentor provider login openai-codex`."
        )
    headers = {
        "Authorization": f"Bearer {token}",
        "OpenAI-Beta": "responses=experimental",
        "originator": DEFAULT_ORIGINATOR,
        "User-Agent": "DeepMentor (python)",

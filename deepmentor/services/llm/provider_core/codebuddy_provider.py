...
from deepmentor.services.llm.provider_core.base import (
    LLMProvider,
    LLMResponse,
    ToolCallRequest,
)
...
class _CodeBuddySession:
    ...
    ``connect()`` and must exit them from the same task. DeepMentor chat turns
...
class CodeBuddyProvider(LLMProvider):
    ...
    endpoint. This adapter keeps one stateful CLI process per DeepMentor session
    and exposes DeepMentor's tool schemas through an in-process SDK MCP server.
    Tool execution remains owned by DeepMentor's existing dispatcher.
    """
    ...
    async def chat(
        ...
        **kwargs: Any,
        ...
        session_id = str(kwargs.pop("deepmentor_session_id", "") or "").strip()
...
def _build_options(
    ...
    raise RuntimeError(
        ...
        '(pip install "deepmentor[codebuddy]").'
    )
                ...
                "content": [
                    {
                        "type": "text",
                        "text": (
                            "DeepMentor accepted this tool call and will provide its result next."
                        ),
                    }
                ]
            }

        pending_result.__name__ = f"deepmentor_{name}"
...
def _reasoning_options(reasoning_effort: str | None) -> dict[str, Any]:
    """Map DeepMentor reasoning levels to CodeBuddy SDK thinking controls.
...
def _friendly_error(exc: Exception) -> str:
    ...
    if "Authentication required" in message:
        return (
            ...
            "`deepmentor provider login codebuddy`, run `codebuddy` and enter `/login`, "

...
(:class:`deepmentor.services.sandbox.backends.RunnerSidecarBackend`).
...
Wire contract (must match ``RunnerSidecarBackend``):
  ...
  ``POST /exec``   -> request/response JSON described by the dataclasses in
                      :mod:`deepmentor.services.sandbox.spec`. Request::
...
Mounts note:
  ...
  (``DEEPMENTOR_RUNNER_ALLOWED_WORKDIRS``) as defence in depth.
"""
...
_ALLOWED_WORKDIR_ROOTS = [
    ...
    for root in os.environ.get(
        "DEEPMENTOR_RUNNER_ALLOWED_WORKDIRS",

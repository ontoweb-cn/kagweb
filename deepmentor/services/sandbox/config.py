...
* ``DEEPMENTOR_SANDBOX_RUNNER_URL`` set ⇒ runner sidecar (Docker deployment);
* else on Linux with a functional ``bwrap`` ⇒ bwrap (bare-metal);
* else, only when ``DEEPMENTOR_SANDBOX_ALLOW_SUBPROCESS=1`` ⇒ restricted
...
:mod:`deepmentor.tools.exec_tool`). Per-user quotas live in
:mod:`deepmentor.services.sandbox.quota`.
"""
...
from deepmentor.services.sandbox.spec import ResourceLimits

RUNNER_URL_ENV = "DEEPMENTOR_SANDBOX_RUNNER_URL"
ALLOW_SUBPROCESS_ENV = "DEEPMENTOR_SANDBOX_ALLOW_SUBPROCESS"

# Per-user execution quotas (see quota.py). Conservative defaults; override
# via the matching env vars.
MAX_CONCURRENT_ENV = "DEEPMENTOR_SANDBOX_MAX_CONCURRENT"
MAX_PER_MINUTE_ENV = "DEEPMENTOR_SANDBOX_MAX_PER_MINUTE"
...
def build_backend(settings: SandboxSettings) -> SandboxBackend | None:
    ...
    backend's ``health()`` — see :mod:`deepmentor.services.sandbox.service`.
    """
    import sys

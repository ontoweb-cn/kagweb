...
class RAGService:
    ...
    DeepMentor's authoritative KB config, with metadata as a legacy fallback.
    """
    ...
    def _resolve_provider(self, kb_name: Optional[str]) -> str:
        """Pick the provider for ``kb_name`` from DeepMentor's binding."""
    ...
    def _capture_raw_logs(self, event_sink):
        ...
        from deepmentor.logging import ProcessLogEvent, capture_process_logs
        from deepmentor.logging.formatters import ContextFilter
        ...
        def should_skip_noisy_retrieve_log(event: ProcessLogEvent) -> bool:
            ...
            if logger_name == "nano-vectordb" and (
                ...
                return True
            if (
                logger_name.startswith("deepmentor.services.embedding.")
                and (

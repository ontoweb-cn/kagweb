"""Bridge DeepMentor runtime configuration into the LightRAG 1.5 native SDK.

This module is the decoupling seam: it exposes availability + mode helpers and
builds the three adapters LightRAG needs from DeepMentor's already-resolved LLM,
...
Decoupling notes:
* ``llm_model_func`` / ``vision_model_func`` wrap DeepMentor's unified model
  callables and DROP LightRAG's internal kwargs (``hashing_kv``,
  ``keyword_extraction``, …) so they never leak into ``factory.complete``.
* ``embedding_func`` reuses DeepMentor's embedding client, wrapped in LightRAG's
...
class LightRagNotConfiguredError(RuntimeError):
    """Raised when DeepMentor's LLM / embedding config can't back LightRAG."""
...
def _retry_classification(exc: Exception) -> tuple[bool, str]:
    ...
    from deepmentor.services.llm.request_compat import is_transient_transport_error
...
def _retry_delay_seconds(exc: Exception, scheduled_delay: float) -> float:
    ...
    from deepmentor.services.llm.error_mapping import retry_after_seconds
...
def is_lightrag_available() -> bool:
    ...
    Opt-in extra: ``pip install 'deepmentor[rag-lightrag]'``. Until installed the
...
def query_kwargs_from_settings() -> dict:
    ...
    try:
        from deepmentor.services.config import load_lightrag_settings
...
def _resolve_override_llm_config(selection: dict[str, str] | None):
    if selection is None:
        return None
    try:
        from deepmentor.services.model_selection.runtime import (
...
def build_llm_model_func(
    *,
    ...
    """Wrap DeepMentor's unified LLM callable for LightRAG.
...
def build_vision_model_func(
    *,
    ...
    """Map rc2 ``image_inputs`` to DeepMentor's vision callable."""
    override = _resolve_override_llm_config(llm_selection)
    if override is None:
        from deepmentor.services.llm import get_llm_client
...
def vision_model_available() -> bool:
    """Return whether the active DeepMentor model is explicitly vision-capable."""
    try:
        from deepmentor.services.llm import get_llm_client
...
def build_embedding_func(*, io_bridge: OwnerLoopBridge | None = None):
    """Wrap DeepMentor's embedding client in LightRAG's ``EmbeddingFunc``."""
    from lightrag.utils import EmbeddingFunc

    from deepmentor.services.embedding import get_embedding_client, get_embedding_config

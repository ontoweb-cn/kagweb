...
PARSER_ENGINE = "deepmentor"


class LightRagContractError(RuntimeError):
    """Raised when the pinned SDK returns a shape DeepMentor cannot trust."""
...
def installed_version() -> str:
    try:
        ...
        raise LightRagContractError(
            "LightRAG is not installed. Install `deepmentor[rag-lightrag]`."
        ) from exc
...
def _require_exact_version() -> None:
    ...
    if current != LIGHTRAG_VERSION:
        raise LightRagContractError(
            f"DeepMentor requires {LIGHTRAG_DISTRIBUTION}=={LIGHTRAG_VERSION}; found {current}"
        )
...
def _controlled_class():
    ...
    class DeepMentorLightRAG(LightRAG):
        def _resolve_source_file_for_parser(
            ...
            *,
            ...
            if parser_engine not in (None, PARSER_ENGINE):
                raise IngressError(
                    f"Unsupported parser engine for DeepMentor ingress: {parser_engine}"
                )
...
def _register_parser() -> None:
    ...
    register_parser(
        ParserSpec(
            ...
            impl="deepmentor.services.rag.pipelines.lightrag.parser:DeepMentorParser",
            suffixes=_SUFFIXES,
            queue_group="native",
        )
    )
...
def workspace_for(working_dir: Path) -> str:
    ...
    return f"deepmentor_{hashlib.sha256(identity).hexdigest()[:16]}"

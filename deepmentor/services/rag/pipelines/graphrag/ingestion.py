...
(its optional ``markitdown`` reader is just a converter). So DeepMentor owns the
"document → text" step and hands GraphRAG ready ``.txt`` files. We deliberately
reuse DeepMentor's existing extraction primitives here so multimodal/parsing
stays a DeepMentor-side concern (matching how the LlamaIndex pipeline already
...
def _extract_parser_text(path: Path, parse_service=None) -> str:  # noqa: ANN001
    from deepmentor.services.parsing import ParserError, get_parse_service

"""Bridge DeepMentor's runtime config into a GraphRAG ``settings.yaml``.
...
DeepMentor's already-resolved LLM + embedding runtime config and let
...
* Completion calls use DeepMentor's registered GraphRAG compatibility adapter,
...
from deepmentor.services.config.embedding_endpoint import canonical_embedding_provider_name
from deepmentor.services.embedding.request_options import should_send_embedding_dimensions
...
# appends the ``/embeddings`` operation path to ``api_base``. DeepMentor stores a
...
class GraphRagNotConfiguredError(RuntimeError):
    """Raised when DeepMentor's LLM / embedding config can't back GraphRAG."""
...
def is_graphrag_available() -> bool:
    ...
    extra: ``pip install 'deepmentor[graphrag]'``. Until it is installed the
...
def graphrag_embedding_api_base(binding: str | None, endpoint: str | None) -> str:
    """Translate a DeepMentor embedding endpoint into GraphRAG ``api_base``.

    DeepMentor's public embedding contract stores and calls the complete
    ...
    Args:
        binding: Active DeepMentor embedding binding.
...
def ensure_graphrag_embedding_transport(
    ...
    # Gemini can use either DeepMentor's native ``batchEmbedContents`` adapter
...
def query_config_from_settings() -> GraphRagQueryConfig:
    ...
    try:
        from deepmentor.services.config import load_graphrag_settings
...
def build_settings(*, llm_cfg: Any = None, embedding_cfg: Any = None) -> dict[str, Any]:
    """Assemble the GraphRAG ``settings.yaml`` payload from DeepMentor config.

    ``llm_cfg`` / ``embedding_cfg`` are injectable for tests; in production they
    are resolved from DeepMentor's catalog. Raises
    :class:`GraphRagNotConfiguredError` if either side has no usable model.
    """
    if llm_cfg is None:
        from deepmentor.services.config import resolve_llm_runtime_config

        llm_cfg = resolve_llm_runtime_config()
    if embedding_cfg is None:
        from deepmentor.services.embedding import get_embedding_config
    ...
    return {
        # GraphRAG defaults to 25 concurrent requests, which can overwhelm
        # rate-limited model providers during graph extraction and entity
        # summarization. Cap it well below the default to avoid 429 storms.
        ...
        # Plain-text input: DeepMentor's ingestion writes parsed ``.txt`` files
        ...
        # GraphRAG/LanceDB defaults to 3072 dimensions; DeepMentor must stamp the

"""Configuration helpers for DeepMentor's LlamaIndex RAG pipeline."""
...
def should_show_progress() -> bool:
    ...
    DeepMentor runs as a server that stream is a pipe whose read end (the
    launcher's relay thread) can close mid-indexing, and the next tqdm write
    then raises :class:`BrokenPipeError`, killing document indexing. DeepMentor
...
class RetrievalConfig:
    ...
    reranker_model: str = ""
    # Optional reranker service base URL (e.g. ``http://localhost:8000``).
    # Empty -> local SentenceTransformers cross-encoder; set -> vLLM /v1/rerank.
...
def normalize_reranker_endpoint(value: str | None) -> str:
    """Return a bounded reranker service base URL (empty uses local loading)."""
    return (value or "").strip().rstrip("/")[:200]
...
def retrieval_config_from_env() -> RetrievalConfig:
    ...
    return RetrievalConfig(
        profile=normalize_retrieval_profile(
            os.getenv("DEEPMENTOR_RAG_RETRIEVAL_PROFILE") or os.getenv("RAG_RETRIEVAL_PROFILE")
        )
    )
...
def _load_runtime_settings() -> dict:
    ...
    from deepmentor.services.config import load_llamaindex_settings
...
def retrieval_config_from_settings() -> RetrievalConfig:
    ...
    return RetrievalConfig(
        ...
        reranker_endpoint=normalize_reranker_endpoint(settings.get("reranker_endpoint")),

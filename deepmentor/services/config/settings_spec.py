"""Machine-readable description of what a DeepMentor install can be told to be.
...
(:mod:`deepmentor.capabilities.setup.tools`) completely generic: they walk this
...
``scope``
    ...
    row is one that :class:`~deepmentor.services.path_service.PathService`
    already resolves per user, a global row is one read through
    :func:`~deepmentor.services.config.loader.get_runtime_settings_dir`.
...
``probe``
    ...
    :func:`~deepmentor.capabilities.setup.apply.apply_setting` refuses to commit
...
Imports of the rest of DeepMentor are deferred into the row callables on
purpose: this module is imported from ``deepmentor.capabilities``, and a
...
def _write_ui(key: str) -> Callable[[str], None]:
    def _write(value: str) -> None:
        from deepmentor.services.settings.interface_settings import set_ui_setting
...
def _interface_specs() -> list[SettingSpec]:
    ...
    return [
        SettingSpec(
            ...
            summary="Language of the DeepMentor UI (menus, buttons, settings).",
            read=language_read,
...
def _parsing_engine_choices() -> tuple[SettingChoice, ...]:
    ...
    """
    from deepmentor.services.parsing.engines._install import installable_engines
    from deepmentor.services.parsing.engines.factory import list_engines
...
def _validate_parsing_engine(value: str) -> str | None:
    from deepmentor.services.parsing.engines.factory import is_engine_available
...
def _catalog_profile_choices(service_name: str) -> Callable[[], tuple[SettingChoice, ...]]:
    def _choices() -> tuple[SettingChoice, ...]:
        from deepmentor.services.config.provider_runtime import SEARCH_PROVIDERS
...
def _clear_runtime_caches() -> None:
    ...
    """
    try:
        from deepmentor.services.llm import clear_llm_config_cache
...
async def _probe_llm(value: str) -> ProbeResult:
    ...
    from deepmentor.services.llm import get_token_limit_kwargs
...
def setting_specs() -> dict[str, SettingSpec]:
    ...
    ``deepmentor.capabilities`` without closing an import cycle.
    """

...
from deepmentor.services.file_io import atomic_write_json as _atomic_write_json
...
DEFAULT_SYSTEM_SETTINGS: dict[str, Any] = {
    ...
    # audited deployments; DEEPMENTOR_VERSION_CHECK_ENABLED is the deployment
...
# Document parsing settings. The parse layer (deepmentor/services/parsing)
...
_DEFAULT_MINERU_ENGINE: dict[str, Any] = {
    ...
    # separate conda) so its heavy deps never conflict with DeepMentor's.
    "local_cli_path": "",
...
# DeepMentor's VLM to describe images.
...
# DeepMentor's legacy text extractors for PDF / Office / text-like files.
...
# ``max_concurrent_files`` sizes the native parser worker pool after DeepMentor
...
IGNORE_PROCESS_OVERRIDES_ENV = "DEEPMENTOR_IGNORE_PROCESS_ENV_OVERRIDES"
...
class RuntimeSettingsService:
    ...
    def render_environment(self) -> dict[str, str]:
        ...
        return {
            "DEEPMENTOR_VERSION_CHECK_ENABLED": _bool_env(system["version_check_enabled"]),
            ...
            "DEEPMENTOR_SANDBOX_ALLOW_SUBPROCESS": _bool_env(system["sandbox_allow_subprocess"]),
            ...
            # rewrites /api/* and /ws/* to DEEPMENTOR_API_BASE_URL and uses
            # DEEPMENTOR_AUTH_ENABLED to gate the login redirect. The launcher and
            # the Docker entrypoint both export these through render_environment,
            # so the two deployment paths stay in sync. DEEPMENTOR_API_BASE_URL is
            ...
            "DEEPMENTOR_AUTH_ENABLED": _bool_env(auth["enabled"]),
    ...
    def _apply_system_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if value := self._process_env_value("DEEPMENTOR_VERSION_CHECK_ENABLED"):
        ...
        if value := (
            self._process_env_value("DEEPMENTOR_BACKEND_WORKERS")
            or self._process_env_value("BACKEND_WORKERS")
        ):
            payload["backend_workers"] = value
        if value := self._process_env_value("DEEPMENTOR_SANDBOX_ALLOW_SUBPROCESS"):
            payload["sandbox_allow_subprocess"] = value
        if value := self._process_env_value("DEEPMENTOR_CAPABILITY_ROUTING_ENABLED"):
    ...
    def _apply_integrations_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if value := self._process_env_value("DEEPMENTOR_TURN_COORDINATION_BACKEND"):
            coordination["backend"] = value
        if value := self._process_env_value("DEEPMENTOR_REDIS_URL"):
            coordination["redis_url"] = value
        if value := self._process_env_value("DEEPMENTOR_REDIS_KEY_PREFIX"):
    ...
    def _apply_llamaindex_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        # (DEEPMENTOR_RAG_RETRIEVAL_PROFILE / RAG_RETRIEVAL_PROFILE); preserve it.
        payload = dict(settings)
        if value := (
            self._process_env_value("DEEPMENTOR_RAG_RETRIEVAL_PROFILE")
    ...
    def _normalize_integrations(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if backend not in {"memory", "redis"}:
            backend = "memory"
        key_prefix = _string(coordination.get("key_prefix")).strip(":") or "deepmentor"
        return {
            "version": 2,

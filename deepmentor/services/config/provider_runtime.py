"""Nanobot-style normalized runtime configuration for DeepMentor."""
...
from deepmentor.services.imagegen.config import ImagegenConfig
from deepmentor.services.keypool import primary_api_key
from deepmentor.services.model_selection import LLMSelection, apply_llm_selection_to_catalog
...
class SearchProviderSpec:
    ...
    its own answer or needs :mod:`deepmentor.services.search.consolidation` to
    ...
    The provider registry (``deepmentor/services/search/providers``), the
    Settings dropdown (``deepmentor/api/routers/settings.py``) and the web
...
def _with_personal_llm_profiles(catalog: dict[str, Any]) -> dict[str, Any]:
    ...
    the shared one (see :mod:`deepmentor.multi_user.personal_models`), so
    ...
    """
    try:
        from deepmentor.multi_user.personal_models import merge_personal_llm_profiles
...
def _register_catalog_capabilities(catalog: dict[str, Any]) -> None:
    ...
    without a restart. Imported lazily: ``deepmentor.services.llm`` imports this
    module for its config, so a module-level import would close a cycle.
    """
    from deepmentor.services.llm.capabilities import set_catalog_capability_overrides
...
def resolve_llm_runtime_config(
    ...
    *,
    ...
    is the same shape and stands in for it on the calls DeepMentor makes on its

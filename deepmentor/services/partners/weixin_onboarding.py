...
This drives the same exchange (:mod:`deepmentor.partners.channels.weixin_qr`) on
...
def _client_version() -> int:
    from deepmentor.partners.channels.weixin import ILINK_APP_CLIENT_VERSION

    return ILINK_APP_CLIENT_VERSION


def _channel_config(partner_id: str) -> dict[str, Any]:
    from deepmentor.services.partners.manager import get_partner_manager
...
async def _apply_outcome(attempt: _Attempt, outcome: QrOutcome) -> None:
    ...
    if outcome.status == "confirmed":
        try:
            ...
            attempt.status = "error"
            attempt.error = (
                "WeChat confirmed the login, but DeepMentor could not save or start "

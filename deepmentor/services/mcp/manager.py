...
DeepMentor's chat runs as per-turn tasks inside one event loop, while MCP
...
from deepmentor.services.mcp.config import (
    MCPConfig,
    MCPServerConfig,
    load_mcp_config,
)
...
class MCPConnectionManager:
    ...
    async def ensure_scope(self, owner: str) -> list[MCPToolAdapter]:
        ...
        from deepmentor.services.mcp.user_config import load_user_mcp_config
    ...
    def _signature(cls, cfg: MCPServerConfig, owner: str) -> str:
        ...
        :mod:`deepmentor.services.mcp.secrets`). Rotating a credential therefore
    ...
    def _materialize(cfg: MCPServerConfig, owner: str) -> MCPServerConfig:
        ...
        one transport open. See :mod:`deepmentor.services.mcp.secrets` for why the
        stored config holds references instead.
        """
        from deepmentor.services.mcp.secrets import resolve_references, resolve_url_references
    ...
    async def _open_transport(
        ...
        *,
        ...
        if self_service:
            ...
            from deepmentor.services.mcp.network import validate_mcp_url_async
        ...
        oauth_auth = None
        if cfg.auth == "oauth":
            from deepmentor.services.mcp.oauth import build_auth, oauth_redirect_uri
...
def _needs_authorization(exc: BaseException) -> bool:
    ...
    """
    from deepmentor.services.mcp.oauth import AuthorizationRequired

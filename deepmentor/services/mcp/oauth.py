...
sibling module :mod:`deepmentor.services.mcp.secrets` holds static credentials in
...
#: How DeepMentor introduces itself on a consent screen.
CLIENT_NAME = "DeepMentor"
CLIENT_URI = "https://deepmentor.info"
...
def _store_path(owner_id: str, server: str) -> Path:
    from deepmentor.multi_user.paths import owner_secrets_dir
...
_PUBLIC_URL_ENV = "DEEPMENTOR_PUBLIC_URL"
...
def oauth_redirect_uri(origin: str = "") -> str:
    ...
    *origin* is the origin the person is actually browsing, which the route that
    ...
    deployment work with no configuration. ``DEEPMENTOR_PUBLIC_URL`` overrides it
...
def client_metadata(redirect_uri: str) -> "Any":
    """How DeepMentor registers itself with an authorization server."""

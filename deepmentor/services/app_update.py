...
GITHUB_LATEST_RELEASE_URL = "https://api.github.com/repos/HKUDS/DeepMentor/releases/latest"
GITHUB_LATEST_RELEASE_WEB_URL = "https://github.com/HKUDS/DeepMentor/releases/latest"
VERSION_CHECK_TTL_SECONDS = 24 * 60 * 60
LAUNCHER_PID_ENV = "DEEPMENTOR_LAUNCHER_PID"
...
def _version_tuple(value: str) -> tuple[int, int, int]:
    ...
    if match is None:
        raise ValueError(f"Invalid DeepMentor version: {value}")
...
def _running_in_container() -> bool:
    configured = os.getenv("DEEPMENTOR_CONTAINER", "").strip().lower()
...
def _distribution_direct_url() -> dict[str, Any] | None:
    try:
        distribution = importlib.metadata.distribution("deepmentor")
...
def detect_installation() -> Installation:
    ...
    if _running_in_container():
        return Installation(
            ...
            command="docker pull ghcr.io/hkuds/deepmentor:latest",
    ...
    if direct_url is None:
        return Installation(
            ...
            reason="The running DeepMentor distribution could not be identified.",
        )
            ...
            try:
                response = await client.get(
                    ...
                    headers={
                        ...
                        "User-Agent": "DeepMentor-Version-Check",
                    },
                )
    ...
    async def _fetch_latest_redirect(self, client: httpx.AsyncClient) -> ReleaseInfo:
        ...
        try:
            response = await client.head(
                self._latest_url,
                headers={"User-Agent": "DeepMentor-Version-Check"},
...
def _release_from_payload(payload: Any) -> ReleaseInfo:
    ...
    if not url.startswith("https://github.com/HKUDS/DeepMentor/releases/"):
...
def _release_from_latest_url(value: str) -> ReleaseInfo:
    ...
    prefix = "/HKUDS/DeepMentor/releases/tag/"
    if (
        parsed.scheme != "https"
...
class UpdateJobStore:
    ...
    def create(self, *, current_version: str, target_version: str) -> UpdateJob:
        ...
        if _version_tuple(target) <= _version_tuple(current):
            raise UpdateRequestError("No newer DeepMentor release is available")
...
def launch_update_worker(store_root: Path, *, parent_pid: int) -> None:
    ...
    command = [
        sys.executable,
        "-m",
        "deepmentor.runtime.update_worker",
        "--store-root",

...
*partner session key*. The first consult of a DeepMentor chat session has none,
so we mint a fresh ``dt-…`` key and return it; the cross-turn registry
(:mod:`deepmentor.services.subagent.sessions`) remembers it against
(chat session, connection), so every later consult in the same DeepMentor chat —
within one turn or across turns — resumes the SAME partner session. The partner
page then sees one complete history session per DeepMentor chat, titled from the
...
from deepmentor.services.subagent.base import OnEvent, SubagentBackend
from deepmentor.services.subagent.config import BackendConfig
from deepmentor.services.subagent.types import (
    EVENT_ERROR,
    EVENT_REASONING,
    EVENT_TEXT,
    EVENT_TOOL,
...
class PartnerBackend(SubagentBackend):
    ...
    async def consult(
        ...
        *,
        ...
        from deepmentor.multi_user.partner_access import assert_partner_allowed
        ...
        # a DeepMentor chat → mint a stable, colon-free key; the registry threads

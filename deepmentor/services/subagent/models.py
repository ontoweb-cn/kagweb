...
reasoning effort DeepMentor drives each backend with. Those lists change over
...
from deepmentor.services.subagent.process import (
    not_found_detail,
    probe_version,
    resolve_cli_command,
)
from deepmentor.services.subagent.registry import get_backend
...
async def _claude_options() -> BackendOptions:
    from deepmentor.services.subagent.claude_models import load_cached_claude_models
...
async def _antigravity_options() -> BackendOptions:
    from deepmentor.services.subagent.antigravity import (
...
async def _free_text_options(
    ...
    if kind == "deepseek_harness" and not ok:
        from deepmentor.services.subagent.deepseek_harness import _sdk_available
...
async def sync_backend_options(kind: str) -> BackendOptions:
    ...
    """
    if kind == "claude_code":
        from deepmentor.services.subagent.claude_models import sync_claude_models

...
function describeToolCall(
  ...
  switch (toolName) {
    ...
    case 'load_tools': {
      const names = Array.isArray(a.names) ? (a.names as unknown[]).map(n => String(n)) : []
      return {
...
function getTraceHeader(
  ...
  if (
    ...
    title = Number.isFinite(idx) ? t('Question {{n}}', { n: idx + 1 }) : t('Question')
 ...
 */
function renderNiceToolArgs(toolName: string | undefined, rawArgs: unknown): ReactNode | null {
 ...
 */
const ENGINE_NAMED_TOOLS = new Set(['web_search', 'rag_search', 'rag', 'paper_search'])
...
function TraceRowBody({
  ...
  const contentText = getTraceText(callEvents, ['content'], isNarrationRound(callEvents))
  const bodyBlocks =
    role === 'observe'
      ? [observationText]
      : role === 'retrieve'
          ...
          {renderableBodyBlocks.length > 0 && (
            <div className="mt-1 space-y-1.5">
              {renderableBodyBlocks.map((text, idx) => (
                <MarkdownRenderer key={`${callId}-body-${idx}`} content={text} variant="trace" />
              ))}
            </div>
          )}
        </div>
...
function hasExpandableContent(callEvents: StreamEvent[], group: string, role: string) {
  ...
  const genericBodyText =
    role === 'observe' ? observationText : role === 'retrieve' ? '' : thoughtText || contentText
  const inlineSources = callEvents.flatMap(event => getTraceMeta(event).sources ?? [])
...
function TraceRowItem({
  ...
  const toolArgs = toolCallEvent?.metadata?.args as Record<string, unknown> | undefined
  ...
  const descriptor =
    isToolRow && toolName ? describeToolCall(toolName, toolArgs, t, provider) : null
  ...
  if (descriptor) {
    headline =
      engine && ENGINE_NAMED_TOOLS.has(toolName)
        ...
        : descriptor.verb
    chip = descriptor.chip ? { text: descriptor.chip, mono: descriptor.mono } : null
    ...
    if (toolName === 'consult_subagent') {
      ...
      headline = agentName ? `${t('Consult Subagent')} ${agentName}` : t('Consult Subagent')
    }
  } else if (isRetrieve) {
    headline = engine ? `${providerLabel(engine)} ${header}` : header
    const query = clip(String(callEvents.map(e => getTraceMeta(e).query).find(Boolean) || ''))
  ...
  return (
    ...
    >
      {isThinking ? (
        deliberation.length ? (
          <div className={`space-y-1.5 leading-[1.6] ${isChatRound ? 'italic' : ''}`}>
            {deliberation.map((text, idx) => (
              <MarkdownRenderer key={`${callId}-say-${idx}`} content={text} variant="trace" />
            ))}
          </div>
        ) : null
      ) : (
  ...
  return (
    <ActivityStack className="mb-3">
      {displayItems.map((item, displayIdx) => {
        ...
        if (item.kind === 'step') {
          ...
          return (
            <div key={item.stepId} className={displayIdx > 0 ? 'mt-2' : ''}>
              <ActivityDivider label={t('Step {{n}}', { n: item.stepId })} active={isActiveStep} />
        ...
        const active =
          Boolean(isStreaming) && isLastDisplayItem && isTracePending(item.trace.events)
        return (
          <TraceRowItem
...
function getDeepResearchStatusLabel(
  ...
  for (let idx = events.length - 1; idx >= 0; idx -= 1) {
    ...
    if (key === 'research_topic' || event.stage === 'researching') {
      ...
      return topicIndex ? t('Researching Topic #{{n}}', { n: topicIndex }) : t('Researching Topic')
    }
...
export function StreamingStatus({
  ...
  else if (mode === 'responding') modeLabel = t('{{name}} Exploring…', { name })
  ...
  // the trace card.
  const turnSeconds = getTurnDurationSeconds(events, nowSeconds, Boolean(isStreaming))
  const durationLabel = turnSeconds != null ? formatTurnDuration(turnSeconds) : null
      ...
      {hasTrace ? (
        ...
        >
          <div className="overflow-hidden">
            ...
            <NestedTraceFlow events={shownTraceEvents} isStreaming={isStreaming} />
          </div>
        </div>
      ) : null}
    </div>
...
function getResearchStageId(event: StreamEvent): ResearchStageCard['id'] {
  ...
  const explicitStage = String(
    (event.metadata as Record<string, unknown> | undefined)?.research_stage_card || ''
  )
  if (

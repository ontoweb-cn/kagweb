...
/**
 * Deep-link a KB document into the knowledge-center preview page.
 *
 * Activity sidebar files should open the user-facing KB detail view, not the
 * raw `/api/...` stream that happens to be suitable for downloads.
 */
export function knowledgeBaseFileRoute(
  name?: string | null,
  filename?: string | null,
): string {
  const base = knowledgeBaseRoute(name);
  const file = filename?.trim();
  if (!base || base === "/knowledge-bases" || !file) return base;
  return `${base}?file=${encodeURIComponent(file)}`;
}

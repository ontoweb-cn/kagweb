...
export function ReaderPane({
  ...
  // Text view over a scanned PDF. The page reader shows the original image —
  // faithful, but a scan carries no PDF text layer, so nothing on it can be
  // selected and every selection-based feature is stranded. The extractor tag
  // records that the server OCR'd this file, which means selectable text exists
  // even though the raw pages have none.
  ...
  const toggleTextView = useCallback(() => {
    if (!material) return;
    setSelection(null);
    window.getSelection()?.removeAllRanges();
    // Hand the remounting view the page being left: the PDF reader lands on
    // its jump prop, the text view on `initialLocator` — both fed from here,
    // so switching costs the reader their zoom but never their place.
    requestJump(currentLocator);
    setTextView((current) => !current);
  }, [currentLocator, material, requestJump]);
  ...
  // "-ocr" is the extractor tag for a PDF whose text came from the configured
  // parser (MinerU/Docling): the page images carry no text layer, but the
  // server holds selectable text for every unit — exactly the reader the
  // toggle swaps in.
  const scanTextAvailable = Boolean(
    material?.has_raw_view && material.extractor.endsWith("-ocr"),
  );
          ...
          <>
            ...
            {scanTextAvailable && (
              <HeaderButton
                icon={Type}
                label={
                  textView ? t("Show page images") : t("Show extracted text")
                }
                active={textView}
        ...
        <div className="min-w-0 flex-1">
          {loadingMaterial ? (
            ...
            <EpubDocumentView
              ...
              onError={setError}
            />
          ) : textView || !material.has_raw_view ? (
            <TextUnitView
              key={material.material_id}
              materialId={material.material_id}
              unit={material.unit}
              ...
              // Only the scanned-PDF toggle mounts mid-document; text
              // materials start at the top as they always have.
              initialLocator={
                material.has_raw_view ? currentLocator : undefined
              }
              annotations={annotations}
              jump={materialJump}
              highlightedAnnotationId={activeAnnotationId}

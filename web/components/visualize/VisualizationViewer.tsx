...
function HtmlRenderer({ html }: { html: string }) {
  ...
  const handleOpenInNewTab = () => {
    try {
      ...
      const wrapper = `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${t("Visualization")}</title><style>html,body,iframe{height:100%;width:100%;margin:0;border:0;}</style></head><body><iframe sandbox="allow-scripts" src="${contentUrl}"></iframe></body></html>`;
...
function PluginIframeRenderer({ result }: { result: VisualizeCanvasResult }) {
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || typeof data !== "object") return;
      if (
        (data.type === "deepmentor:visualization:prompt" ||
      ...
      if (
        (data.type === "deepmentor:visualization:resize" ||
          data.type === "dt:visualize-height") &&
        typeof data.height === "number" &&
        Number.isFinite(data.height)
      ) {
    ...
    <iframe
      ...
      onLoad={() => {
        iframeRef.current?.contentWindow?.postMessage(
          {
            type: "deepmentor:visualization:render",

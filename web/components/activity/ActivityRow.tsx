...
export function ActivityRow({
  ...
  children?: ReactNode
  className?: string
  /** Links this row to sidebar provenance: `data-trace-call-id`. */
  traceCallId?: string
}) {
  ...
  return (
    <div data-trace-call-id={traceCallId} className={`group/row ${className}`}>
      <div
        ...
        className={`flex items-start gap-2.5 py-1 text-[14px] leading-[1.45] text-[var(--muted-foreground)] ${
          expandable ? 'cursor-pointer transition-colors hover:text-[var(--foreground)]' : ''
        }`}
      >
        ...
        <div className="min-w-0 flex-1">
          {detail ? (
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className={`shrink-0 font-medium ${breathing ? 'dt-breathing-text' : ''}`}>
                {title}
              </span>
              <span
            ...
            <span
              className={`font-medium ${
                clampTitle === 3 ? 'line-clamp-3' : clampTitle === 2 ? 'line-clamp-2' : 'block'

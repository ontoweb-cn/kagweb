...
attaching a per-turn ``cost_summary`` so the frontend can render
``$cost · tokens · calls`` in its message footer. This module centralizes
the merge + emit so every capability emits the same envelope shape.
"""

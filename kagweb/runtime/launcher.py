"""Local Web launcher for the installed KAGWeb app."""
...
from kagweb.runtime.home import (
    KAGWEB_HOME_ENV,
    PACKAGE_ROOT,
    get_runtime_home,
    validate_runtime_home,
)
from kagweb.runtime.memory_probe import SUPERVISOR_PID_ENV
from kagweb.runtime.process import is_process_alive
from kagweb.services.app_update import LAUNCHER_PID_ENV
...
SOURCE_PRODUCTION_DIST_DIR = ".next-kagweb"
SOURCE_BUILD_MARKER = ".kagweb-build.json"
...
DETACHED_WORKER_ENV = "KAGWEB_DETACHED_WORKER"
DETACHED_TOKEN_ENV = "KAGWEB_DETACHED_TOKEN"
...
def _reset_runtime_singletons() -> None:
    """Make a just-selected KAGWEB_HOME visible to path/config singletons."""
    try:
...
def _packaged_web_dir() -> Path | None:
    try:
        ...
        return None
    path = Path(kagweb_web.__file__).resolve().parent
...
def _copy_packaged_web_if_needed(
    ...
    *,
    ...
    marker = cache / ".kagweb-web-runtime.json"
...
def _ensure_web_dependencies(source: Path, npm: str) -> None:
    ...
    if result.returncode != 0:
        raise SystemExit(
            ...
            "Fix the error above, then retry `kagweb start`."
        )
...
def _source_production_env(*, api_base: str, auth_enabled: bool) -> dict[str, str]:
    ...
    env = os.environ.copy()
    env["KAGWEB_NEXT_DIST_DIR"] = SOURCE_PRODUCTION_DIST_DIR
...
def _source_build_fingerprint(source: Path, env: dict[str, str]) -> str:
    ...
    version_file = PACKAGE_ROOT / "kagweb" / "__version__.py"
...
def _ensure_source_production_build(
    ...
    *,
    ...
    Long-running ``kagweb start`` processes must not use ``next dev``: its
...
def _resolve_frontend(
    ...
    *,
    ...
    if packaged is not None:
        if not node:
            raise SystemExit("Node.js 20+ is required to run the packaged KAGWeb Web app.")
    ...
    raise SystemExit(
        "KAGWeb Web assets are not installed. Install the full app with `pip install -U kagweb`, "
...
def _launch_detached(
    ...
    *,
    ...
    env = os.environ.copy()
    env[KAGWEB_HOME_ENV] = str(runtime_home.resolve())
...
def stop(home: str | Path | None = None, *, timeout: float = 15) -> bool:
    """Request a graceful stop from a detached KAGWeb launcher."""
...
def start(
    ...
    *,
    ...
    common_env[KAGWEB_HOME_ENV] = str(runtime_home)
    ...
    common_env["KAGWEB_API_BASE_URL"] = backend_url
    common_env["KAGWEB_AUTH_ENABLED"] = "true" if auth_enabled else "false"
    common_env["PYTHONUNBUFFERED"] = "1"
    common_env["PYTHONIOENCODING"] = "utf-8:replace"
    # Anchor for kagweb.runtime.memory_probe: the backend and the frontend
    # are siblings under this process, so only the supervisor's pid identifies
    # the tree that is "KAGWeb". Without it the probe can only measure the
    # backend itself.
    ...
    # ``KAGWEB_NEXT_DIST_DIR`` is a build-only override.  In particular it
    ...
    # clean the live ``.next-kagweb`` tree out from under the standalone
    ...
    common_env.pop("KAGWEB_NEXT_DIST_DIR", None)
    ...
    backend_cmd = [
        ...
        # middleware (kagweb/api/main.py) surfaces only non-200s, so routine

"""Detached executor for a launcher-managed KAGWeb update."""
...
def build_restart_command(job: UpdateJob) -> tuple[list[str], Path]:
    ...
    return [sys.executable, "-m", "kagweb_cli.main", *job.restart_argv], home
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Apply a managed KAGWeb update")

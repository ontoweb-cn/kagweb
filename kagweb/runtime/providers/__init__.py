...
An *external provider* is something that contributes tools KAGWeb did not
...
``load_tools`` (see :mod:`kagweb.runtime.registry.deferred_tools`). What
they do *not* share is authorisation, so that stays per-kind and explicit in
:mod:`~kagweb.runtime.providers.authorize`.
...
upwards. Import it directly (``from kagweb.runtime.providers.view import
...
from kagweb.runtime.providers.text import (

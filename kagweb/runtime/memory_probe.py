"""Resident-memory snapshot of the running KAGWeb process tree.

The counterpart to :mod:`kagweb.runtime.memory_reclaim`: that module *acts*
...
Two things make "how much memory does KAGWeb use" more than one ``getrusage``
call:

* The backend is only one of the processes. ``kagweb.runtime.launcher``
  ...
  ``uvicorn kagweb.api.main:app`` run would climb into the developer's shell
...
# find the root of the KAGWeb process tree it belongs to.
SUPERVISOR_PID_ENV = "KAGWEB_SUPERVISOR_PID"
...
class ProcessMemory:
    """One process in the KAGWeb tree."""
...
def _classify(pid: int, name: str, cmdline: str, root_pid: int | None = None) -> str:
    ...
    the supervisor runs as a bare ``kagweb`` console script with nothing in
    ...
    if "next" in lowered or "node" in lowered:
        return "web"
    if "uvicorn" in lowered or "kagweb.api" in lowered:
        return "backend"
...
def _scan_proc() -> tuple[list[ProcessMemory], bool]:
    """psutil-free fallback for Linux — the platform KAGWeb ships in Docker on."""

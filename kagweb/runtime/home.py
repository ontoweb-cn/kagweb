"""Runtime home resolution for installed and source KAGWeb runs."""
...
def validate_runtime_home(home: str | Path) -> None:
    ...
    if resolved in invalid_homes:
        raise ValueError(
            f"Invalid KAGWeb runtime home: {resolved}. Runtime data must live under "
            f"{PACKAGE_ROOT / 'data'} without nesting. Start KAGWeb with "
            f"KAGWEB_HOME={PACKAGE_ROOT} from {PACKAGE_ROOT}."
        )
...
def get_runtime_home(home: str | Path | None = None) -> Path:
    ...
    2. ``KAGWEB_HOME`` environment variable.
    ...
    raw = home if home is not None else os.getenv(KAGWEB_HOME_ENV)

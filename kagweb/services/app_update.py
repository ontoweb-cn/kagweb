...
GITHUB_LATEST_RELEASE_URL = "https://api.github.com/repos/ontoweb-cn/kagweb/releases/latest"
GITHUB_LATEST_RELEASE_WEB_URL = "https://github.com/ontoweb-cn/kagweb/releases/latest"
...
def _version_tuple(value: str) -> tuple[int, int, int]:
    ...
    if match is None:
        raise ValueError(f"Invalid KAGWeb version: {value}")
...
def _running_in_container() -> bool:
    configured = os.getenv("KAGWEB_CONTAINER", "").strip().lower()
...
def _distribution_direct_url() -> dict[str, Any] | None:
    try:
        distribution = importlib.metadata.distribution("kagweb")
...
def detect_installation() -> Installation:
    ...
    if direct_url is None:
        return Installation(
            ...
            reason="The running KAGWeb distribution could not be identified.",
        )
            ...
            try:
                response = await client.get(
                    ...
                    headers={
                        ...
                        "User-Agent": "KAGWeb-Version-Check",
                    },
                )
    ...
    async def _fetch_latest_redirect(self, client: httpx.AsyncClient) -> ReleaseInfo:
        ...
        try:
            response = await client.head(
                self._latest_url,
                headers={"User-Agent": "KAGWeb-Version-Check"},
...
def _release_from_payload(payload: Any) -> ReleaseInfo:
    ...
    if not url.startswith("https://github.com/ontoweb-cn/kagweb/releases/"):
...
def _release_from_latest_url(value: str) -> ReleaseInfo:
    ...
    prefix = "/ontoweb-cn/kagweb/releases/tag/"
    if (
        parsed.scheme != "https"
...
class UpdateJobStore:
    ...
    def create(self, *, current_version: str, target_version: str) -> UpdateJob:
        ...
        if _version_tuple(target) <= _version_tuple(current):
            raise UpdateRequestError("No newer KAGWeb release is available")

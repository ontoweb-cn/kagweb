...
class ConfigTestRunner:
    ...
    async def _test_llm(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from kagweb.services.llm import clear_llm_config_cache, get_token_limit_kwargs
    ...
    def _test_search(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        run.emit("info", "Running search query: KAGWeb configuration health check")
        result = web_search("KAGWeb configuration health check", provider=provider)
        run.emit(
            "response",
    ...
    async def _test_tts(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from kagweb.services.config.provider_runtime import resolve_tts_runtime_config
        from kagweb.services.voice import synthesize_speech
        ...
        sample = "KAGWeb voice check. 这是一段语音合成测试。"
    ...
    async def _test_stt(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from kagweb.services.config.provider_runtime import resolve_stt_runtime_config
        from kagweb.services.voice import transcribe_audio
    ...
    async def _test_imagegen(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from kagweb.services.config.provider_runtime import resolve_imagegen_runtime_config
        from kagweb.services.imagegen import generate_image

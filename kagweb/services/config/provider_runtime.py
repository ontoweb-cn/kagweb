"""Nanobot-style normalized runtime configuration for KAGWeb."""
...
from kagweb.services.imagegen.config import ImagegenConfig
from kagweb.services.keypool import primary_api_key
from kagweb.services.model_selection import LLMSelection, apply_llm_selection_to_catalog
...
from kagweb.services.voice.config import (
    AUTH_API_KEY_HEADER,
    AUTH_BEARER,
    STT_BASE64_JSON,
    STT_MULTIPART,
...
class SearchProviderSpec:
    ...
    its own answer or needs :mod:`kagweb.services.search.consolidation` to
    ...
    The provider registry (``kagweb/services/search/providers``), the
    Settings dropdown (``kagweb/api/routers/settings.py``) and the web
...
def _with_personal_llm_profiles(catalog: dict[str, Any]) -> dict[str, Any]:
    ...
    the shared one (see :mod:`kagweb.multi_user.personal_models`), so
...
def _register_catalog_capabilities(catalog: dict[str, Any]) -> None:
    ...
    without a restart. Imported lazily: ``kagweb.services.llm`` imports this
    module for its config, so a module-level import would close a cycle.
    """
    from kagweb.services.llm.capabilities import set_catalog_capability_overrides
...
def resolve_llm_runtime_config(
    ...
    *,
    ...
    is the same shape and stands in for it on the calls KAGWeb makes on its

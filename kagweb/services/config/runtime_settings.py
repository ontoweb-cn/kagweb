...
# Document parsing settings. The parse layer (kagweb/services/parsing)
...
_DEFAULT_MINERU_ENGINE: dict[str, Any] = {
    ...
    # separate conda) so its heavy deps never conflict with KAGWeb's.
    "local_cli_path": "",
...
# KAGWeb's VLM to describe images.
...
# KAGWeb's legacy text extractors for PDF / Office / text-like files.
...
IGNORE_PROCESS_OVERRIDES_ENV = "KAGWEB_IGNORE_PROCESS_ENV_OVERRIDES"
...
class RuntimeSettingsService:
    ...
    def render_environment(self) -> dict[str, str]:
        ...
        return {
            "KAGWEB_VERSION_CHECK_ENABLED": _bool_env(system["version_check_enabled"]),
            ...
            # rewrites /api/* and /ws/* to KAGWEB_API_BASE_URL and uses
            # KAGWEB_AUTH_ENABLED to gate the login redirect. The launcher and
            # the Docker entrypoint both export these through render_environment,
            # so the two deployment paths stay in sync. KAGWEB_API_BASE_URL is
            ...
            "KAGWEB_AUTH_ENABLED": _bool_env(auth["enabled"]),
    ...
    def _apply_system_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if value := self._process_env_value("KAGWEB_VERSION_CHECK_ENABLED"):
        ...
        if value := (
            self._process_env_value("KAGWEB_BACKEND_WORKERS")
    ...
    def _apply_integrations_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if value := self._process_env_value("KAGWEB_TURN_COORDINATION_BACKEND"):
            coordination["backend"] = value
        if value := self._process_env_value("KAGWEB_REDIS_URL"):
            coordination["redis_url"] = value
        if value := self._process_env_value("KAGWEB_REDIS_KEY_PREFIX"):
    ...
    def _apply_llamaindex_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        # (KAGWEB_RAG_RETRIEVAL_PROFILE / RAG_RETRIEVAL_PROFILE); preserve it.
        payload = dict(settings)
        if value := (
            self._process_env_value("KAGWEB_RAG_RETRIEVAL_PROFILE")
    ...
    def _normalize_integrations(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if backend not in {"memory", "redis"}:
            backend = "memory"
        key_prefix = _string(coordination.get("key_prefix")).strip(":") or "kagweb"
        return {
            "version": 2,

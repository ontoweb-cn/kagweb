...
Unified service layer for KAGWeb providing:
...
Usage:
    ...
    from kagweb.services.config import load_config_with_main
...
def __getattr__(name: str):
    ...
    if name == "llm":
        return importlib.import_module("kagweb.services.llm")
    if name == "prompt":
        return importlib.import_module("kagweb.services.prompt")
    if name == "search":
        return importlib.import_module("kagweb.services.search")
    if name == "setup":
        return importlib.import_module("kagweb.services.setup")
    if name == "session":
        return importlib.import_module("kagweb.services.session")
    if name == "config":
        return importlib.import_module("kagweb.services.config")

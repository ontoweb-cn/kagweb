...
BUILTIN_PARTNER_COMMANDS: tuple[PartnerCommandSpec, ...] = (
    ...
    PartnerCommandSpec(
        "/link",
        "Connect this chat account to your KAGWeb account.",
...
class PartnerCommandHandler:
    ...
    def _link(self, msg: InboundMessage, args: list[str]) -> PartnerCommandResult:
        ...
        from kagweb.services.partners.links import redeem_link_code
        ...
        if not args:
            return PartnerCommandResult(
                "Usage: /link <code>. Open this partner in KAGWeb and choose "
        ...
        if not user_id:
            return PartnerCommandResult(
                ...
                "Generate a fresh one in KAGWeb and try again."
            )

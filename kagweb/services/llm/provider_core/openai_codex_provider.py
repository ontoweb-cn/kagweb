"""OpenAI Codex Responses provider backed by KAGWeb's private OAuth service."""
...
from kagweb.services.codex_auth import CodexAuthError, get_codex_oauth_service
from kagweb.services.codex_auth.constants import CODEX_DEFAULT_MODEL_ID, CODEX_RESPONSES_URL
from kagweb.services.codex_auth.contracts import CodexToken
from kagweb.services.llm.exceptions import LLMProviderTransportError
from kagweb.services.llm.openai_http_client import disable_ssl_verify_enabled
from kagweb.services.llm.provider_core.base import LLMProvider, LLMResponse, ToolCallRequest
from kagweb.services.llm.provider_core.openai_responses import (
    consume_sse,
    convert_messages,
    convert_tools,
)
from kagweb.services.llm.request_compat import is_transient_transport_error

DEFAULT_ORIGINATOR = "KAGWeb"
...
def _build_headers(account_id: str, token: str) -> dict[str, str]:
    if not token:
        raise RuntimeError(
            "OpenAI Codex is not logged in. Run `kagweb provider login openai-codex`."
        )
    headers = {
        "Authorization": f"Bearer {token}",
        "OpenAI-Beta": "responses=experimental",
        "originator": DEFAULT_ORIGINATOR,
        "User-Agent": "KAGWeb (python)",

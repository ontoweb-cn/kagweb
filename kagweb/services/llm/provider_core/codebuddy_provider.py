...
from kagweb.services.llm.provider_core.base import (
    LLMProvider,
    LLMResponse,
    ToolCallRequest,
)
...
class _CodeBuddySession:
    ...
    ``connect()`` and must exit them from the same task. KAGWeb chat turns
...
class CodeBuddyProvider(LLMProvider):
    ...
    endpoint. This adapter keeps one stateful CLI process per KAGWeb session
    and exposes KAGWeb's tool schemas through an in-process SDK MCP server.
    Tool execution remains owned by KAGWeb's existing dispatcher.
    """
    ...
    async def chat(
        ...
        **kwargs: Any,
        ...
        session_id = str(kwargs.pop("kagweb_session_id", "") or "").strip()
                ...
                "content": [
                    {
                        "type": "text",
                        "text": (
                            "KAGWeb accepted this tool call and will provide its result next."
                        ),
                    }
                ]
            }

        pending_result.__name__ = f"kagweb_{name}"
...
def _reasoning_options(reasoning_effort: str | None) -> dict[str, Any]:
    """Map KAGWeb reasoning levels to CodeBuddy SDK thinking controls.
...
def _friendly_error(exc: Exception) -> str:
    ...
    if "Authentication required" in message:
        return (
            ...
            "`kagweb provider login codebuddy`, run `codebuddy` and enter `/login`, "

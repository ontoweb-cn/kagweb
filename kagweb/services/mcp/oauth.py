...
sibling module :mod:`kagweb.services.mcp.secrets` holds static credentials in
...
#: How KAGWeb introduces itself on a consent screen.
CLIENT_NAME = "KAGWeb"
CLIENT_URI = "https://kagweb.info"
...
def oauth_redirect_uri(origin: str = "") -> str:
    ...
    *origin* is the origin the person is actually browsing, which the route that
    ...
    deployment work with no configuration. ``KAGWEB_PUBLIC_URL`` overrides it
...
def client_metadata(redirect_uri: str) -> "Any":
    """How KAGWeb registers itself with an authorization server."""

"""
Authentication service for KAGWeb.
...
Quick setup (single user via data/user/settings/auth.json):
    ...
    3. Generate a password hash:
           python -c "from kagweb.services.auth import hash_password; print(hash_password('yourpassword'))"
...
from kagweb.multi_user.models import AccountPreset, Role
from kagweb.services.config import load_auth_settings, load_integrations_settings
...
if AUTH_ENABLED and not POCKETBASE_ENABLED and not AUTH_SECRET:
    from kagweb.multi_user.identity import load_or_create_auth_secret
...
def _make_user_record(
    ...
    from kagweb.multi_user.identity import new_user_id
...
def delete_user(username: str) -> bool:
    ...
    """
    from kagweb.multi_user.identity import delete_user as _delete_user
...
def set_role(username: str, role: str) -> bool:
    ...
    from kagweb.multi_user.identity import set_role as _set_role
...
def set_avatar(username: str, avatar: str) -> bool:
    ...
    """
    from kagweb.multi_user.identity import set_avatar as _set_avatar
...
def decode_token(token: str) -> TokenPayload | None:
    ...
    if POCKETBASE_ENABLED:
        from kagweb.services.pocketbase_client import validate_pb_token
...
def authenticate_pb(username: str, password: str) -> tuple[TokenPayload, str] | None:
    ...
    <username>@kagweb.local to match the email used at registration.
    """
    try:
        from kagweb.services.pocketbase_client import get_pb_client

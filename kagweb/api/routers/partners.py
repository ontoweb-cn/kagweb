...
from kagweb.multi_user.partner_access import (
...
from kagweb.partners.config.paths import get_partner_media_dir
...
from kagweb.services.partners import (
    get_partner_manager,
    slugify_partner_id,
    slugify_soul_id,
)
...
from kagweb.services.partners.drafts import PartnerDraftStore
...
from kagweb.services.partners.runtime_status import (
    get_partner_runtime_status_repository,
)
from kagweb.services.partners.workspace import (
    list_assets,
    provision_assets,
    read_soul,
    remove_asset,
...
def _validate_channels_payload(channels: dict) -> None:
    ...
    from kagweb.api.routers._partners_channel_schema import resolve_config_model
    from kagweb.partners.channels.registry import discover_all
...
def _validate_llm_selection_payload(
    ...
    from kagweb.services.model_selection import apply_llm_selection_to_catalog
    from kagweb.services.partners.model_runtime import normalize_partner_llm_selection
...
def _caller_tool_reach() -> tuple[set[str] | None, set[str] | None]:
    ...
    from kagweb.multi_user.tool_access import allowed_mcp_tools, allowed_optional_tools
...
def _resolve_soul_content(soul: SoulSpec | None) -> tuple[str, dict[str, str]]:
    ...
    from kagweb.services.partners.workspace import DEFAULT_SOUL
...
async def start_partner(partner_id: str):
    ...
    # comes back in this state after a KAGWeb restart (a manual /stop clears
...
async def create_partner_link_code(partner_id: str):
    ...
    """
    from kagweb.services.partners.links import issue_link_code
...
async def list_partner_links(partner_id: str):
    ...
    from kagweb.services.partners.links import list_links
...
async def delete_partner_link(partner_id: str, key: str):
    ...
    from kagweb.services.partners.links import remove_link
...
async def partner_command_palette():
    from kagweb.services.partners.commands import partner_command_palette
...
def _partner_upload_caps() -> tuple[int, int]:
    try:
        from kagweb.services.config.runtime_settings import get_chat_attachment_limits
...
async def _partner_chat_stream(
    ...
    from kagweb.core.stream import StreamEventType
...
async def partner_chat_ws(ws: WebSocket, partner_id: str):
    ...
    * ``{"type": "done"}`` / ``{"type": "error"}`` / ``{"type": "proactive"}``.
    """
    from kagweb.api.routers.auth import ws_auth_failed, ws_require_auth
    from kagweb.multi_user.context import get_current_user_or_none, reset_current_user
    from kagweb.services.partners.interaction import personal_actor_id

...
from kagweb.api.contracts.turn_protocol import (
...
def get_llm_config(*args, **kwargs):
    from kagweb.services.llm.config import get_llm_config as resolve

    return resolve(*args, **kwargs)


def get_token_limit_kwargs(*args, **kwargs):
    from kagweb.services.llm.config import get_token_limit_kwargs as resolve

    return resolve(*args, **kwargs)


async def llm_complete(*args, **kwargs):
    from kagweb.services.llm import complete

    return await complete(*args, **kwargs)


def web_search(*args, **kwargs):
    from kagweb.services.search import web_search as search
...
async def request_managed_update(_request: ManagedUpdateRequest) -> dict[str, Any]:
    ...
    if not launcher_available():
        raise HTTPException(
            status_code=409,
            detail="Web updates require KAGWeb to be running under `kagweb start`.",
        )
    installation = get_update_installation()
    ...
    if not result.update_available:
        raise HTTPException(status_code=409, detail="No newer KAGWeb release is available")
    ...
    if job is None:
        raise HTTPException(
            status_code=409,
            detail="Finish the active conversation before updating KAGWeb.",
        )
...
async def get_memory_usage():
    """Resident memory of the running KAGWeb process tree.
...
async def test_search_connection():
    ...
    try:
        ...
        result = web_search("KAGWeb health check", provider=search_config.provider)
        response_time = (time.time() - start_time) * 1000

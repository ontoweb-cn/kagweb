...
from kagweb.services.llm.config import clear_llm_config_cache
from kagweb.services.model_selection import list_llm_options
...
def _provider_choices() -> dict[str, list[dict[str, Any]]]:
    ...
    from kagweb.services.provider_registry import PROVIDERS
...
def _mineru_settings_payload() -> dict[str, Any]:
    ...
    """
    from kagweb.services.parsing.engines.mineru.backend import local_cli_probe
...
def _document_parsing_payload() -> dict[str, Any]:
    ...
    from kagweb.services.parsing.engines.factory import (
        get_parser,
        list_engines,
    )
    from kagweb.services.parsing.engines.mineru.backend import local_cli_probe
...
async def test_document_parsing(payload: DocumentParsingTest):
    ...
    _require_settings_admin()
    from kagweb.services.parsing.engines.factory import get_parser, is_engine_available
...
async def test_docling_remote_connection(payload: DoclingRemoteTest):
    ...
    is untouched."""
    _require_settings_admin()
    from kagweb.services.parsing.engines.docling.config import (
        DoclingConfig,
        resolve_docling_config,
    )
    from kagweb.services.parsing.engines.docling.remote import verify_remote
...
async def test_tika_remote_connection(payload: TikaRemoteTest):
    ...
    from kagweb.services.parsing.engines.tika.remote import verify_remote
    ...
    if not cmd:
        return {
            ...
            "message": (
                ...
                f"(pip install kagweb[parse-{engine}]) so its CLI is on PATH."
            ),
        }
...
async def mineru_models_download_status(cursor: int = 0):
    _require_settings_admin()
    from kagweb.services.parsing.engines.mineru.models import get_model_download_manager
...
async def test_mineru_connection(payload: MinerUSettingsUpdate):
    ...
    _require_settings_admin()
    from kagweb.services.parsing.engines.mineru.cloud import verify_credentials
    from kagweb.services.parsing.engines.mineru.config import MinerUConfig, MinerUError

    if payload.mode == "local":
        from kagweb.services.parsing.engines.mineru.backend import (
            local_cli_probe,
            local_cli_version,
        )
        ...
        if not mineru_version_is_current(version):
            ...
            return {
                "ok": False,
                "message": (
                    f"Local MinerU CLI reported {detail}. KAGWeb needs MinerU >= "
...
async def fetch_models_from_provider(payload: FetchModelsPayload):
    ...
    """
    _require_settings_admin()
    from kagweb.services.llm.factory import fetch_models as fetch_llm_models
...
async def resolve_model_capabilities(payload: ModelCapabilitiesQuery):
    ...
    """
    _require_settings_admin()
    from kagweb.services.llm.capabilities import effective_capabilities
...
async def complete_tour(payload: TourCompletePayload | None = None):
    ...
    return {
        "status": "completed",
        "message": "Configuration saved. KAGWeb will restart shortly.",
        "launch_at": launch_at,
